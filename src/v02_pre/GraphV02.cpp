...
bool GraphV02::route_agreement_votes_for_node(
    ...
    if (!query_key.empty() && candidate_keys.size() == vote_positions.size()) {
        ...
        for (std::size_t candidate_index = 0; candidate_index < vote_positions.size();
            ...
            const double score =
                KeySignatureCoreV02::key_shape_score(query_key, candidate_key);
    ...
    if (source_id.rfind("retry-", 0) == 0) {
        ...
        if (retry_source == "key-shape") {
            return "retry-source:key-shape|len:" +
                   ...
                   "|digits:" +
                   std::to_string(KeySignatureCoreV02::digit_count(query_key));
        }
                ...
                std::stable_sort(
                    ...
                    [&query](const ByteDataset::KVRecord* lhs,
                        ...
                        const double lhs_score =
                            KeySignatureCoreV02::key_shape_score(query.key, lhs->key);
                        const double rhs_score =
                            KeySignatureCoreV02::key_shape_score(query.key, rhs->key);
            ...
            [this, &query, span_offset](
                ...
                const auto span_route_code_score =
                    [this, &query_code_signature](
                        ...
                        return KeySignatureCoreV02::byte_signature_shape_score(
