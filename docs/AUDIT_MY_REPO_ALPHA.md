...
Each benchmark output writes `benchmark_manifest.json` and `benchmark_sha256sums.txt`. The manifest binds the benchmark runner source sha, audit entrypoint sha, local verifier sha, label source kind, label input sha, optional label-intake manifest shas, optional feedback input sha, mode, namespace, budgets, case audit manifests, repo snapshot gate counts, readiness thresholds, and blocked readiness flags. Benchmark runs require a fresh output directory by default; `--overwrite` replaces benchmark-managed artifacts, including the managed `case_runs/` tree, refuses to delete unrelated output-root files, and restores existing managed artifacts if the benchmark run fails before final verification. `--verify-existing` schema-validates the benchmark JSON artifacts, recomputes those bindings, re-verifies any bound label-intake bundle, and re-verifies every per-case audit output.
