...
| Offline suite | `offline-suite.yml` -> `offline-suite` (10-shard matrix) | GitHub-hosted `ubuntu-latest` | push to `main`, manual | the deterministic, hardware-free `experiments/test_*.sh` (~272 tests), sharded in parallel |
...
references) mentions GPU/accelerator or NVMe hardware. The
`offline-suite.yml` lane is intentionally **not** a required PR check yet; once
shard timing is confirmed it can be promoted to a required status check on
PRs to `main`.
