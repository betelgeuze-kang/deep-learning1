...
v52i: A/B/G/H same-query measured 1000-row packet is implemented and covered by `experiments/test_v52i_abgh_same_query_measured_1000.sh`. The runner assembles `results/v52i_abgh_same_query_measured_1000/measured_001/`, reuses or reruns the full v53e 1000-row canary query scale, writes shared `frozen_query_rows.csv`, `frozen_source_span_rows.csv`, `source_manifest_rows.csv`, `abgh_system_rows.csv`, `abgh_answer_rows.csv`, `abgh_citation_rows.csv`, `abgh_retrieval_rows.csv`, `abgh_abstain_rows.csv`, `abgh_wrong_answer_guard_rows.csv`, `abgh_resource_rows.csv`, `routehint_rows.csv`, `abgh_system_metric_rows.csv`, copied v53e evidence, `V52I_ABGH_SAME_QUERY_BOUNDARY.md`, `v52i_abgh_same_query_measured_1000_manifest.json`, and `sha256_manifest.csv`. The smoke verifies `v52i_abgh_same_query_measured_1000_ready=1`, four local systems A/B/G/H, 1000 shared query rows, 4000 answer/citation/abstain/wrong-answer/resource rows, 12000 retrieval rows, 2000 G/H RouteHint rows with raw context appending disabled, same query/source manifest coverage, local/no-external-model execution, and local v52 absorb readiness, while keeping C/D/E evidence, 30B/70B baselines, full v52, and release claims blocked.
...
v61t: local checkpoint materialization verifier is implemented and covered by `experiments/test_v61t_local_checkpoint_materialization_verifier.sh` plus `experiments/test_v61t_local_checkpoint_materialization_verifier_target_override.sh`. The runner refreshes v61p, binds v61q and v61r, and writes `local_checkpoint_materialization_rows.csv`, `sampled_local_page_hash_verification_rows.csv`, `local_checkpoint_materialization_metric_rows.csv`, materialization gap rows, boundary, manifest, copied v61p/v61q/v61r/v61o evidence, and hash rows. It verifies local shards only when exact byte length, safetensors header hash, and sampled page hash checks pass; the current host records 0 local existing shards, 0 header-hash matches, 0 sampled local page-hash matches, `local_checkpoint_materialization_ready=0`, `full_safetensors_page_hash_binding_ready=0`, and `real_checkpoint_weight_bytes_materialized=0`. The target-override smoke verifies that `V61T_WAREHOUSE_ROOT` is passed through to the v61p shard-presence preflight and all materialization target paths, while keeping full checkpoint materialization, real Mixtral generation, near-frontier quality, production-latency, and release claims blocked.
...
v61bu: ubuntu-1 partial checkpoint materialization witness is implemented and covered by `experiments/test_v61bu_ubuntu1_partial_checkpoint_materialization_witness.sh`. The runner consumes v61bq and v61t, copies the live target presence and local materialization verifier evidence, and writes partial witness rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies 59 checkpoint shard rows, 1 live existing shard, 1 live size-match shard, 1 local existing shard, 1 local size-match shard, 1 safetensors-header hash match, 1 identity-verified shard, `local_identity_verified_bytes=4932529864`, `remaining_identity_unverified_shard_rows=58`, `partial_checkpoint_materialization_witness_ready=1`, `full_checkpoint_materialization_ready=0`, `full_safetensors_page_hash_binding_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61bu=0`, `observed_external_checkpoint_payload_bytes=4932529864`, and zero checkpoint payload bytes committed to the repo, while keeping receipt-backed full materialization, full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61bv: ubuntu-1 remaining checkpoint materialization queue is implemented and covered by `experiments/test_v61bv_ubuntu1_remaining_checkpoint_materialization_queue.sh`. The runner consumes v61bp and v61bu, skips already identity-verified shards, rewrites the remaining ubuntu-1 dry-run-first materialization queue, and writes remaining queue rows, verified-skip rows, chunk rows, requirement rows, metric rows, script probe rows, dry-run probe rows, operator file rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies 59 checkpoint shard rows, 1 verified/skip shard, 58 remaining queue rows, three remaining priority chunks, `remaining_unverified_bytes=276308963480`, `local_identity_verified_bytes=4932529864`, `remaining_bytes_fit_current_free_space=1`, `remaining_queue_ready=1`, `dry_run_guard_ready=1`, `payload_execution_launch_ready=0`, `download_execution_ready=0`, `full_checkpoint_materialization_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61bv=0`, and zero checkpoint payload bytes committed to the repo, while keeping explicit payload execution, receipt-backed full materialization, full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61bw: ubuntu-1 partial page-hash witness is implemented and covered by `experiments/test_v61bw_ubuntu1_partial_page_hash_witness.sh`. The runner consumes v61bu and v61q, reads every page of the current identity-verified ubuntu-1 shard, and writes partial page-hash witness rows, shard status rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies 59 checkpoint shard rows, 134161 total checkpoint page rows, 1 identity-verified local shard, `local_identity_verified_bytes=4932529864`, `identity_shard_page_rows=2353`, `identity_shard_page_bytes=4932529864`, `page_hash_witness_rows=2353`, `page_hash_witness_bytes=4932529864`, `partial_full_shard_page_hash_ready=1`, `full_safetensors_page_hash_binding_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61bw=0`, and zero checkpoint payload bytes committed to the repo, while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61bx: ubuntu-1 page-hash coverage ledger is implemented and covered by `experiments/test_v61bx_ubuntu1_page_hash_coverage_ledger.sh`. The runner consumes v61bw, v61bv, and v61q, promotes the partial page-hash witness into a 59-shard full-checkpoint coverage ledger, and writes ledger rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies 59 checkpoint shard rows, 134161 total checkpoint page rows, 1 verified page-hash shard, `verified_page_hash_rows=2353`, `verified_page_hash_bytes=4932529864`, `remaining_page_hash_shard_rows=58`, `remaining_page_hash_rows=131808`, `remaining_page_hash_bytes=276308963480`, `remaining_materialization_queue_rows=58`, `partial_page_hash_coverage_ledger_ready=1`, `full_safetensors_page_hash_binding_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61bx=0`, and zero checkpoint payload bytes committed to the repo, while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61by: ubuntu-1 remaining page-hash execution plan is implemented and covered by `experiments/test_v61by_ubuntu1_remaining_page_hash_execution_plan.sh`. The runner consumes v61bx and v61bv, skips page-hashed shards, emits remaining-only guarded page-hash execution chunks, and writes chunk rows, skip rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies 59 checkpoint shard rows, 134161 total checkpoint page rows, `verified_page_hash_rows=2353`, `skipped_verified_page_hash_rows=2353`, `remaining_page_hash_rows=131808`, `remaining_page_hash_bytes=276308963480`, `remaining_page_hash_execution_chunk_size_pages=512`, `remaining_page_hash_execution_chunk_rows=286`, `remaining_page_hash_execution_plan_ready=1`, `full_safetensors_page_hash_binding_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61by=0`, and zero checkpoint payload bytes committed to the repo, while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61bz: ubuntu-1 remaining page-hash operator bundle is implemented and covered by `experiments/test_v61bz_ubuntu1_remaining_page_hash_operator_bundle.sh`. The runner consumes v61by, mirrors the remaining page-hash chunks and verified skip rows into a dry-run-first operator bundle, and writes operator scripts, env template, result schema rows, requirement rows, metric rows, dry-run probe rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies `v61by_ubuntu1_remaining_page_hash_execution_plan_ready=1`, `verified_page_hash_rows=2353`, `skipped_verified_page_hash_rows=2353`, `remaining_page_hash_rows=131808`, `remaining_page_hash_bytes=276308963480`, `remaining_page_hash_execution_chunk_rows=286`, `operator_bundle_file_rows=7`, `script_probe_rows=2`, `script_bash_syntax_pass_rows=2`, `dry_run_guard_ready=1`, `remaining_page_hash_operator_bundle_ready=1`, `page_hash_execution_ready=0`, `full_safetensors_page_hash_binding_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61bz=0`, and zero checkpoint payload bytes committed to the repo, while keeping explicit remaining page-hash execution, completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61ca: ubuntu-1 remaining page-hash result intake is implemented and covered by `experiments/test_v61ca_ubuntu1_remaining_page_hash_result_intake.sh`. The runner consumes v61bz, defines the hash-only result schema for `remaining_page_hash_result_rows.csv`, preserves existing verified page-hash rows, records missing/default-deferred chunk status rows when no result artifact is supplied, and writes required field rows, template rows, validation rows, invalid rows, chunk status rows, preservation rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies `v61bz_ubuntu1_remaining_page_hash_operator_bundle_ready=1`, `page_hash_result_input_supplied=0`, `expected_remaining_page_hash_result_rows=131808`, `accepted_remaining_page_hash_result_rows=0`, `missing_remaining_page_hash_result_rows=131808`, `existing_verified_page_hash_rows=2353`, `total_required_page_hash_rows=134161`, `total_verified_page_hash_rows=2353`, `remaining_page_hash_execution_chunk_rows=286`, `remaining_page_hash_result_intake_ready=0`, `completed_full_safetensors_page_hash_coverage_ready=0`, `full_safetensors_page_hash_binding_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61ca=0`, and zero checkpoint payload bytes committed to the repo, while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61cb: ubuntu-1 full page-hash coverage promotion gate is implemented and covered by `experiments/test_v61cb_ubuntu1_full_page_hash_coverage_promotion_gate.sh`. The runner consumes v61ca, aggregates the result intake into shard-level promotion rows, preserves the existing verified shard, and keeps remaining shards blocked until accepted page-hash result rows cover every page. The smoke verifies `v61ca_ubuntu1_remaining_page_hash_result_intake_ready=1`, `checkpoint_shard_rows=59`, `ready_full_page_hash_shard_rows=1`, `blocked_full_page_hash_shard_rows=58`, `existing_verified_page_hash_shard_rows=1`, `remaining_page_hash_shard_rows=58`, `accepted_remaining_page_hash_result_rows=0`, `missing_remaining_page_hash_result_rows=131808`, `existing_verified_page_hash_rows=2353`, `total_required_page_hash_rows=134161`, `total_verified_page_hash_rows=2353`, `full_page_hash_coverage_promotion_ready=0`, `completed_full_safetensors_page_hash_coverage_ready=0`, `full_safetensors_page_hash_binding_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cb=0`, and zero checkpoint payload bytes committed to the repo, while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61cc: ubuntu-1 page-hash generation admission bridge is implemented and covered by `experiments/test_v61cc_ubuntu1_page_hash_generation_admission_bridge.sh`. The runner consumes v61cb page-hash promotion, v53t complete-source audit readiness, and v61bt actual generation result intake schema, then emits 1000 complete-source generation admission bridge rows. The smoke verifies `v61cb_ubuntu1_full_page_hash_coverage_promotion_gate_ready=1`, `v53t_complete_source_audit_readiness_gate_ready=1`, `v61bt_ubuntu1_actual_generation_result_intake_ready=1`, `complete_source_query_rows=1000`, `generation_admission_bridge_rows=1000`, `machine_complete_source_surface_ready=1`, `complete_source_review_return_ready=0`, `full_safetensors_page_hash_binding_ready=0`, `total_verified_page_hash_rows=2353`, `total_required_page_hash_rows=134161`, `generation_execution_admitted_rows=0`, `page_hash_blocked_rows=1000`, `review_return_blocked_rows=1000`, `generation_result_artifact_blocked_rows=1000`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cc=0`, and zero checkpoint payload bytes committed to the repo, while keeping completed full page-hash coverage, complete-source review return, actual generation, production-latency, near-frontier, and release claims blocked.

v61cd: ubuntu-1 generation unblocker closure bundle is implemented and covered by `experiments/test_v61cd_ubuntu1_generation_unblocker_closure_bundle.sh`. The runner consumes v61cc, v61ca, v53s, and v61bt, then emits three ordered closure phases, 11 required return artifact rows, seven operator command rows, an operator README, a return manifest template, and a shape verification script. The smoke verifies `v61cc_ubuntu1_page_hash_generation_admission_bridge_ready=1`, `closure_phase_rows=3`, `return_artifact_rows=11`, `operator_command_rows=7`, `page_hash_return_required_rows=131808`, `page_hash_return_accepted_rows=0`, `human_review_required_rows=7000`, `human_review_accepted_rows=0`, `adjudication_required_rows=1000`, `adjudication_accepted_rows=0`, `generation_result_required_artifacts=5`, `generation_result_accepted_artifacts=0`, `generation_execution_admitted_rows=0`, `generation_unblocker_closure_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cd=0`, and zero checkpoint payload bytes committed to the repo, while keeping completed full page-hash coverage, complete-source review return, actual generation, production-latency, near-frontier, and release claims blocked.

v61ce: ubuntu-1 generation closure return intake is implemented and covered by `experiments/test_v61ce_ubuntu1_generation_closure_return_intake.sh`. The runner consumes v61cd, v61cb, v53t, v61bt, and v61cc, then rechecks remaining page-hash return, complete-source review return, and actual generation result return as three closure gates plus 1000 generation closure admission rows. The smoke verifies `v61cd_ubuntu1_generation_unblocker_closure_bundle_ready=1`, `closure_gate_rows=3`, `generation_closure_admission_rows=1000`, `page_hash_return_required_rows=131808`, `page_hash_return_accepted_rows=0`, `total_required_page_hash_rows=134161`, `total_verified_page_hash_rows=2353`, `human_review_required_rows=7000`, `human_review_accepted_rows=0`, `adjudication_required_rows=1000`, `adjudication_accepted_rows=0`, `generation_result_required_artifacts=5`, `generation_result_accepted_artifacts=0`, `accepted_generation_rows=0`, `generation_closure_return_intake_ready=0`, `generation_execution_admitted_rows=0`, `page_hash_blocked_rows=1000`, `review_return_blocked_rows=1000`, `generation_result_artifact_blocked_rows=1000`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61ce=0`, and zero checkpoint payload bytes committed to the repo, while keeping completed full page-hash coverage, complete-source review return, actual generation, production-latency, near-frontier, and release claims blocked.

v61cf: ubuntu-1 source-bound generation execution packet is implemented and covered by `experiments/test_v61cf_ubuntu1_source_bound_generation_execution_packet.sh`. The runner consumes v61ce, v53r, and v61bt, then emits 1000 source-bound generation execution packet rows, four prompt contract rows, five return artifact rows, six operator command rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies `v61ce_ubuntu1_generation_closure_return_intake_ready=1`, `execution_packet_rows=1000`, `prompt_manifest_rows=4`, `return_manifest_rows=5`, `operator_command_rows=6`, `complete_source_query_rows=1000`, `expected_generation_result_artifacts=5`, `generation_closure_return_intake_ready=0`, `generation_execution_admission_ready=0`, `generation_execution_ready=0`, `generation_execution_admitted_rows=0`, `blocked_execution_rows=1000`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cf=0`, and zero checkpoint payload bytes committed to the repo, while keeping actual generation, production-latency, near-frontier, and release claims blocked.

v61cg: ubuntu-1 source-bound generation operator bundle is implemented and covered by `experiments/test_v61cg_ubuntu1_source_bound_generation_operator_bundle.sh`. The runner consumes v61cf, then emits four operator bundle files, four bundle command rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied v61cf evidence, hash rows, and an executable `operator_bundle/VERIFY_EXECUTION_PACKET.sh` verifier. The smoke verifies `v61cf_ubuntu1_source_bound_generation_execution_packet_ready=1`, `execution_packet_rows=1000`, `prompt_manifest_rows=4`, `return_manifest_rows=5`, `carried_operator_command_rows=6`, `bundle_operator_command_rows=4`, `total_operator_command_rows=10`, `operator_bundle_file_rows=4`, `operator_bundle_handoff_ready=1`, `generation_execution_ready=0`, `blocked_execution_rows=1000`, `generation_operator_execution_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cg=0`, and zero checkpoint payload bytes committed to the repo, while keeping actual generation, production-latency, near-frontier, and release claims blocked.

v61ch: real-model page manifest release index is implemented and covered by `experiments/test_v61ch_real_model_page_manifest_release_index.sh`. The runner consumes v61ao, v61cb, and v61cg, then emits a zero-payload `release_index/` with `MANIFEST_INDEX.csv`, shard audit rows, tensor-role coverage rows, 1344-row MoE layer/expert/tensor coverage rows, page-hash status rows, generation handoff status rows, a zero-payload boundary, import checklist, executable `VERIFY_RELEASE_INDEX.sh`, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies `redistributable_manifest_index_ready=1`, 8 source artifact rows, 10 release index files, 59 shards, 134161 manifest pages, 135841 tensor/page segment rows, 1344/1344 MoE coverage rows, 2353/134161 verified page hashes, 131808 remaining page hashes, `operator_bundle_handoff_ready=1`, `generation_operator_execution_ready=0`, `actual_model_generation_ready=0`, `redistributed_checkpoint_payload_bytes=0`, `checkpoint_payload_bytes_downloaded_by_v61ch=0`, and zero checkpoint payload bytes committed to the repo, while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61ci: real manifest runtime substitution gate is implemented and covered by `experiments/test_v61ci_real_manifest_runtime_substitution_gate.sh`. The runner consumes v61j, v61k, and v61ch, then maps the logical SSD-resident fixture surfaces onto the real Mixtral zero-payload manifest/index through four logical-fixture replacement contract rows and five runtime substitution binding rows. It writes replacement contract rows, runtime binding rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies `logical_fixture_replaced_by_real_manifest_ready=1`, `zero_payload_runtime_input_ready=1`, 134161 manifest pages, 135841 tensor/page segment rows, 1344/1344 MoE coverage rows, 2353/134161 verified page hashes, 131808 remaining page hashes, `runtime_execution_admission_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61ci=0`, and zero checkpoint payload bytes committed to the repo, while keeping full page-hash coverage, real runtime execution, actual generation, production-latency, near-frontier, and release claims blocked.

v61cj: real manifest immediate target bridge is implemented and covered by `experiments/test_v61cj_real_manifest_immediate_target_bridge.sh`. The runner consumes v61ci, v61l, v61m, v61s, and v61n source rows, then binds the four immediate v61 real-model targets into `real_manifest_immediate_target_rows.csv` and `real_manifest_runtime_evidence_bridge_rows.csv`. The smoke verifies 4/4 ready immediate target rows, 3/3 ready runtime bridge rows, `real_manifest_immediate_target_bridge_ready=1`, `logical_fixture_replaced_by_real_manifest_ready=1`, ROCm page-kernel timing `gpu_kernel_avg_ms=0.513442`, `kv_cache_policy_ready=1`, source-bound command pass 37/37, `complete_source_1000_query_ready=0`, `completed_full_safetensors_page_hash_coverage_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cj=0`, and zero repo payload bytes, while keeping full page-hash coverage, complete-source 1000-query generation, production-latency, near-frontier, and release claims blocked.

v61ck: real generation unblocker operator matrix is implemented and covered by `experiments/test_v61ck_real_generation_unblocker_operator_matrix.sh`. The runner consumes v61cj, v61bv, v61bz, v61ca, v61cb, v61cm, v61cn, v61co, v61cq, v61cr, v53u, v61bt, and v61cg, then emits a seven-row unblocker matrix, eleven-row operator execution order, claim boundary rows, operator matrix files, requirement/metric/boundary/manifest/hash rows, and copied source evidence. The smoke verifies 7/7 ready operator surfaces, 58 remaining checkpoint materialization queue rows, 59 checkpoint materialization promotion rows with 1 ready shard and 58 blocked shards, 286 page-hash execution admission rows with 0 admitted and 286 materialization-blocked chunks, 37 runtime execution admission candidate rows with 0 admitted, 1000 complete-source runtime admission expansion rows, 1000 complete-source runtime admission return rows with 0 accepted, 5 missing runtime admission return artifacts, 276308963480 missing materialization/page-hash bytes, 131808 remaining page-hash rows with 0 accepted, 7000 human review rows with 0 accepted, 1000 adjudication rows with 0 accepted, 5 generation result artifacts with 0 accepted, `generation_unblocker_operator_matrix_ready=1`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61ck=0`, and zero repo payload bytes, while keeping full materialization, full page-hash coverage, runtime execution admission, complete-source runtime admission execution, review return, actual generation, production-latency, near-frontier, and release claims blocked.

v61cl: ubuntu-1 remaining checkpoint materialization return intake is implemented and covered by `experiments/test_v61cl_ubuntu1_remaining_checkpoint_materialization_return_intake.sh`. The runner consumes v61bv, defines the metadata-only return schema for 58 remaining shard materialization receipts, preserves the one existing identity-verified shard, and writes required-field rows, template rows, invalid-row diagnostics, queue/chunk status rows, preservation rows, validation/requirement/metric/runtime-gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies 58 expected/missing return rows, 276308963480 expected/missing materialization bytes, one existing identity-verified shard, three remaining chunks, `remaining_materialization_return_intake_ready=0`, `full_checkpoint_materialization_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cl=0`, and zero checkpoint payload bytes committed to the repo, while keeping completed full checkpoint materialization, full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61cm: ubuntu-1 full checkpoint materialization promotion gate is implemented and covered by `experiments/test_v61cm_ubuntu1_full_checkpoint_materialization_promotion_gate.sh`. The runner consumes v61cl, aggregates accepted materialization returns and existing identity-verified shard evidence into 59 shard-level promotion rows, and writes promotion rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies 59 checkpoint shard rows, 1 ready identity-verified shard, 58 blocked remaining shards, 58 expected/missing materialization return rows, 276308963480 expected/missing materialization bytes, `total_identity_verified_checkpoint_shard_rows=1`, `full_checkpoint_materialization_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cm=0`, and zero checkpoint payload bytes committed to the repo, while keeping full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61cn: ubuntu-1 page-hash execution materialization admission gate is implemented and covered by `experiments/test_v61cn_ubuntu1_page_hash_execution_materialization_admission_gate.sh`. The runner consumes v61bz and v61cm, binds the 286 remaining page-hash execution chunks to full-checkpoint materialization promotion rows, and writes admission rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies 286 remaining page-hash execution chunks, 0 admitted chunks, 286 materialization-blocked chunks, 131808 blocked page-hash rows, 276308963480 blocked page-hash bytes, `page_hash_execution_admission_ready=0`, `page_hash_execution_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cn=0`, and zero checkpoint payload bytes committed to the repo, while keeping page-hash execution, completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.

v61co: real manifest runtime execution admission bridge is implemented and covered by `experiments/test_v61co_real_manifest_runtime_execution_admission_bridge.sh`. The runner consumes v61cj, v61ci, v61cm, v61cn, v61n, and v61s, then maps the 37 source-bound QA seed rows onto the real-manifest runtime prerequisites. It writes runtime execution admission rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies 37 runtime execution candidate rows, 0 admitted rows, 37 materialization-blocked runtime rows, 37 page-hash-admission-blocked runtime rows, 37/37 source-bound query pass rows, one ready checkpoint shard, 58 blocked checkpoint shards, 0/286 admitted page-hash execution chunks, 131808 blocked page-hash rows, 276308963480 blocked page-hash bytes, `real_manifest_runtime_execution_admission_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61co=0`, and zero checkpoint payload bytes committed to the repo, while keeping real runtime execution, actual generation, production-latency, near-frontier, and release claims blocked.

v61cp: complete-source runtime admission coverage gate is implemented and covered by `experiments/test_v61cp_complete_source_runtime_admission_coverage_gate.sh`. The runner consumes v61co, v61cf, and v61cc, then maps every 1000-row complete-source generation packet row against the 37-row real-manifest seed runtime admission bridge. It writes complete-source runtime admission coverage rows, requirement rows, metric rows, runtime gap rows, boundary, manifest, copied source evidence, and hash rows. The smoke verifies 1000 complete-source query rows, 37 seed runtime candidate rows, 37/37 seed query pass rows, direct query overlap 0/1000, 1000 seed-uncovered complete-source rows, 0 complete-source runtime admitted rows, `complete_source_runtime_admission_coverage_ready=0`, 1000 page-hash/review/generation-result blocked rows inherited from v61cc, `checkpoint_payload_bytes_downloaded_by_v61cp=0`, and zero checkpoint payload bytes committed to the repo, while keeping complete-source real-model generation, production-latency, near-frontier, and release claims blocked.
...
v61cr: complete-source runtime admission return intake is implemented and covered by `experiments/test_v61cr_complete_source_runtime_admission_return_intake.sh`. The runner consumes v61cq, defines the five-artifact return surface for complete-source runtime admission, and records the default no-return path as missing/default-deferred. The smoke verifies `v61cq_complete_source_runtime_admission_expansion_packet_ready=1`, 1000 complete-source query rows, 1000 runtime admission expansion rows, five expected runtime admission return artifacts, zero accepted return artifacts, five missing return artifacts, 1000 expected runtime admission result rows, zero accepted runtime admission result rows, `complete_source_runtime_admission_execution_ready=0`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cr=0`, and zero checkpoint payload bytes committed to the repo, while keeping complete-source runtime admission execution, actual generation, production-latency, near-frontier, and release claims blocked.
