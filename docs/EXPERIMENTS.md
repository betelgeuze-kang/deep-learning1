...
v40: machine-verified research artifact over v36-v39 is implemented and covered by `experiments/test_v40_machine_verified_research_artifact.sh`. The runner assembles `results/v40_machine_verified_research_artifact/artifact_001/`, copies the v36 release-claim audit, v37 no-return human-review intake state, v38 dispatch bundle evidence, and v39 transfer archive evidence, then writes `MACHINE_VERIFIED_RESEARCH_ARTIFACT.md`, `release_mode_rows.csv`, `allowed_claim_rows.csv`, `blocked_claim_rows.csv`, `evidence_index.csv`, `v40_machine_verified_research_artifact_manifest.json`, `artifact_manifest.csv`, and `sha256_manifest.csv`. The smoke verifies `v40_machine_verified_research_artifact_ready=1`, `automated_research_artifact_ready=1`, and `machine_verified_prototype_ready=1` while explicitly keeping `human_review_completed=0`, `human_review_required_for_public_release=1`, and `real_release_package_ready=0`; human-reviewed release, production readiness, Transformer/general LLM replacement, frontier long-context, GPU acceleration, and full commercial deployment claims remain blocked.
