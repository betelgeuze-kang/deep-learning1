...
v53t: Complete source audit readiness gate is implemented and covered by `experiments/test_v53t_complete_source_audit_readiness_gate.sh`. The runner consumes v52y/v53i/v53ap/v53q/v53r/v53s, copies the relevant source summaries plus direct v53i query/span rows and direct v53ap answer/citation/evaluator/resource/adapter-trace rows, writes complete-source audit requirement, PM freeze check, foundation freeze certificate, claim, metric, boundary, manifest, and hash rows, and records `machine_complete_source_surface_ready=1`, `pm_v53_freeze_ready=1`, `foundation_freeze_certificate_rows=10`, `foundation_machine_freeze_ready=1`, and `foundation_direct_evidence_ready=1`. The certificate binds 10 pinned public repositories, 1000 source-span-bound queries, 160 negative/abstain controls, 100 unsupported rows, 30 missing-specific rows, 140 doc-code conflict rows, 4000 direct separate answer/citation/resource evaluator rows, A/B/G/H same-query deterministic source-span adapter rows, replay hashes, and closed public-comparison wording. It explicitly carries `v53ap_expected_answer_oracle_replay=0`, `v53ap_deterministic_source_span_adapter_execution=1`, `v53ap_actual_adapter_execution_ready=1`, and `v53ap_real_system_performance_claim_ready=0`, so current A/B/G/H rows prove replayable deterministic adapter execution but not real system performance evidence. It keeps accepted human review rows at 0/7000, accepted adjudication rows at 0/1000, `review_return_ready=0`, `quality_comparison_claim_ready=0`, `v53_ready=0`, `v1_0_comparison_ready=0`, and release claims blocked.
