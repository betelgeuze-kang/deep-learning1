...
v61gi: post-v61gh authority-bound operator input scaffold is implemented and covered by `experiments/test_v61gi_post_gh_authority_bound_operator_input_scaffold.sh`. The runner consumes v61gh, splits the 14 root artifact contracts into 12 final operator input files plus two generated provenance markers, emits `.template` files for the final input tree, and provides a preflight verifier plus v61gh assembly wrapper. The default smoke verifies 12 template files, zero templates counted as evidence, 12 operator input rows, two generated marker rows, `operator_input_preflight_ready=0`, assembled roots 0/2, real review/generation rows zero, `authority_bound_replay_admission_ready=0`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes; it also rejects the template tree as final input.

v61gj: post-v61gi operator input receiver is implemented and covered by `experiments/test_v61gj_post_gi_operator_input_receiver.sh`. The runner consumes v61gi, accepts an optional final `V61GJ_OPERATOR_INPUT_ROOT`, preflights all 12 final operator files for presence, non-empty content, placeholder/template/fixture text rejection, CSV/JSON schema, minimum row counts, acceptance-summary SHA-256 bindings, cross-file ID consistency, selected-slice binding, and authority-statement finality, and only admits assembly when `V61GJ_OUTPUT_ROOT` is supplied outside the repository. The default smoke verifies supplied input 0, preflight rows 0/12 ready, cross-file consistency rows 0/12 ready, selected-slice binding rows 0/12 ready, authority statement rows 0/2 ready, assembly admitted/executed 0, assembled roots 0/2, real review/generation rows zero, `row_acceptance_ready=0`, `dual_external_return_real_ready=0`, `real_return_replay_admission_ready=0`, `generation_acceptance_closure_ready=0`, `authority_bound_replay_admission_ready=0`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes. The receiver also exposes schema, minimum-row, hash-binding, cross-file, selected-slice, and authority-statement checks as separate stage/decision gates. The template-tree, invalid-schema, invalid-consistency, invalid-selected-slice, and invalid-authority smokes verify scaffold templates, malformed final files, mismatched row IDs, wrong subset-target rows, or nonfinal authority statements are rejected as final input and do not trigger assembly, and v61gj exposes the subset replay gates directly from the assembly replay outputs, flips its shell-quoted operator-input command row to ready only when assembly is admitted, and copies the operator replay source evidence into its own receiver package when real operator input is supplied.
