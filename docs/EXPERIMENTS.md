...
v53aq: Complete-source A/B/G/H query-text-only real-adapter measured rows are implemented and covered by `experiments/test_v53aq_complete_source_abgh_real_adapter_measured.sh`. The runner consumes v53i and writes `adapter_selection_contract_rows.csv`, `abgh_answer_rows.csv`, `abgh_citation_rows.csv`, `abgh_retrieval_rows.csv`, `abgh_evaluator_rows.csv`, `abgh_adapter_trace_rows.csv`, `abgh_abstain_rows.csv`, `abgh_wrong_answer_guard_rows.csv`, `abgh_resource_rows.csv`, `abgh_same_query_internal_prebaseline_rows.csv`, `route_memory_rows.csv`, `routehint_rows.csv`, system metrics, boundary, manifest, copied v53i evidence, and hash rows. The smoke verifies 4000 answer/citation/retrieval/evaluator/adapter-trace/abstain/guard/resource rows, a 1000-row per-query internal pre-baseline ledger with shared evaluator/resource surfaces, 2000 G/H RouteMemory/RouteHint rows, `selection_question_text_only=1`, `selection_oracle_field_used=0`, `expected_answer_oracle_replay=0`, `deterministic_source_span_adapter_execution=0`, `real_adapter_execution_ready=1`, `internal_real_adapter_metric_claim_ready=1`, `public_real_system_performance_claim_ready=0`, 3713 answer-hash matches, 287 coherent wrong-key rows, and public comparison/release claims closed.
...
v54c: Complete-source grounded generation 1000 is implemented and covered by `experiments/test_v54c_complete_source_grounded_generation_1000.sh`. The runner assembles `results/v54c_complete_source_grounded_generation_1000/generation_001/` from the frozen v53i complete-source source-span benchmark and v53ap A/B/G/H same-query deterministic source-span adapter surface, copies `source_v53ap/abgh_adapter_trace_rows.csv` and `source_v53ap/abgh_evaluator_rows.csv`, and writes `answer_rows.csv`, `citation_rows.csv`, `unsupported_claim_rows.csv`, `abstain_rows.csv`, `generator_resource_rows.csv`, `wrong_answer_guard_rows.csv`, `generator_input_rows.csv`, `compact_routehint_rows.csv`, `V54C_COMPLETE_SOURCE_GROUNDED_GENERATION_BOUNDARY.md`, `v54c_complete_source_grounded_generation_manifest.json`, `sha256_manifest.csv`, and `sha256sums.txt`. The smoke verifies `v54c_complete_source_grounded_generation_1000_ready=1`, `v54_generation_1000_ready=1`, 1000 source-span-generated answers, 1000 v53ap H-adapter-trace provenance bindings, 1000 v53ap H-evaluator provenance bindings, 1000 separate answer/citation/resource evaluator checks, 1000 citation/resource/guard rows, 160 unsupported/abstain rows, 30 missing-specific abstain rows, zero attention/Transformer/raw-prompt-context rows, zero wrong-answer rows, and release blocking.
...
- The h10 PM real-label promotion readiness gate binds h10-s to the current
  ...
  query-text-only real-adapter wrong-key/provenance rows, the v53t
...
query-text-only real-adapter wrong-key/provenance evidence, and the v53t
...
- coherent wrong-key, chunk exact, near-miss slash, missing-query abstain,
  ...
  acceptance rows
- coherent wrong-key evidence cites the v53aq real-adapter split: A has 287
  coherent wrong-key rows and H has zero under scorer/policy traces, with the
