...
v61gj: post-v61gi operator input receiver is implemented and covered by `experiments/test_v61gj_post_gi_operator_input_receiver.sh`. The runner consumes v61gi, accepts an optional final `V61GJ_OPERATOR_INPUT_ROOT`, preflights all 12 final operator files for presence, non-empty content, and placeholder/template/fixture text rejection, and only admits assembly when `V61GJ_OUTPUT_ROOT` is supplied outside the repository. The default smoke verifies supplied input 0, preflight rows 0/12 ready, assembly admitted/executed 0, assembled roots 0/2, real review/generation rows zero, `authority_bound_replay_admission_ready=0`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes. The template-tree smoke verifies scaffold templates are rejected as final input and do not trigger assembly.
