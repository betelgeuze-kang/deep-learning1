...
v61fq: post-v61fp v1.0 comparison readiness refresh is implemented and covered by `experiments/test_v61fq_post_fp_v1_comparison_readiness_refresh.sh`. The runner consumes v52y, v53t, v53am, v61dh, and v61fp, then emits readiness rows, claim-boundary rows, next-action rows, source copies, a metadata-only refresh package, and checksum manifest. The smoke verifies `v52_ready=1`, `f_optional_final_disposition=deferred-with-reason-final`, `comparison_30b_150b_wording_status=allowed-with-disclosure`, `v53_machine_complete_source_surface_ready=1`, complete-source 10 repos, 1000 queries, 7000 answer rows, accepted human review 0/7000, accepted adjudication 0/1000, `full_shard_prerequisites_closed=1`, 21 readiness rows with 11 ready and 10 blocked, eight comparison claim rows with four allowed and four blocked, `v1_0_comparison_ready=0`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes.
...
v61ft: active goal completion audit is implemented and covered by `experiments/test_v61ft_active_goal_completion_audit.sh`. The runner consumes v52y, v53t, v61dg, v61fq, and v61fs, then emits active-goal requirement rows, section rows, blocker rows, next-action rows, metric rows, a metadata-only audit package, source copies, and checksum manifest. The smoke verifies `active_goal_complete=0`, 20 requirement rows with 13 pass and seven blocked, three objective sections with one pass and two blocked, seven blocker rows, five next actions with two ready, `v52_ready=1`, `v53_machine_complete_source_surface_ready=1`, `post_full_shard_runtime_evidence_ready=1`, successful ready commands 4/4, external inputs 0/5, `v1_0_comparison_ready=0`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes.
...
v61fz: post-v61fy active-goal status refresh is implemented and covered by `experiments/test_v61fz_post_fy_active_goal_status_refresh.sh`. The runner consumes v61ft, v61fu, v61fx, and v61fy, then emits a metadata-only status refresh package with requirement rows, blocker rows, next-action rows, metric rows, source copies, boundary, manifest, and checksum manifest. The smoke verifies `active_goal_complete=0`, v52 ready, v53 complete-source machine surface 10 repos / 1000 queries / 7000 answer rows, post-full-shard runtime evidence ready, root-pinned handoff receipt ready, 18 requirements with seven ready and 11 blocked, 11 blocker rows, six next actions with one ready and five blocked, 91 missing external return artifacts, missing human review rows 7000, missing adjudication rows 1000, missing generation-result artifacts 5, `actual_model_generation_ready=0`, and zero checkpoint payload bytes.

v61ga: post-v61fz generation unblock runway is implemented and covered by `experiments/test_v61ga_post_fz_generation_unblock_runway.sh`. The runner consumes v61fz, v53ao, and v61fu, then emits a metadata-only runway package with requirement rows, blocker rows, six minimum return batches, replay command rows, delta focus rows, metric rows, source copies, boundary, manifest, and checksum manifest. The smoke verifies v52 ready, v53 complete-source machine surface ready, post-full-shard runtime evidence ready, root-pinned replay ready, v53ao ready actions 2/2 successful, 18 runway requirements with five ready and 13 blocked, six blocked minimum batches, five replay commands with two ready and three blocked, 14 open delta focus rows, 91 missing external return artifacts, human review 0/7000, adjudication 0/1000, generation-result artifacts 0/5, generation-result rows 0/1000, `actual_model_generation_ready=0`, and zero checkpoint payload bytes.
