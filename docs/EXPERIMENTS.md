...
v61gi: post-v61gh authority-bound operator input scaffold is implemented and covered by `experiments/test_v61gi_post_gh_authority_bound_operator_input_scaffold.sh`. The runner consumes v61gh, splits the 14 root artifact contracts into 12 final operator input files plus two generated provenance markers, emits `.template` files for the final input tree plus a top-level `OPERATOR_INPUT_RECEIPT.json.template`, emits one minimal-slice CSV template with content-witness path fields, a seven-row content-witness manifest, and an env template, and provides a witness-directory-to-CSV builder, minimal-slice materializer, receipt builder, preflight verifier, v61gh assembly wrapper, fail-closed minimal-slice-to-dual-replay wrapper, and content-witness contract for final assembly authority. The default smoke verifies 13 final-file template rows, one receipt template, one minimal-slice template, seven witness manifest rows, ready env/builder/materializer tools, zero templates counted as evidence, 12 operator input rows, two generated marker rows, two ready local commands, six blocked build/materialize/receipt/preflight/assembly/replay commands, `operator_input_receipt_ready=0`, `operator_input_preflight_ready=0`, assembled roots 0/2, real review/generation rows zero, `authority_bound_replay_admission_ready=0`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes; it also rejects the template tree and no-env builder/replay wrapper as final input/execution.
