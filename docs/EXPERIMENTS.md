...
v59b: One-command candidate/intake-chain demo is implemented and covered by `experiments/test_v59b_one_command_candidate_demo.sh`. The repository entrypoint `examples/v1_0_architecture_challenge_candidate_demo.sh` runs `experiments/run_v59b_one_command_candidate_demo.sh`, which assembles `results/v59b_one_command_candidate_demo/candidate_001/`, replays or reuses v52b-v58b candidate/intake artifacts, writes `candidate_stage_replay_rows.csv`, `candidate_one_command_rows.csv`, `candidate_demo_gate_rows.csv`, `candidate_demo.sh`, `README_RESULT.md`, `V59B_ONE_COMMAND_CANDIDATE_DEMO_BOUNDARY.md`, `v59b_one_command_candidate_demo_manifest.json`, and `sha256_manifest.csv`. The smoke verifies `v59b_one_command_candidate_demo_ready=1`, 11 candidate stage rows, one-command candidate entrypoint readiness, copied candidate artifacts, hash binding, and release blocking, while keeping `v59_ready=0`, real 30B/70B rows, optional 100B+ row/final deferral, complete-source audit, human domain review, human blind review, and release evidence blocked.
