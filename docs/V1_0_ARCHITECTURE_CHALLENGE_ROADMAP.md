...
20. Closed as v61l/v61m measurement seeds: add GPU/ROCm page-dequant-matmul timing and KV-cache residency/eviction policy over the v61k page geometry, while keeping the payload synthetic, checkpoint weights blocked, and host-RAM KV spill disabled. Next v61 runtime steps are local checkpoint shard/header intake, safetensors page-hash binding, and source-bound workloads without opening near-frontier or release claims until external review passes.
