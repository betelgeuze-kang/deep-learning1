...
- `v59c` one-command measured-registry demo. Started. `experiments/test_v59c_one_command_measured_registry_demo.sh` now verifies `examples/v1_0_architecture_challenge_measured_registry_demo.sh`, which promotes the v52j A/B/G/H measured registry into the v59 replay path while preserving the local-only claim boundary and keeping C/D/E, complete-source audit, human review, full v59, and release claims blocked.
