...
- `v48` provides the Multi-Domain RouteHint Generator evidence run: 24 generation rows across RULER NIAH, LongBench v2, codebase QA, and internal docs QA, with RouteMemory-derived evidence, compact RouteHint, tiny non-attention generation, grounded answer, citation, abstain, audit trail, v18 commercial-return verification, and release blocking.
...
- `v50` provides the Public Repo Auditor 3-repo evidence run over v42/v43/v18: actual public repo snapshots for `pypa/sampleproject`, `psf/requests`, and `pallets/click`, 9 audit cases across doc-code conflict, deprecated/legacy usage, and config mismatch, source-span binding, v18 commercial-return verification, and release/upstream-defect claim blocking.
- Current verified state after PR run `27029089994` plus v33/v34/v35/v36/v37/v38/v39/v40/v41/v42/v43/v44/v45/v46/v47/v48/v49/v50: `independent_rerun_actual_ready=1`, `candidate_external_benchmark_result_ready=1`, `candidate_external_benchmark_expansion_ready=1`, `closed_corpus_poc_actual_ready=1`, `real_external_benchmark_verified=1`, `v33_evidence_closure_packet_ready=1`, `v34_official_benchmark_expansion_packet_ready=1`, `v35_commercial_pilot_packet_ready=1`, `v36_release_claim_audit_packet_ready=1`, `v37_human_review_intake_ready=1`, `v38_human_review_dispatch_bundle_ready=1`, `v39_human_review_dispatch_archive_ready=1`, `v40_machine_verified_research_artifact_ready=1`, `v41_ruler_niah_50row_scale_ready=1`, `v42_codebase_auditor_200query_ready=1`, `v43_doc_code_conflict_detection_ready=1`, `v44_tiny_non_attention_generator_hint_ready=1`, `v45_longbench_v2_small_slice_ready=1`, `v46_source_verified_scorer_mainline_ready=1`, `v47_offline_domain_policy_update_ready=1`, `v48_multi_domain_generator_evidence_ready=1`, `v49_ruler_niah_200_500_scale_ready=1`, and `v50_public_repo_auditor_3repo_ready=1`; `maximum_allowed_claim_decided=1`, `human_review_request_ready=1`, `return_template_ready=1`, `verify_script_ready=1`, `archive_sha256_ready=1`, `archive_file_list_ready=1`, `machine_verification_ready=1`, `automated_research_artifact_ready=1`, `machine_verified_prototype_ready=1`, `row_count_ready=1`, `same_context_length=1`, `no_oracle_no_extractor_ready=1`, `v18_verified=1`, `target_200_ready=1`, `target_500_ready=1`, `rows_200=200`, `rows_500=500`, `lineage_rows_200=200`, `lineage_rows_500=500`, `context_length_fixed=1`, `architecture_fixed=1`, `query_rows=200`, `poc_result_rows=200`, `audit_trail_rows=200`, `conflict_rows=8`, `consistent_rows=4`, `supporting_source_spans_ready=1`, `no_raw_prompt_stuffing_ready=1`, `non_attention_generator_ready=1`, `answer_grounded_rate=1.000000`, `task_categories=6`, `v18_candidate_external_benchmark_result_ready=1`, `source_verified_label_rows=12`, `scorer_top1_accuracy=1.000000`, `wrong_candidate_guard_ready=1`, `policy_rows=15`, `domain_count=4`, `generation_rows=24`, `abstain_rows=4`, `public_repo_count=3`, `public_repo_audit_case_rows=9`, `public_repo_source_span_rows=18`, `raw_context_in_hint_rows=0`, `raw_prompt_context_appended_rows=0`, `expert_replacement_claim=0`, `release_ready_claim=0`, `privacy_review_ready=1`, `resource_envelope_ready=1`, and `v18_closed_corpus_poc_actual_ready=1`, but `human_review_return_supplied=0`, `human_review_completed=0`, `human_review_required_for_public_release=1`, and `real_release_package_ready=0` remain blocked until external review accepts the evidence set.
...
- v48 has added `results/v48_multi_domain_generator_evidence/run_001/` as the first post-v47 evidence-scale generator run. It verifies the same RouteHint generation path across RULER, LongBench, codebase QA, and internal docs QA.
...
- v50 has added `results/v50_public_repo_auditor_3repo/audit_001/` as the public repo auditor evidence run. It verifies actual public repo snapshots across 3 repositories and 3 audit types.
...
- Month 3: done. `v48` expands the tiny non-attention generator from smoke to multi-domain answer generation evidence across RULER, LongBench, codebase QA, and internal docs QA.
...
- Month 3: done. `v50` expands the Codebase Auditor from local repository evidence to 3 actual public repositories with doc-code conflict, deprecated/legacy usage, config mismatch, source citation, and audit trail.
- The v41-v50 impact roadmap is closed. Do not add another internal packaging layer unless a real return exposes a verifier gap; the next work should collect real-return evidence, with human review deferred until release-ready wording becomes necessary.
...
- `v48` Multi-Domain RouteHint Generator evidence. Done. Goal: expand v44 from smoke to multi-domain answer generation. Success message: RouteMemory evidence, compact RouteHint, tiny non-attention generator, grounded answer, citation, abstain, and audit trail hold across RULER, LongBench, codebase QA, and internal docs QA.
...
- `v50` Public Repo Auditor 3 repositories. Done. Goal: move the Codebase Auditor from local repository evidence into actual public repositories. Success message: doc-code conflict, deprecated/legacy usage, and config mismatch detection hold across 3 public repos with source citations, abstentions, wrong-answer guard, and audit trail.
- `v51` Real-return evidence intake. Next. Goal: replace more supplied/mechanical confidence with actual return evidence. Success message: at least one external/human or buyer PoC acceptance return, real teacher-source import candidate, or measured CPU/HIP/NVMe workload trace is bound into the v18/v40 evidence ladder without opening release-ready wording.
