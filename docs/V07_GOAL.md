...
The next research boundary after h10-h/v08-b/v08-c is external teacher-label source
...
The benchmark adapter and evidence schemas now cover RULER, LongBench,
codebase retrieval, and real document QA, but they have no
dataset/result/baseline/license evidence yet. Until external-label and
benchmark evidence exist, the current default policy stays diagnostic-only and
...
CPU-only HIP error handling, the h7 goal closure, and v08-b/v08-c benchmark
adapter/evidence/readiness. CPU/HIP parity remains an optional extended check
until a complete ROCm/HIP install proves fixture parity.
