...
source_v61at_summary = read_csv(run_dir / "source_v61at/v61at_prefetch_overlap_admission_gate_summary.csv")[0]
source_v61at_tokens = read_csv(run_dir / "source_v61at/prefetch_overlap_token_rows.csv")
source_bound_token_rows = len(source_v61at_tokens)
bootstrap_cold_fill_page_rows = int(source_v61at_tokens[0]["cache_miss_page_rows"])
steady_state_prefetch_issue_rows = sum(int(row["cache_miss_page_rows"]) for row in source_v61at_tokens[1:])
no_prefetch_required_rows = sum(1 for row in source_v61at_tokens[1:] if row["cache_miss_page_rows"] == "0")
steady_state_deadline_met_rows = steady_state_prefetch_issue_rows
steady_state_deadline_miss_rows = 0
total_cold_fill_page_rows = bootstrap_cold_fill_page_rows + steady_state_prefetch_issue_rows
expected = {
    ...
    "total_cold_fill_page_rows": str(total_cold_fill_page_rows),
    "bootstrap_cold_fill_page_rows": str(bootstrap_cold_fill_page_rows),
    "steady_state_prefetch_issue_rows": str(steady_state_prefetch_issue_rows),
    "steady_state_deadline_met_rows": str(steady_state_deadline_met_rows),
    "steady_state_deadline_miss_rows": str(steady_state_deadline_miss_rows),
    ...
    "ssd_read_latency_ms_p95_per_page": source_v61at_summary["ssd_read_latency_ms_p95_per_page"],
    "prior_token_compute_window_ms": source_v61at_summary["token_page_kernel_compute_window_ms"],
    "min_deadline_slack_ms": source_v61at_summary["min_steady_state_overlap_slack_ms"],
...
if len(token_rows) != source_bound_token_rows or len(issue_rows) != total_cold_fill_page_rows or len(queue_rows) != 1:
...
if sum(1 for row in token_rows if row["scheduler_status"] == "scheduled-before-deadline") != sum(1 for row in source_v61at_tokens[1:] if int(row["cache_miss_page_rows"]) > 0):
    raise SystemExit("v61au steady-state scheduled token row count mismatch")
if sum(1 for row in token_rows if row["scheduler_status"] == "no-prefetch-required") != no_prefetch_required_rows:
    raise SystemExit("v61au no-prefetch-required token row count mismatch")
if sum(1 for row in issue_rows if row["prefetch_issue_status"] == "scheduled-before-deadline") != steady_state_deadline_met_rows:
    raise SystemExit("v61au steady-state issue row count mismatch")
if sum(1 for row in issue_rows if row["prefetch_issue_status"] == "bootstrap-blocked-no-prior-window") != bootstrap_cold_fill_page_rows:
    raise SystemExit("v61au bootstrap issue row count mismatch")
...
if manifest.get("steady_state_prefetch_issue_rows") != steady_state_prefetch_issue_rows:
...
for snippet in [
    f"steady_state_prefetch_issue_rows={steady_state_prefetch_issue_rows}",
    f"steady_state_deadline_met_rows={steady_state_deadline_met_rows}",
    f"steady_state_deadline_miss_rows={steady_state_deadline_miss_rows}",
    "configured_prefetch_queue_depth=4",
    "max_steady_state_required_queue_depth=1",
    "steady_state_scheduler_ready=1",
    "bootstrap_scheduler_ready=0",
