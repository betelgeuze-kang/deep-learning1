...
acceptance_rows = [
    {
        ...
        "machine_evidence_status": "diagnostic-pass" if h10_diagnostic_scorer_signal_ready else "blocked",
        "real_label_status": "pass" if coherent_wrong_real_label_ready else "blocked",
        "evidence": f"h10s_coherent_wrong_negative_rate={h10s.get('coherent_wrong_negative_rate', '0')}",
    ...
    },
    {
        ...
        "evidence": f"v53q_scorer_rows={v53q.get('symmetric_scorer_rows')}; v54c_citation_correct={v54c.get('citation_correct_rows')}",
...
decision_rows = [
    ...
    ("v53ap-abgh-same-query-prebaseline", "pass" if same_query_abgh_ready else "blocked", "A/B/G/H share v53i complete-source query/source set"),
