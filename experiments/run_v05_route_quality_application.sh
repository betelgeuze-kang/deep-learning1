...
CYCLES_PER_EPOCH=10
PROPOSAL_COUNT=22
PREFIX="v05_route_quality_application"
KEY_COUNTS=(128)
SEEDS=(1 2)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=6
  PROPOSAL_COUNT=18
  PREFIX="v05_route_quality_application_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=16
...
compute_metrics() {
  ...
  awk -F, '
    BEGIN {
      name_count = split("fixture_query_byte_acc route_quality_apply_active route_quality_source_ranking_beta route_quality_source_ranking_delta_mean route_quality_selected_raw_rate route_quality_selected_keyshape_rate route_quality_selected_noisy_rate route_quality_retry_raw_proxy_mean route_quality_retry_keyshape_proxy_mean route_quality_retry_noisy_proxy_mean route_quality_retry_raw_delta_mean route_quality_retry_keyshape_delta_mean route_quality_retry_noisy_delta_mean route_quality_selected_raw_qacc route_quality_selected_keyshape_qacc route_quality_selected_noisy_qacc route_source_retry_raw_selected_rate route_source_retry_keyshape_selected_rate route_source_retry_noisy_selected_rate route_quality_logdet_mean route_quality_score_mean route_hint_candidate_lookup_count route_hint_value_read_distance_mean routing_trigger_rate active_jump_rate", names, " ")
    }
    NR == 1 {
...
run_case() {
  ...
  local key_count="$2"
  local seed="$3"
  local quality_apply="$4"
  local quality_beta="$5"
  local retry_tiebreak="$6"
  local retry_priorities_csv="$7"
  local retry_candidates_csv="$8"
  local route_hash_source="${9:-route-code-key}"
  ...
  echo "quality-application: ${scenario} apply=${quality_apply} beta=${quality_beta}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
    ...
    --route-hash-source "$route_hash_source" \
    --route-code-aux 1 \
    ...
    --route-source-retry-candidates "$retry_candidates_csv" \
...
run_arm_set() {
  ...
  run_case apply-none-source-order "$key_count" "$seed" none 0.0 source-order \
    "raw-key:0.0,key-shape:0.0,noisy-route-code:0.0" \
    raw-key,key-shape,noisy-route-code \
    route-code-key
  run_case source-ranking-b0p10 "$key_count" "$seed" source-ranking 0.10 source-order \
    "raw-key:0.0,key-shape:0.0,noisy-route-code:0.0" \
    raw-key,key-shape,noisy-route-code \
    route-code-key
  run_case source-ranking-b0p25 "$key_count" "$seed" source-ranking 0.25 source-order \
    "raw-key:0.0,key-shape:0.0,noisy-route-code:0.0" \
    raw-key,key-shape,noisy-route-code \
    route-code-key
  run_case source-ranking-keyshape-prior "$key_count" "$seed" source-ranking 0.10 source-prior \
    "key-shape:0.2,raw-key:0.0,noisy-route-code:-1.0" \
    raw-key,key-shape,noisy-route-code \
    route-code-key
  run_case fixed-keyshape-none "$key_count" "$seed" none 0.0 source-order \
    "key-shape:0.0,noisy-route-code:0.0" \
    key-shape,noisy-route-code \
    route-code-key
}
