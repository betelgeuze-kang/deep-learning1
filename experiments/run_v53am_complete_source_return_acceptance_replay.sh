...
if [[ -n "$RETURN_BUNDLE_DIR" ]]; then
  ...
  V53X_REVIEW_CHUNK_RETURN_DIR="$RETURN_BUNDLE_DIR" V53X_REUSE_EXISTING=0 "$ROOT_DIR/experiments/run_v53x_complete_source_review_chunk_return_intake.sh" >/dev/null
  ...
  V53Z_REVIEW_CHUNK_RETURN_DIR="$RETURN_BUNDLE_DIR" V53Z_REVIEW_RETURN_DIR="$RETURN_BUNDLE_DIR/aggregate_review_return" V53Z_REUSE_EXISTING=0 "$ROOT_DIR/experiments/run_v53z_complete_source_review_return_v61_handoff_bridge.sh" >/dev/null
  ...
  V53AE_DISPATCH_RECEIPT_DIR="$RETURN_BUNDLE_DIR" V53AE_REVIEW_CHUNK_RETURN_DIR="$RETURN_BUNDLE_DIR" V53AE_REVIEW_RETURN_DIR="$RETURN_BUNDLE_DIR/aggregate_review_return" V53AE_GENERATION_RESULT_DIR="$RETURN_BUNDLE_DIR/generation_result_return" V53AE_REUSE_EXISTING=0 "$ROOT_DIR/experiments/run_v53ae_complete_source_review_return_generation_rendezvous_gate.sh" >/dev/null
else
...
command_rows = [
    ...
    {"command_id": "03-review-chunks", "ready_to_run_now": str(preflight_pass), "command": "V53X_REVIEW_CHUNK_RETURN_DIR=/path/to/final_return_bundle V53X_REUSE_EXISTING=0 ./experiments/run_v53x_complete_source_review_chunk_return_intake.sh"},
