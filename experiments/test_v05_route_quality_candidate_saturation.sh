...
SUMMARY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_saturation_smoke_summary.csv"
AGG_CSV="$ROOT_DIR/results/v05_route_quality_candidate_saturation_smoke_aggregate.csv"

"$ROOT_DIR/experiments/run_v05_route_quality_candidate_saturation.sh" --smoke
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing candidate-saturation column: " required[i], 2)
      }
    }
    next
  }
  {
    ...
    if ($idx["quality_apply"] != "candidate-weight" ||
      ...
      die("candidate-saturation should keep candidate-weight apply active: " arm, 4)
    }
    if (qacc < 0 || qacc > 1 || factor < 0.5 || factor > cap ||
        factor_p90 < 0.5 || factor_p90 > cap ||
        factor_max < 0.5 || factor_max > cap) {
      die("candidate-saturation factor/qacc out of range: " arm, 5)
    }
    if (entropy < 0 || top_share < 0 || top_share > 1) {
      die("candidate-saturation concentration metric out of range: " arm, 6)
    }
    ...
    if (metric("routing_trigger_rate") != 0 || metric("active_jump_rate") != 0) {
      die("candidate-saturation must not activate jump-neighbor routing: " arm, 8)
    }
    if (metric("route_quality_selected_noisy_rate") > 0.25) {
      die("candidate-saturation selected noisy source too often: " arm, 9)
    }
    if (metric("route_quality_candidate_weight_factor_gap") <= 0) {
      die("candidate-saturation should preserve positive factor gap: " arm, 10)
    }
  ...
  END {
    ...
    required_arms_count = split("candidate-b1p25-cap2p0 candidate-b1p25-cap3p0 candidate-b1p50-cap2p0 candidate-b1p50-cap3p0 candidate-b2p00-cap2p0 candidate-b2p00-cap3p0", required_arms, " ")
...
awk -F, '
  ...
  NR == 1 {
    required_count = split("arm rows qacc_mean route_quality_candidate_weight_beta_mean route_quality_candidate_weight_factor_p90_mean route_quality_candidate_weight_factor_max_mean route_quality_candidate_weight_entropy_mean_mean route_quality_candidate_weight_top_share_mean_mean route_quality_selected_noisy_rate_mean lookup_count_mean read_distance_mean routing_trigger_rate_mean active_jump_rate_mean", required, " ")
    for (i = 1; i <= NF; i++) idx[$i] = i
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing aggregate candidate-saturation column: " required[i], 20)
      }
    }
    next
  }
  ...
  END {
    if (rows != 6) {
      die("expected 6 aggregate rows, found " rows, 23)
    }
  }
' "$AGG_CSV"

echo "route quality candidate-saturation smoke passed"
