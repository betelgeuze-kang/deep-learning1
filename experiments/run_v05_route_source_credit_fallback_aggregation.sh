...
CYCLES_PER_EPOCH=16
PROPOSAL_COUNT=30
PREFIX="v05_route_source_credit_fallback_aggregation"

if [[ "$MODE" == "smoke" ]]; then
  EPOCHS=8
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_fallback_aggregation_smoke"
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=28
  CYCLES_PER_EPOCH=20
  PROPOSAL_COUNT=30
...
compute_metrics() {
  ...
  awk -F, '
    BEGIN {
      split("fixture_query_byte_acc key_region_route_decode_acc route_primary_recall route_fallback_used_rate route_fallback_recall route_fallback_qacc route_fallback_success_rate route_fallback_hi_acc route_fallback_lo_acc route_candidate_top1_rate route_candidate_rank_mean route_hint_correct_value_vote_share_mean route_hint_vote_entropy_mean route_hint_unique_values_mean route_hint_vote_margin_mean route_hint_vote_candidate_count_mean route_lowconf_query_rate route_highconf_query_rate route_lowconf_qacc route_highconf_qacc route_lowconf_top1 route_highconf_top1 route_lowconf_correct_value_vote_share route_highconf_correct_value_vote_share route_agg_policy_vote_rate route_agg_policy_weighted_rate route_hint_candidate_lookup_count route_hint_value_read_distance_mean routing_trigger_rate active_jump_rate", names, " ")
    }
    NR == 1 {
...
run_case() {
  ...
  echo "fallback aggregation: ${scenario} source=${fallback_source} agg=${route_hint_agg} low=${lowconf_agg} high=${highconf_agg}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
    ...
    --route-source-credit-learning 0 \
    --route-source-credit-apply-mode off \
    --csv "$csv_path"
...
run_arm_set() {
  ...
  run_case "${label}-top1" "$key_count" "$seed" "$fallback_source" top1 vote weighted-vote
  run_case "${label}-vote" "$key_count" "$seed" "$fallback_source" vote vote weighted-vote
  run_case "${label}-weighted" "$key_count" "$seed" "$fallback_source" weighted-vote vote weighted-vote
  run_case "${label}-gated-vote-weighted" "$key_count" "$seed" "$fallback_source" confidence-gated vote weighted-vote
  run_case "${label}-gated-weighted-weighted" "$key_count" "$seed" "$fallback_source" confidence-gated weighted-vote weighted-vote
}

run_pair() {
  local key_count="$1"
  local seed="$2"
  run_arm_set "$key_count" "$seed" raw-key raw
  run_arm_set "$key_count" "$seed" key-shape keyshape
}
