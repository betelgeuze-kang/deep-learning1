...
PREFIX="v52g_small_local_rag_measured_300"
RUN_ID="${V52G_RUN_ID:-measured_001}"
...
if [[ "${V52G_REUSE_EXISTING:-1}" != "1" || ! -s "$RESULTS_DIR/v53e_canary_query_scale_1000_summary.csv" ]]; then
...
decision_rows = [
    ("small-local-rag-300-measured", "pass", "B baseline writes 300 measured answer/citation/abstain/guard/resource rows"),
    ("same-frozen-query-set", "pass", "uses a frozen stratified 300-row subset of the v53e 1000-row query ids and source spans"),
    ...
    ("v52-absorb-ready", "pass", "B-300 can be absorbed into v52 comparison registry"),
    ("v52-b-1000", "blocked", "B baseline still needs the next 1000-row measured run"),
    ...
    ("real-release-package", "blocked", "B-300 measured run is not a release package"),
]
write_csv(decision_csv, ["gate", "status", "reason"], [{"gate": gate, "status": status, "reason": reason} for gate, status, reason in decision_rows])

(run_dir / "V52G_SMALL_LOCAL_RAG_300_BOUNDARY.md").write_text(
    "# v52g Small Local RAG 300-Row Boundary\n\n"
    "This is the 300-row measured expansion for baseline B over a frozen stratified 300-row subset of the v53e canary query scale. "
    ...
    "- external_model_used=0\n\n"
    "Still blocked:\n\n"
    "- B 1000-row measured run\n"
    "- A/G/H rows on the same frozen query set\n"
    "- C/D/E/F evidence directories and validated rows\n"
    "- complete-source v53 audit rows\n\n"
    "Do not publish 30B-150B comparison claims from this B-only 300-row measured run.\n",
    encoding="utf-8",
)

manifest = {
    "manifest_scope": "v52g-small-local-rag-measured-300",
...
(run_dir / "v52g_small_local_rag_measured_300_manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
print(f"v52g_small_local_rag_measured_300_dir: {run_dir}")
