...
SUMMARY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_preset_policy_smoke_summary.csv"
POLICY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_preset_policy_smoke_policy.csv"
AGG_CSV="$ROOT_DIR/results/v05_route_quality_candidate_preset_policy_smoke_aggregate.csv"

"$ROOT_DIR/experiments/run_v05_route_quality_candidate_preset_policy.sh" --smoke
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing preset-policy summary column: " required[i], 2)
      }
    }
    next
  }
  {
    ...
    if (metric("key_count") != 64 || metric("seed") != 1) {
      die("preset-policy smoke should run key_count=64 seed=1", 3)
    }
    if (metric("apply_active") != 1 || metric("beta") != 8) {
      die("preset-policy should keep candidate-weight beta=8 active", 4)
    }
    ...
    if (metric("routing_trigger_rate") != 0 || metric("active_jump_rate") != 0) {
      die("preset-policy must not activate jump-neighbor routing", 6)
    }
    seen[$idx["arm"]] = 1
  }
  END {
    if (rows != 2) {
      die("expected 2 preset-policy summary rows, found " rows, 7)
    }
    if (!seen["base-default"] || !seen["hybrid-safe"]) {
      die("preset-policy smoke missing base-default or hybrid-safe row", 8)
    }
  }
' "$SUMMARY_CSV"
...
awk -F, '
  ...
  function metric(name) { return $idx[name] + 0 }
  NR == 1 {
    required_count = split("scenario key_count seed noisy_source_rate base_qacc hybrid_qacc qacc_delta base_factor_gap hybrid_factor_gap factor_gap_delta base_factor_max hybrid_factor_max factor_max_delta base_quality_score_gap hybrid_quality_score_gap quality_score_gap_delta base_wrong_strength hybrid_wrong_strength wrong_strength_delta lookup_count_mean read_distance_mean routing_trigger_rate_mean active_jump_rate_mean recommendation", required, " ")
    for (i = 1; i <= NF; i++) idx[$i] = i
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing preset-policy column: " required[i], 20)
      }
    }
    next
  }
  {
    ...
    if (metric("qacc_delta") < -0.001001) {
      die("hybrid-safe qacc regressed beyond tolerance", 21)
    }
    if (metric("factor_gap_delta") >= 0) {
      die("hybrid-safe should lower factor gap in the smoke", 22)
    }
    if (metric("factor_max_delta") > 0) {
      die("hybrid-safe should not raise factor max in the smoke", 23)
    }
    if (metric("lookup_count_mean") <= 0 || metric("read_distance_mean") <= 0) {
      die("value-bearing route path should remain populated in policy row", 24)
    }
    if (metric("routing_trigger_rate_mean") != 0 || metric("active_jump_rate_mean") != 0) {
      die("policy row should preserve jump-neighbor inactivity", 25)
    }
    if ($idx["recommendation"] !~ /^hybrid-safe/) {
      die("preset-policy smoke should recommend hybrid-safe", 26)
    }
  }
  END {
    if (rows != 1) {
      die("expected 1 preset-policy row, found " rows, 27)
    }
  }
' "$POLICY_CSV"
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing aggregate preset-policy column: " required[i], 40)
      }
    }
    next
  }
  {
    rows++
    if (metric("rows") != 1) {
      die("aggregate preset-policy row count mismatch", 41)
    }
    ...
    if (metric("hybrid_recommended_rate") < 0.999999) {
      die("hybrid-safe should be recommended in smoke aggregate", 46)
    }
    if (metric("routing_trigger_rate_mean") != 0 || metric("active_jump_rate_mean") != 0) {
  ...
  END {
    if (rows != 1) {
      die("expected one aggregate preset-policy row, found " rows, 48)
    }
  }
' "$AGG_CSV"

echo "route quality candidate preset-policy smoke passed"
