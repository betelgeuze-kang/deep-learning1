...
SUMMARY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_auto_factor_threshold_smoke_summary.csv"
AGG_CSV="$ROOT_DIR/results/v05_route_quality_candidate_auto_factor_threshold_smoke_aggregate.csv"
BY_KEY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_auto_factor_threshold_smoke_by_key_noise.csv"

"$ROOT_DIR/experiments/run_v05_route_quality_candidate_hybrid_guardrail.sh" --auto-factor-threshold-smoke
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing candidate-auto-factor-threshold column: " required[i], 2)
      }
    }
    next
  }
  {
    ...
    if (metric("key_count") != 128 || metric("seed") != 1) {
      die("auto-factor-threshold smoke should run key_count=128 seed=1", 3)
    }
    ...
    if (metric("route_quality_apply_active") != 1 ||
      ...
      die("auto-factor-threshold smoke should keep candidate-weight beta=8 active: " arm, 5)
    }
    if (metric("route_quality_candidate_weight_factor_max") > 8.0) {
      die("auto-factor-threshold smoke factor max exceeded clamp: " arm, 6)
    }
    if (basis == "auto") {
      if (mode != "factor") {
        die("auto-factor-threshold auto arms must use factor mode: " arm, 7)
      }
      if (!close_enough(metric("route_quality_candidate_weight_auto_hybrid_rate"),
                        metric("route_quality_candidate_weight_auto_factor_trigger_rate"))) {
        die("factor threshold mode should hybrid-switch exactly on factor trigger", 8)
      }
    }
    ...
    if (metric("routing_trigger_rate") != 0 || metric("active_jump_rate") != 0) {
      die("auto-factor-threshold basis must not activate jump-neighbor routing: " arm, 10)
    }
    if (metric("route_quality_selected_noisy_rate") > 0.25) {
      die("auto-factor-threshold selected noisy source too often: " arm, 11)
    }
    rows++
    seen[arm] = 1
  }
  END {
    ...
    if (!seen["base-default"] || !seen["hybrid-m0p25"] ||
        !seen["factor-f5p6"] || !seen["factor-f5p8"] ||
        !seen["factor-f6p0"] || !seen["factor-f6p2"] ||
        !seen["factor-f6p4"]) {
      die("auto-factor-threshold smoke missing required arm", 13)
    }
  }
' "$SUMMARY_CSV"
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing aggregate candidate-auto-factor-threshold column: " required[i], 20)
      }
    }
    next
  }
  ...
  END {
    if (rows != 7) {
      die("expected 7 aggregate rows, found " rows, 22)
    }
  }
' "$AGG_CSV"
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing by-key candidate-auto-factor-threshold column: " required[i], 30)
      }
    }
    next
  }
  ...
  END {
    if (rows != 7) {
      die("expected 7 by-key rows, found " rows, 32)
    }
  }
' "$BY_KEY_CSV"

echo "route quality candidate-auto-factor-threshold smoke passed"
