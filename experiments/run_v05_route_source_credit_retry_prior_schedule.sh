...
CYCLES_PER_EPOCH=12
PROPOSAL_COUNT=24
PREFIX="v05_route_source_credit_retry_prior_schedule"
KEY_COUNTS=(128)
SEEDS=(1 2)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_retry_prior_schedule_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=20
...
run_case() {
  ...
  local prior_scale="$9"
  local retry_priorities_csv="${10}"
  local retry_priorities_label="${11}"
  local retry_source="${12}"
  ...
  echo "retry-prior-schedule: ${scenario} policy=${retry_policy} tiebreak=${retry_tiebreak} prior=${prior_mode} scale=${prior_scale}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
...
run_arm_set() {
  ...
  run_case source-order "$key_count" "$seed" source-credit source-order none 1.0 0 0.0 \
    "raw-key:0.0,key-shape:0.0,noisy-route-code:0.0" \
    "raw-key:0.0+key-shape:0.0+noisy-route-code:0.0" \
    off
  run_case static-keyshape-prior "$key_count" "$seed" source-credit source-prior static 1.0 0 1.0 \
    "key-shape:0.2,raw-key:0.0,noisy-route-code:-1.0" \
    "key-shape:0.2+raw-key:0.0+noisy-route-code:-1.0" \
    off
  run_case decay-keyshape-prior "$key_count" "$seed" source-credit source-prior decay 0.95 0 0.95 \
    "key-shape:0.2,raw-key:0.0,noisy-route-code:-1.0" \
    "key-shape:0.2+raw-key:0.0+noisy-route-code:-1.0" \
    off
  run_case warmup-keyshape-prior "$key_count" "$seed" source-credit source-prior warmup 1.0 "$EPOCHS" 1.0 \
    "key-shape:0.2,raw-key:0.0,noisy-route-code:-1.0" \
    "key-shape:0.2+raw-key:0.0+noisy-route-code:-1.0" \
    off
  run_case fixed-keyshape "$key_count" "$seed" fixed source-order none 1.0 0 0.0 \
    "raw-key:0.0,key-shape:0.0,noisy-route-code:0.0" \
    "raw-key:0.0+key-shape:0.0+noisy-route-code:0.0" \
    key-shape
}
