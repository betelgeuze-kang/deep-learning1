...
PREFIX="v52n_30b_open_weight_llm_rag_measured_seed"
RUN_ID="${V52N_RUN_ID:-measured_001}"
RUN_DIR="$RESULTS_DIR/$PREFIX/$RUN_ID"
SUMMARY_CSV="$RESULTS_DIR/${PREFIX}_summary.csv"
DECISION_CSV="$RESULTS_DIR/${PREFIX}_decision.csv"
MODEL_ID="${V52N_OLLAMA_MODEL:-qwen2.5:32b-instruct}"
PARAMETER_COUNT_B="${V52N_PARAMETER_COUNT_B:-32.0}"
OPEN_WEIGHT_LICENSE_URI="${V52N_OPEN_WEIGHT_LICENSE_URI:-https://huggingface.co/Qwen/Qwen2.5-32B-Instruct/blob/main/LICENSE}"

# shellcheck source=/dev/null
source "$ROOT_DIR/scripts/ollama_rocm_env.sh"
export PATH="/opt/rocm/bin:${PATH:-}"

if [[ "${V52N_REUSE_EXISTING:-0}" == "1" && -s "$SUMMARY_CSV" && -s "$RUN_DIR/sha256_manifest.csv" ]]; then
  echo "v52n_30b_open_weight_llm_rag_measured_seed_dir: $RUN_DIR"
...
evidence_dir = run_dir / "d_llm_rag_evidence"
...
for query in queries:
    ...
    context = "\n".join(context_lines)
    prompt = (
        "You are baseline D: a 30B open-weight LLM with a RAG evidence packet. "
    ...
    response = ollama_json("/api/generate", payload=payload, timeout=600)
...
env = os.environ.copy()
env["V52D_30B_LLM_RAG_EVIDENCE_DIR"] = str(evidence_dir)
...
if v52d_summary.get("d_30b_supplied_evidence_ready") != "1":
    raise SystemExit("v52d did not validate the generated D evidence directory")
...
summary = {
    ...
    "required_30b_baseline_ready": int(v52d_summary["required_30b_baseline_ready"]),
...
decision_rows = [
    ...
    ("d-open-weight-generation", "pass", "generated nine D answer rows through Ollama"),
    ("d-evidence-directory", "pass", "model identity, answer, citation, and resource rows were written"),
    ("v52d-supplied-evidence-validation", "pass", "v52d accepted the generated D evidence directory"),
    ("v52-full-d-baseline-scale", "blocked", "D is measured only on the v50 9-query seed, not the v53e 1000-query shared set"),
    ("70b-real-row", "blocked", "E 70B real evidence row is still missing"),
    ("real-release-package", "blocked", "v52n is not a release package"),
]
write_csv(decision_csv, ["gate", "status", "reason"], [{"gate": gate, "status": status, "reason": reason} for gate, status, reason in decision_rows])
write_csv(run_dir / "v52n_decision_rows.csv", ["gate", "status", "reason"], [{"gate": gate, "status": status, "reason": reason} for gate, status, reason in decision_rows])

(run_dir / "V52N_30B_OPEN_WEIGHT_LLM_RAG_MEASURED_SEED_BOUNDARY.md").write_text(
    "# v52n 30B Open-Weight LLM + RAG Measured Seed Boundary\n\n"
    "This is a real local Ollama baseline-D measured seed over the v50 9-query public-repo audit set. "
    ...
    "Still blocked: D over the shared v53e 1000-query set, E 70B real evidence directory, optional F handling, full v52, v59 full replay, and release claims.\n",
    encoding="utf-8",
)

manifest_out = {
    "manifest_scope": "v52n-30b-open-weight-llm-rag-measured-seed",
...
(run_dir / "v52n_30b_open_weight_llm_rag_measured_seed_manifest.json").write_text(json.dumps(manifest_out, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
print(f"v52n_30b_open_weight_llm_rag_measured_seed_dir: {run_dir}")
