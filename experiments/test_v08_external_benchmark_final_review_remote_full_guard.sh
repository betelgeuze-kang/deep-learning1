...
"$ROOT_DIR/experiments/test_v08_external_benchmark_source_import_real_verification_gate.sh" >/dev/null
...
awk -F, '
  ...
  }
  {
    ...
    if ($idx["final_review_source"] != "provided-csv" ||
        ...
        $idx["action"] != "external-benchmark-source-import-real-verification-placeholder-domain") {
      die("fully remote-style benchmark artifacts must still block while real verification evidence uses placeholder domains", 4)
    }
