...
awk -F, '
  ...
  }
  {
    ...
    if ($idx["final_review_source"] != "provided-csv" ||
        ...
        $idx["action"] != "external-benchmark-source-import-missing") {
      die("fully remote-style benchmark artifacts must still block without real source import verification", 4)
    }
