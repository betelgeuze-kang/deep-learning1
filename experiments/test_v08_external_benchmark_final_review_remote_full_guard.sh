...
awk -F, '
  ...
  }
  {
    ...
    if ($idx["final_review_source"] != "provided-csv" ||
        ...
        $idx["action"] != "external-benchmark-source-import-real-public-registry-missing") {
      die("fully remote-style benchmark artifacts must still block before public registry-backed source-import verification", 4)
    }
...
awk -F, '
  ...
  }
  {
    ...
    if ($idx["gate"] == "source-import" && ($idx["status"] != "blocked" || $idx["reason"] !~ /contract_ready=1/ || $idx["reason"] !~ /auth_review_ready=1/)) die("source import should block after authoritative review readiness without real public registry verification", 25)
