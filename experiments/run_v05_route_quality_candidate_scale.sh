...
CYCLES_PER_EPOCH=10
PROPOSAL_COUNT=22
PREFIX="v05_route_quality_candidate_scale"
KEY_COUNTS=(64 128)
SEEDS=(1 2 3)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=6
  PROPOSAL_COUNT=18
  PREFIX="v05_route_quality_candidate_scale_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
  NOISY_RATES=(0.25)
elif [[ "$MODE" == "full" ]]; then
...
run_case() {
  ...
  echo "quality-candidate-scale: ${scenario} apply=${quality_apply} candidate_beta=${candidate_beta}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
...
for key_count in "${KEY_COUNTS[@]}"; do
  for seed in "${SEEDS[@]}"; do
    for noisy_rate in "${NOISY_RATES[@]}"; do
      run_case "proxy-off" "none" "0.0" "$key_count" "$seed" "$noisy_rate"
      run_case "candidate-b0p25" "candidate-weight" "0.25" "$key_count" "$seed" "$noisy_rate"
      run_case "candidate-b0p50" "candidate-weight" "0.50" "$key_count" "$seed" "$noisy_rate"
      run_case "candidate-b0p75" "candidate-weight" "0.75" "$key_count" "$seed" "$noisy_rate"
    done
  done
done
