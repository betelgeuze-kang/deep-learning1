...
run_or_reuse "$RESULTS_DIR/v52j_measured_registry_absorb_summary.csv" "$ROOT_DIR/experiments/run_v52j_measured_registry_absorb.sh"
...
STAGES = [
    {
        "stage": "v52j",
        "summary": results / "v52j_measured_registry_absorb_summary.csv",
        "source_dir": results / "v52j_measured_registry_absorb" / "registry_001",
        "ready_field": "v52j_measured_registry_absorb_ready",
        ...
        "claim": "v52 measured registry with A/B/G/H over the same 1000-query set",
    },
    {
        "stage": "v53e",
...
v52j = read_summary(results / "v52j_measured_registry_absorb_summary.csv")
command_rows = [
    {
...
gate_rows = [
    ("measured-registry-replay", "pass", "v52j A/B/G/H measured registry is regenerated and copied"),
    ("same-query-source-local-systems", "pass", "A/B/G/H retain shared v53e query IDs and source manifest"),
    ...
    ("7b14b-real-rows", "blocked", "real C 7B-14B evidence directory is still missing"),
...
(run_dir / "README_RESULT.md").write_text(
    ...
    "```\n\n"
    "This bundle promotes the v52j measured registry into the one-command replay path. "
    "It includes A/B/G/H over the same frozen v53e 1000-query set and source manifest. "
    ...
    f"- query_rows={v52j['query_rows']}\n"
    f"- answer_rows={v52j['answer_rows']}\n"
    f"- citation_rows={v52j['citation_rows']}\n"
    f"- abstain_rows={v52j['abstain_rows']}\n"
    f"- wrong_answer_guard_rows={v52j['wrong_answer_guard_rows']}\n"
    f"- resource_rows={v52j['resource_rows']}\n"
    f"- routehint_rows={v52j['routehint_rows']}\n\n"
    "Still blocked: real C 7B-14B rows, real D/E 30B/70B LLM+RAG rows, optional 100B+ row or final deferral, complete-source audit rows, human expert review, human blind review, and release review.\n",
    encoding="utf-8",
)

(run_dir / "V59C_ONE_COMMAND_MEASURED_REGISTRY_BOUNDARY.md").write_text(
    "# v59c One-Command Measured Registry Boundary\n\n"
    "This is a one-command replay of the v52j local measured registry plus the current v53-v58 candidate chain. "
...
summary = {
    ...
    "measured_registry_ready": int(v52j["v52j_measured_registry_absorb_ready"]),
    "local_measured_systems": v52j["local_measured_systems"],
    "query_rows": int(v52j["query_rows"]),
    "answer_rows": int(v52j["answer_rows"]),
    "citation_rows": int(v52j["citation_rows"]),
    "abstain_rows": int(v52j["abstain_rows"]),
    "wrong_answer_guard_rows": int(v52j["wrong_answer_guard_rows"]),
    "resource_rows": int(v52j["resource_rows"]),
    "routehint_rows": int(v52j["routehint_rows"]),
