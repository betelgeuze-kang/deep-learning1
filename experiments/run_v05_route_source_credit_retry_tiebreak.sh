...
CYCLES_PER_EPOCH=12
PROPOSAL_COUNT=24
PREFIX="v05_route_source_credit_retry_tiebreak"
KEY_COUNTS=(128)
SEEDS=(1 2)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_retry_tiebreak_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=20
...
run_case() {
  ...
  local retry_candidates_label="${11}"
  local filter_mode="${12}"
  local filter_threshold="${13}"
  local scenario_slug="${arm//\//-}"
  local scenario="${scenario_slug}-k${key_count}-s${seed}"
  ...
  echo "retry-tiebreak: ${scenario} source=${fallback_source} policy=${retry_policy} tiebreak=${retry_tiebreak} priorities=${retry_priorities_label}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
...
run_arm_set() {
  ...
  run_case "noisy-filter" "$key_count" "$seed" noisy-route-code off fixed source-order \
    "raw-key:0.0,key-shape:0.0,noisy-route-code:0.0" \
    "raw-key:0.0+key-shape:0.0+noisy-route-code:0.0" \
    "raw-key,key-shape,noisy-route-code" \
    "raw-key+key-shape+noisy-route-code" \
    negative-credit 0.0
  run_case "policy-source-order" "$key_count" "$seed" noisy-route-code off source-credit source-order \
    "raw-key:0.0,key-shape:0.0,noisy-route-code:0.0" \
    "raw-key:0.0+key-shape:0.0+noisy-route-code:0.0" \
    "raw-key,key-shape,noisy-route-code" \
    "raw-key+key-shape+noisy-route-code" \
    negative-credit 0.0
  run_case "policy-keyshape-prior" "$key_count" "$seed" noisy-route-code off source-credit source-prior \
    "key-shape:0.2,raw-key:0.0,noisy-route-code:0.0" \
    "key-shape:0.2+raw-key:0.0+noisy-route-code:0.0" \
    "raw-key,key-shape,noisy-route-code" \
    "raw-key+key-shape+noisy-route-code" \
    negative-credit 0.0
  run_case "policy-noisy-penalty/mixed" "$key_count" "$seed" noisy-route-code off source-credit source-prior \
    "key-shape:0.2,raw-key:0.0,noisy-route-code:-1.0" \
    "key-shape:0.2+raw-key:0.0+noisy-route-code:-1.0" \
    "raw-key,key-shape,noisy-route-code" \
    "raw-key+key-shape+noisy-route-code" \
    negative-credit 0.0
  run_case "fixed-keyshape" "$key_count" "$seed" noisy-route-code key-shape fixed source-order \
    "raw-key:0.0,key-shape:0.0,noisy-route-code:0.0" \
    "raw-key:0.0+key-shape:0.0+noisy-route-code:0.0" \
    "raw-key,key-shape,noisy-route-code" \
    "raw-key+key-shape+noisy-route-code" \
