...
CYCLES_PER_EPOCH=16
PROPOSAL_COUNT=30
PREFIX="v05_route_source_credit_fallback_policy"

if [[ "$MODE" == "smoke" ]]; then
  EPOCHS=8
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_fallback_policy_smoke"
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=28
  CYCLES_PER_EPOCH=20
  PROPOSAL_COUNT=30
...
run_case() {
  ...
  local source_credit_learning="$6"
  local plasticity_ledger="$7"
  ...
  echo "fallback policy: ${scenario} fallback=${fallback_source} apply=${source_apply_mode} source_credit=${source_credit_learning} ledger=${plasticity_ledger}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
...
run_arm_set() {
  ...
  run_case "raw-key-ceiling" "$key_count" "$seed" raw-key off 0 0
  run_case "key-shape-learn-only" "$key_count" "$seed" key-shape off 1 0
  run_case "key-shape-ranking" "$key_count" "$seed" key-shape ranking 1 0
  run_case "key-shape-strength" "$key_count" "$seed" key-shape strength 1 0
  run_case "key-shape-ranking-strength" "$key_count" "$seed" key-shape ranking-strength 1 0
  run_case "noisy-learn-only" "$key_count" "$seed" noisy-route-code off 1 0
  run_case "noisy-ranking-strength" "$key_count" "$seed" noisy-route-code ranking-strength 1 0
}
