...
CYCLES_PER_EPOCH=10
PROPOSAL_COUNT=22
PREFIX="v05_route_quality_candidate_regression"
KEY_COUNTS=(64 128 256)
SEEDS=(1 2 3)
NOISY_RATES=(0.25 0.50)
ARMS=(
  "proxy-off:none:0.00:2.0"
  "candidate-b0p75-cap2p0:candidate-weight:0.75:2.0"
  "candidate-b1p50-cap2p0:candidate-weight:1.50:2.0"
  "candidate-b2p00-cap2p0:candidate-weight:2.00:2.0"
  "candidate-b2p00-cap3p0:candidate-weight:2.00:3.0"
)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=6
  PROPOSAL_COUNT=18
  PREFIX="v05_route_quality_candidate_regression_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
  NOISY_RATES=(0.25)
  ARMS=(
...
emit_aggregate() {
  ...
  awk -F, '
    NR == 1 {
      for (i = 1; i <= NF; i++) idx[$i] = i
      print "arm,key_count,noisy_source_rate,rows,qacc_mean,qacc_std,top_share_mean,factor_max_mean,selected_noisy_rate_mean,active_jump_rate_mean"
      next
    }
    {
    ...
    END {
      for (i = 1; i <= key_count_seen; i++) {
        ...
        printf "%s,%s,%s,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
          ...
          sum_fmax[key] / count[key], sum_noisy[key] / count[key],
...
run_case() {
  ...
  echo "quality-candidate-regression: ${scenario} apply=${quality_apply} beta=${candidate_beta} cap=${candidate_max}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
  ...
  for seed in "${SEEDS[@]}"; do
    for noisy_rate in "${NOISY_RATES[@]}"; do
      for arm_spec in "${ARMS[@]}"; do
        IFS=: read -r arm quality_apply candidate_beta candidate_max <<<"$arm_spec"
        run_case "$arm" "$quality_apply" "$candidate_beta" "$candidate_max" \
