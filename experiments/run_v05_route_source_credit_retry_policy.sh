...
CYCLES_PER_EPOCH=12
PROPOSAL_COUNT=24
PREFIX="v05_route_source_credit_retry_policy"
KEY_COUNTS=(128)
SEEDS=(1 2)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_retry_policy_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=20
...
compute_metrics() {
  ...
  awk -F, '
    BEGIN {
      split("fixture_query_byte_acc route_fallback_recall route_fallback_qacc route_source_credit_gap route_source_credit_noisy_mean route_source_credit_noisy_slashed_rate route_noisy_source_selected_rate route_source_filter_abstain_rate route_source_retry_used_rate route_source_retry_success_rate route_source_retry_raw_selected_rate route_source_retry_keyshape_selected_rate route_source_retry_noisy_selected_rate route_hint_candidate_lookup_count route_hint_value_read_distance_mean routing_trigger_rate active_jump_rate", names, " ")
    }
    NR == 1 {
...
run_case() {
  ...
  local retry_source="$5"
  local retry_policy="$6"
  local retry_candidates="$7"
  local retry_candidates_label="$8"
  local filter_mode="$9"
  local filter_threshold="${10}"
  ...
  echo "retry-policy: ${scenario} source=${fallback_source} retry=${retry_source} policy=${retry_policy} candidates=${retry_candidates_label}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
    ...
    --route-source-retry-candidates "$retry_candidates" \
    ...
    --route-source-filter-mode "$filter_mode" \
    --route-source-filter-threshold "$filter_threshold" \
    --csv "$csv_path"
...
run_arm_set() {
  ...
  run_case "noisy-filter" "$key_count" "$seed" noisy-route-code off fixed raw-key off negative-credit 0.0
  run_case "fixed-raw" "$key_count" "$seed" noisy-route-code raw-key fixed raw-key off negative-credit 0.0
  run_case "fixed-keyshape" "$key_count" "$seed" noisy-route-code key-shape fixed raw-key off negative-credit 0.0
  run_case "policy-mixed" "$key_count" "$seed" noisy-route-code off source-credit "raw-key,key-shape,noisy-route-code" "raw-key+key-shape+noisy-route-code" negative-credit 0.0
  run_case "policy-raw-noisy" "$key_count" "$seed" noisy-route-code off source-credit "raw-key,noisy-route-code" "raw-key+noisy-route-code" negative-credit 0.0
}
