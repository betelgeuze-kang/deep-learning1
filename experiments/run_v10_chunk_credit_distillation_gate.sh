...
awk -F, -v policy_csv="$POLICY_CSV" -v joint_csv="$JOINT_AGG_CSV" -v fallback_csv="$FALLBACK_AGG_CSV" -v contract_csv="$CONTRACT_SUMMARY_CSV" -v collection_csv="$COLLECTION_SUMMARY_CSV" -v training_csv="$TRAINING_SUMMARY_CSV" -v scorer_csv="$SCORER_SUMMARY_CSV" -v source_verified_scorer_csv="$SOURCE_VERIFIED_SCORER_SUMMARY_CSV" -v external_csv="$EXTERNAL_SUMMARY_CSV" -v source_csv="$SOURCE_SUMMARY_CSV" -v summary_csv="$SUMMARY_CSV" -v decision_csv="$DECISION_CSV" '
  ...
  FILENAME == contract_csv {
    contract_rows++
    contract_teacher_label_contract_ready = $cidx["teacher_label_contract_ready"] + 0
    contract_routing = $cidx["routing_trigger_rate"] + 0
    contract_jump = $cidx["active_jump_rate"] + 0
    next
  }
  FILENAME == collection_csv && FNR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in lidx)) die("missing h10 distillation teacher collection column: " required[i], 10)
    }
    next
  }
  FILENAME == collection_csv {
    collection_rows++
    teacher_correct_labels = $lidx["correct_labels"] + 0
    teacher_wrong_labels = $lidx["wrong_labels"] + 0
    teacher_near_miss_labels = $lidx["near_miss_labels"] + 0
    teacher_missing_query_labels = $lidx["missing_query_labels"] + 0
    teacher_abstain_labels = $lidx["abstain_labels"] + 0
    teacher_candidate_label_rows = $lidx["candidate_label_rows"] + 0
    teacher_grounded_candidate_rows = $lidx["grounded_candidate_rows"] + 0
    teacher_collection_contract_ready = $lidx["teacher_label_contract_ready"] + 0
    teacher_label_collection_ready = $lidx["teacher_label_collection_ready"] + 0
    teacher_external_labels_ready = $lidx["teacher_external_labels_ready"] + 0
    teacher_label_source = $lidx["label_source"]
    teacher_routing = $lidx["routing_trigger_rate"] + 0
    teacher_jump = $lidx["active_jump_rate"] + 0
    next
  }
  FILENAME == training_csv && FNR == 1 {
    for (i = 1; i <= NF; i++) tidx[$i] = i
    required_count = split("teacher_distillation_training_ready teacher_distillation_eval_ready action_accuracy learner_id routing_trigger_rate active_jump_rate", required, " ")
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in tidx)) die("missing h10 distillation teacher training column: " required[i], 12)
    }
    next
  }
  FILENAME == training_csv {
    ...
    teacher_distillation_eval_ready = $tidx["teacher_distillation_eval_ready"] + 0
    teacher_distillation_action_accuracy = $tidx["action_accuracy"] + 0
    teacher_learner_id = $tidx["learner_id"]
  ...
  FILENAME == source_verified_scorer_csv && FNR == 1 {
    for (i = 1; i <= NF; i++) vidx[$i] = i
    required_count = split("source_verified_feature_labels_ready source_verified_learned_chunk_scorer_ready feature_source_link_ready feature_label_source feature_csv_provided reason routing_trigger_rate active_jump_rate", required, " ")
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in vidx)) die("missing h10 distillation source-verified scorer column: " required[i], 18)
    }
    next
  }
  FILENAME == source_verified_scorer_csv {
    source_verified_scorer_rows++
    source_verified_feature_labels_ready = $vidx["source_verified_feature_labels_ready"] + 0
    source_verified_learned_chunk_scorer_ready = $vidx["source_verified_learned_chunk_scorer_ready"] + 0
    source_verified_feature_source_link_ready = $vidx["feature_source_link_ready"] + 0
    source_verified_feature_label_source = $vidx["feature_label_source"]
    source_verified_feature_csv_provided = $vidx["feature_csv_provided"] + 0
    source_verified_scorer_reason = $vidx["reason"]
    source_verified_scorer_routing = $vidx["routing_trigger_rate"] + 0
  ...
  FILENAME == external_csv && FNR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in eidx)) die("missing h10 distillation external ingestion column: " required[i], 14)
    }
    next
  }
  FILENAME == external_csv {
    external_rows++
    teacher_external_schema_ready = $eidx["external_schema_ready"] + 0
    teacher_external_label_source_ready = $eidx["external_label_source_ready"] + 0
    teacher_external_labels_ready = $eidx["teacher_external_labels_ready"] + 0
    teacher_external_label_source = $eidx["label_source"]
    external_routing = $eidx["routing_trigger_rate"] + 0
    external_jump = $eidx["active_jump_rate"] + 0
    next
  }
  FILENAME == source_csv && FNR == 1 {
    for (i = 1; i <= NF; i++) sidx[$i] = i
    required_count = split("teacher_source_source teacher_source_chain_verified real_teacher_source_verified action routing_trigger_rate active_jump_rate", required, " ")
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in sidx)) die("missing h10 distillation source verifier column: " required[i], 16)
    }
    next
  }
  FILENAME == source_csv {
    source_rows++
    teacher_external_source_evidence = $sidx["teacher_source_source"]
  ...
  END {
    ...
    teacher_grounded_span_coverage = 0.0
    if (teacher_candidate_label_rows > 0) {
      teacher_grounded_span_coverage = teacher_grounded_candidate_rows / teacher_candidate_label_rows
    }
    ...
    } else if (!teacher_label_collection_ready) {
    ...
    printf "%s,%s,%s,%d,%d,%d,%d,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%s,%s,%d,%d,%d,%s,%d,%s,%d,%d,%d,%s,%s,%d,%d,%s,%d,%d,%.6f,%s,%.6f,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%s,%s,%.6f,%.6f\n",
      ...
      source_verified_learned_chunk_scorer_ready,
      source_verified_feature_source_link_ready,
      ...
      teacher_external_label_source_ready,
      teacher_external_labels_ready,
      teacher_external_label_source,
      teacher_external_source_evidence,
    ...
    printf "source-verified-learned-chunk-scorer,%s,source_verified_ready=%d reason=%s\n",
    ...
    printf "external-label-source,%s,source_ready=%d\n",
      teacher_external_label_source_ready ? "pass" : "blocked",
      teacher_external_label_source_ready >> decision_csv
