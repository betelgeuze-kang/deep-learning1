...
v52y_input_available = int(
    summaries["v52y"].get("v52y_f_optional_final_policy_ready") == "1"
    or summaries["v52y"].get("v52y_dependency_blocker_ready") == "1"
)
if v52y_input_available != 1:
    raise SystemExit("v53t requires v52y ready or v52y dependency blocker evidence")
...
v53r = summaries["v53r"]
v53s = summaries["v53s"]
v52y_ready = int(v52y.get("v52y_f_optional_final_policy_ready", "0") == "1")
v52y_dependency_blocker_ready = int(v52y.get("v52y_dependency_blocker_ready", "0") == "1")
f_optional_final_disposition = v52y.get("f_optional_final_disposition", "blocked-by-v52y-dependency")
f_optional_final_disposition_ready = v52y.get("f_optional_final_disposition_ready", "0")
...
v53aq_internal_prebaseline_contract_rows = read_csv(v53aq_dir / "abgh_internal_prebaseline_contract_rows.csv")
family_query_rows = {
...
v53aq_question_only_selection_contract_ready = int(
    v53aq_selection_contract_by_field.get("sanitized_question", {}).get("selection_allowed") == "1"
    and v53aq_selection_contract_by_field.get("question", {}).get("selection_allowed") == "0"
    and v53aq.get("selection_sanitized_question_only", "0") == "1"
    and v53aq.get("source_locator_in_question_removed_rows", "0") == "4000"
    and all(
...
foundation_real_adapter_evaluator_rows = sum(
    ...
    and row.get("source_span_oracle_selection_used") == "0"
    and row.get("future_neighbor_used") == "0"
...
foundation_real_adapter_same_query_ledger_ready = int(
    ...
    and all(row.get("source_span_oracle_selection_used_any") == "0" for row in v53aq_same_query_internal_prebaseline_rows)
    and all(row.get("future_neighbor_used_any") == "0" for row in v53aq_same_query_internal_prebaseline_rows)
    ...
    and all(row.get("internal_real_adapter_metric_claim_ready") == "1" for row in v53aq_same_query_internal_prebaseline_rows)
    and all(row.get("public_real_system_performance_claim_ready") == "0" for row in v53aq_same_query_internal_prebaseline_rows)
)
foundation_real_adapter_internal_contract_rows = len(v53aq_internal_prebaseline_contract_rows)
foundation_real_adapter_internal_contract_ready_rows = sum(
    1 for row in v53aq_internal_prebaseline_contract_rows if row.get("contract_ready") == "1"
)
foundation_real_adapter_internal_contract_ready = int(
    foundation_real_adapter_internal_contract_rows == 4
    and foundation_real_adapter_internal_contract_ready_rows == 4
    and {row.get("system_id") for row in v53aq_internal_prebaseline_contract_rows} == set(abgh_systems)
    and all(row.get("same_query_set") == "1" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("same_evaluator_contract") == "1" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("same_resource_contract") == "1" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("selection_question_text_only") == "1" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("selection_oracle_field_used") == "0" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("source_span_oracle_selection_used") == "0" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("future_neighbor_used") == "0" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("expected_answer_oracle_replay_rows") == "0" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("deterministic_source_span_adapter_rows") == "0" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("internal_real_adapter_metric_claim_ready") == "1" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("public_real_system_performance_claim_ready") == "0" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("public_comparison_claim_ready") == "0" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("required_30b_baseline_ready") == "0" for row in v53aq_internal_prebaseline_contract_rows)
    and all(row.get("required_70b_baseline_ready") == "0" for row in v53aq_internal_prebaseline_contract_rows)
)
foundation_real_adapter_evidence_ready = int(
    ...
    and foundation_real_adapter_internal_contract_ready == 1
    ...
    and v53aq.get("selection_runtime_guard_passed_rows") == "4000"
    and v53aq.get("selection_oracle_field_used") == "0"
    and v53aq.get("source_span_oracle_selection_used") == "0"
    and v53aq.get("future_neighbor_used") == "0"
    ...
    and v53aq.get("internal_real_adapter_metric_claim_ready") == "1"
    and v53aq.get("public_real_system_performance_claim_ready") == "0"
...
requirements = [
    {
        ...
        "status": "pass" if f_optional_final_disposition_ready == "1" else "blocked",
        "required_value": "supplied-ready-or-deferred-with-reason-final",
        "actual_value": f_optional_final_disposition,
        "reason": "F optional 100B+ baseline must be supplied or explicitly final-deferred; this does not block the v53 machine foundation freeze",
    },
    {
...
real_adapter_freeze_rows = [
    {
        ...
        "actual_value": f"same_query_hash={v53aq_same_complete_source_query_hash}; same_query_rows_ready={foundation_real_adapter_same_query_rows_ready}; same_query_ledger_ready={foundation_real_adapter_same_query_ledger_ready}; same_query_ledger_rows={foundation_real_adapter_same_query_ledger_rows}; internal_contract_ready={foundation_real_adapter_internal_contract_ready}; internal_contract_rows={foundation_real_adapter_internal_contract_rows}",
    ...
    },
    {
        ...
        "required_value": "adapter selection may read only sanitized question text; raw question locators and oracle/source/answer fields forbidden",
        "actual_value": f"selection_question_text_only={v53aq.get('selection_question_text_only', '0')}; selection_sanitized_question_only={v53aq.get('selection_sanitized_question_only', '0')}; source_locator_in_question_removed_rows={v53aq.get('source_locator_in_question_removed_rows', '0')}; selection_oracle_field_used={v53aq.get('selection_oracle_field_used', '1')}",
    ...
    },
    {
        ...
        "actual_value": f"evaluator_rows={len(v53aq_evaluator_rows)}; separate_evaluator_rows={foundation_real_adapter_evaluator_rows}; answer_hash_match_rows={v53aq.get('answer_hash_match_rows', '0')}; coherent_wrong_key_rows={v53aq.get('coherent_wrong_key_rows', '0')}; internal_real_adapter_metric_claim_ready={v53aq.get('internal_real_adapter_metric_claim_ready', '0')}; public_real_system_performance_claim_ready={v53aq.get('public_real_system_performance_claim_ready', '1')}",
        "evidence_path": "source_v53aq/abgh_evaluator_rows.csv",
        "claim_boundary": "Allows internal A/B/G/H real-adapter metric wording; public performance and public comparison remain blocked",
    },
    {
        "certificate_id": "v53-complete-source-abgh-real-adapter-freeze",
        "criterion_id": "public-comparison-boundary-closed",
        "status": "pass" if v53aq.get("public_real_system_performance_claim_ready") == "0" and v53aq.get("public_comparison_claim_ready") == "0" and v53aq.get("required_30b_baseline_ready") == "0" and v53aq.get("required_70b_baseline_ready") == "0" else "blocked",
        "required_value": "public performance, public comparison, D/E replacement, and release claims blocked",
        "actual_value": f"public_real_system_performance_claim_ready={v53aq.get('public_real_system_performance_claim_ready', '0')}; public_comparison_claim_ready={v53aq.get('public_comparison_claim_ready', '0')}; required_30b_baseline_ready={v53aq.get('required_30b_baseline_ready', '0')}; required_70b_baseline_ready={v53aq.get('required_70b_baseline_ready', '0')}",
        "evidence_path": "source_v53aq/v53aq_complete_source_abgh_real_adapter_measured_summary.csv",
        "claim_boundary": "Explicitly forbids public performance and public comparison wording until D/E and release evidence exist",
    },
]
write_csv(
...
pm_acceptance_evidence_rows = [
    ...
    pm_acceptance_row(
        ...
        "M3-abgh-same-query-measured-run",
        "pass" if foundation_real_adapter_same_query_ledger_ready and foundation_real_adapter_internal_contract_ready else "blocked",
        "source_v53aq/abgh_same_query_internal_prebaseline_rows.csv",
        f"same_query_ledger_rows={foundation_real_adapter_same_query_ledger_rows}; internal_contract_rows={foundation_real_adapter_internal_contract_rows}; internal_contract_ready={foundation_real_adapter_internal_contract_ready}; question_text_only={v53aq.get('selection_question_text_only', '0')}; oracle_field_used={v53aq.get('selection_oracle_field_used', '1')}; coherent_wrong_key_rows={v53aq.get('coherent_wrong_key_rows', '0')}; internal_real_adapter_metric_claim_ready={v53aq.get('internal_real_adapter_metric_claim_ready', '0')}; public_real_system_performance_claim_ready={v53aq.get('public_real_system_performance_claim_ready', '1')}; public_comparison_claim_ready={v53aq.get('public_comparison_claim_ready', '0')}",
    ...
    pm_acceptance_row(
        ...
        "M3-abgh-same-query-measured-run",
        "pass" if v53aq.get("public_real_system_performance_claim_ready") == "0" and v53aq.get("public_comparison_claim_ready") == "0" and v53s["quality_comparison_claim_ready"] == "0" else "blocked",
        "complete_source_abgh_real_adapter_freeze_rows.csv",
        f"v53aq_public_real_system_performance_claim_ready={v53aq.get('public_real_system_performance_claim_ready', '0')}; v53aq_public_comparison_claim_ready={v53aq.get('public_comparison_claim_ready', '0')}; quality_comparison_claim_ready={v53s['quality_comparison_claim_ready']}; required_30b_baseline_ready={v53aq.get('required_30b_baseline_ready', '0')}; required_70b_baseline_ready={v53aq.get('required_70b_baseline_ready', '0')}",
        "Explicitly forbids public performance, public comparison, v53-ready, and release-ready wording until D/E, review, and release evidence exist",
    ),
]
write_csv(
...
v1_exit_repo_count_within_band = int(10 <= int(v53i["repo_count"]) <= 30)
v1_exit_query_rows_within_band = int(1000 <= len(v53i_query_rows) <= 3000)
v1_exit_negative_control_share_ready = int(int(v53i["negative_abstain_rows"]) * 10 >= len(v53i_query_rows))


def v1_exit_criterion_row(
    criterion_id,
    requirement,
    status,
    required_value,
    ...
    return {
        ...
        "replay_command": "experiments/test_v53t_complete_source_audit_readiness_gate.sh",
        "claim_boundary_status": "pass" if status == "pass" else "blocked",
        "replay_artifact_status": "pass" if evidence_file.is_file() and evidence_file.stat().st_size > 0 else "blocked",
        "blocker_false_positive_status": "pass",
        "tests_only_merge_condition": "0",
        "claim_boundary": claim_boundary,
    }
...
v1_exit_criteria_rows = [
    v1_exit_criterion_row(
        "repo-count-band-10-30",
        "v1.0 public repo code/doc QA must cover 10-30 pinned public repos",
        "pass" if v1_exit_repo_count_within_band and foundation_direct_pinned_manifest_ready else "blocked",
        "10 <= repo_count <= 30 plus direct pinned manifest",
        f"repo_count={v53i['repo_count']}; direct_pinned_manifest_ready={foundation_direct_pinned_manifest_ready}; repo_manifest_rows={len(v53i_repo_coverage_rows)}",
        "source_v53i/source_v53h/source_v53g/complete_source_repo_coverage_rows.csv",
        "Allows v53 machine foundation repo-count wording only; no release or human-reviewed quality claim",
    ),
    v1_exit_criterion_row(
        "query-row-band-1000-3000",
        "v1.0 public repo code/doc QA must cover 1000-3000 source-span-bound query rows",
        "pass" if v1_exit_query_rows_within_band and foundation_query_span_binding_audit_ready else "blocked",
        "1000 <= query_rows <= 3000 and all query/span binding checks pass",
    ...
    v1_exit_criterion_row(
        ...
        "M2 corpus must include at least 10% negative/abstain rows plus unsupported, missing, and doc-code conflict controls",
        "pass" if v1_exit_negative_control_share_ready and unsupported_control_rows > 0 and missing_specific_control_rows > 0 and doc_code_conflict_rows > 0 else "blocked",
        "negative_abstain_rows >= 10% of query rows; unsupported/missing/doc-code conflict rows present",
        f"negative_abstain_rows={v53i['negative_abstain_rows']}; query_rows={len(v53i_query_rows)}; unsupported_control_rows={unsupported_control_rows}; missing_specific_control_rows={missing_specific_control_rows}; doc_code_conflict_rows={doc_code_conflict_rows}",
        "source_v53i/complete_source_query_family_rows.csv",
        "Allows corpus-control wording only; not model-quality or release wording",
    ),
    v1_exit_criterion_row(
        "answer-citation-separate-evaluator",
        "Evaluator must score answer, citation/source, and resource binding separately",
        "pass" if foundation_direct_evaluator_separate_rows == 4000 and foundation_real_adapter_evaluator_rows == 4000 else "blocked",
        "4000 deterministic evaluator rows and 4000 real-adapter evaluator rows keep answer/citation/resource checks separate",
        f"deterministic_separate_evaluator_rows={foundation_direct_evaluator_separate_rows}; real_adapter_separate_evaluator_rows={foundation_real_adapter_evaluator_rows}",
        "source_v53ap/abgh_evaluator_rows.csv",
        "Allows separated evaluator-contract wording; not human-reviewed answer quality",
    ),
    v1_exit_criterion_row(
        "abgh-same-query-internal-prebaseline",
        "A/B/G/H must use the same v53i query/source set before D/E public baselines",
        "pass" if abgh_same_query_ready and foundation_real_adapter_same_query_ledger_ready and v53aq_same_complete_source_query_hash else "blocked",
        "A/B/G/H deterministic rows and sanitized-question-only real-adapter ledger share the same query hash",
        f"abgh_same_query_ready={abgh_same_query_ready}; same_query_ledger_rows={foundation_real_adapter_same_query_ledger_rows}; v53aq_same_complete_source_query_hash={v53aq_same_complete_source_query_hash}; public_comparison_claim_ready={v53aq.get('public_comparison_claim_ready', '0')}",
        "source_v53aq/abgh_same_query_internal_prebaseline_rows.csv",
        "Allows internal v1.0 pre-baseline A/B/G/H wording only; public comparison remains blocked without D/E and review evidence",
    ),
    v1_exit_criterion_row(
        "claim-boundary-replay-blocker-gate",
        "M2/M3 merge evidence must be claim-bounded, replayable, and not tests-only",
        "pass" if pm_acceptance_evidence_ready_rows == pm_acceptance_evidence_row_count and pm_acceptance_evidence_tests_only_rows == 0 and v53aq.get("public_real_system_performance_claim_ready") == "0" and v53aq.get("public_comparison_claim_ready") == "0" else "blocked",
        "all PM acceptance rows ready, replayable, blocker-safe, and public comparison/performance claims closed",
        f"pm_acceptance_evidence_ready_rows={pm_acceptance_evidence_ready_rows}; pm_acceptance_evidence_rows={pm_acceptance_evidence_row_count}; tests_only_rows={pm_acceptance_evidence_tests_only_rows}; public_real_system_performance_claim_ready={v53aq.get('public_real_system_performance_claim_ready', '1')}; public_comparison_claim_ready={v53aq.get('public_comparison_claim_ready', '1')}",
        "complete_source_pm_acceptance_evidence_rows.csv",
        "Allows PM freeze acceptance wording only; forbids v53-ready, public comparison, public performance, and release claims",
    ),
]
write_csv(
    run_dir / "complete_source_v1_exit_criteria_rows.csv",
    list(v1_exit_criteria_rows[0].keys()),
    v1_exit_criteria_rows,
)
v1_exit_criteria_row_count = len(v1_exit_criteria_rows)
v1_exit_criteria_ready_rows = sum(1 for row in v1_exit_criteria_rows if row["status"] == "pass")
v1_exit_criteria_blocked_rows = v1_exit_criteria_row_count - v1_exit_criteria_ready_rows
v1_exit_machine_foundation_ready = int(v1_exit_criteria_blocked_rows == 0)
...
metric = {
    ...
    "v52y_f_optional_final_policy_ready": v52y.get("v52y_f_optional_final_policy_ready", "0"),
    "v52y_dependency_blocker_ready": str(v52y_dependency_blocker_ready),
    ...
    "v1_exit_criteria_rows": str(v1_exit_criteria_row_count),
    "v1_exit_criteria_ready_rows": str(v1_exit_criteria_ready_rows),
    "v1_exit_criteria_blocked_rows": str(v1_exit_criteria_blocked_rows),
    "v1_exit_repo_count_within_band": str(v1_exit_repo_count_within_band),
    "v1_exit_query_rows_within_band": str(v1_exit_query_rows_within_band),
    "v1_exit_negative_control_share_ready": str(v1_exit_negative_control_share_ready),
    "v1_exit_machine_foundation_ready": str(v1_exit_machine_foundation_ready),
    ...
    "foundation_real_adapter_internal_contract_ready": str(foundation_real_adapter_internal_contract_ready),
    "foundation_real_adapter_internal_contract_rows": str(foundation_real_adapter_internal_contract_rows),
    "foundation_real_adapter_internal_contract_ready_rows": str(foundation_real_adapter_internal_contract_ready_rows),
    ...
    "negative_abstain_rows": v53i["negative_abstain_rows"],
    ...
    "v53aq_selection_runtime_guard_passed_rows": v53aq.get("selection_runtime_guard_passed_rows", "0"),
    "v53aq_selection_oracle_field_used": v53aq.get("selection_oracle_field_used", "1"),
    "v53aq_source_span_oracle_selection_used": v53aq.get("source_span_oracle_selection_used", "1"),
    "v53aq_future_neighbor_used": v53aq.get("future_neighbor_used", "1"),
...
decision_rows = [
    {"gate": "v52y-f-final-policy-input", "status": "pass" if v52y_ready else "blocked", "reason": f"f_optional_final_disposition={f_optional_final_disposition}; v52y_dependency_blocker_ready={v52y_dependency_blocker_ready}"},
...
- v52y_f_optional_final_policy_ready={v52y.get('v52y_f_optional_final_policy_ready', '0')}
- v52y_dependency_blocker_ready={v52y_dependency_blocker_ready}
- f_optional_final_disposition_ready={f_optional_final_disposition_ready}
- f_optional_final_disposition={f_optional_final_disposition}
- complete_source_repo_count={v53i['repo_count']}
- complete_source_query_rows={v53i['complete_source_query_rows']}
...
- v1_exit_criteria_rows={v1_exit_criteria_row_count}
- v1_exit_criteria_ready_rows={v1_exit_criteria_ready_rows}
- v1_exit_criteria_blocked_rows={v1_exit_criteria_blocked_rows}
- v1_exit_repo_count_within_band={v1_exit_repo_count_within_band}
- v1_exit_query_rows_within_band={v1_exit_query_rows_within_band}
- v1_exit_negative_control_share_ready={v1_exit_negative_control_share_ready}
- v1_exit_machine_foundation_ready={v1_exit_machine_foundation_ready}
...
- foundation_real_adapter_internal_contract_ready={foundation_real_adapter_internal_contract_ready}
- foundation_real_adapter_internal_contract_rows={foundation_real_adapter_internal_contract_rows}
- foundation_real_adapter_internal_contract_ready_rows={foundation_real_adapter_internal_contract_ready_rows}
...
manifest = {
    ...
    "v52y_f_optional_final_policy_ready": v52y_ready,
    "v52y_dependency_blocker_ready": v52y_dependency_blocker_ready,
    "f_optional_final_disposition_ready": int(f_optional_final_disposition_ready),
    "f_optional_final_disposition": f_optional_final_disposition,
    "machine_complete_source_surface_ready": machine_complete_source_surface_ready,
    "review_return_ready": review_return_ready,
    "quality_comparison_claim_ready": 0,
    ...
    "v1_exit_negative_control_share_ready": v1_exit_negative_control_share_ready,
    "v1_exit_machine_foundation_ready": v1_exit_machine_foundation_ready,
    "v1_exit_criteria_rows_sha256": sha256(run_dir / "complete_source_v1_exit_criteria_rows.csv"),
    "foundation_machine_freeze_ready": foundation_machine_freeze_ready,
    "foundation_direct_evidence_ready": foundation_direct_evidence_ready,
    "foundation_query_span_binding_audit_ready": foundation_query_span_binding_audit_ready,
    "foundation_query_span_binding_audit_rows": foundation_query_span_binding_audit_rows,
    ...
    "foundation_direct_evaluator_separate_rows": foundation_direct_evaluator_separate_rows,
    "foundation_direct_same_query_rows_ready": foundation_direct_same_query_rows_ready,
    "negative_abstain_rows": int(v53i["negative_abstain_rows"]),
    "unsupported_control_rows": unsupported_control_rows,
    "ambiguous_control_rows": ambiguous_control_rows,
    "missing_specific_control_rows": missing_specific_control_rows,
    "doc_code_conflict_rows": doc_code_conflict_rows,
    ...
    "v53aq_selection_sanitized_question_only": int(v53aq.get("selection_sanitized_question_only", "0")),
    "v53aq_source_locator_in_question_removed_rows": int(v53aq.get("source_locator_in_question_removed_rows", "0")),
    "v53aq_selection_runtime_guard_passed_rows": int(v53aq.get("selection_runtime_guard_passed_rows", "0")),
    "v53aq_selection_oracle_field_used": int(v53aq.get("selection_oracle_field_used", "1")),
    "v53aq_source_span_oracle_selection_used": int(v53aq.get("source_span_oracle_selection_used", "1")),
    "v53aq_future_neighbor_used": int(v53aq.get("future_neighbor_used", "1")),
    ...
    "v53aq_internal_real_adapter_metric_claim_ready": int(v53aq.get("internal_real_adapter_metric_claim_ready", "0")),
    "v53aq_public_real_system_performance_claim_ready": int(v53aq.get("public_real_system_performance_claim_ready", "1")),
