...
CYCLES_PER_EPOCH=10
PROPOSAL_COUNT=22
PREFIX="v05_route_quality_candidate_saturation"
KEY_COUNTS=(128)
SEEDS=(1 2 3)
NOISY_RATES=(0.25 0.50)
BETAS=(0.75 1.00 1.25 1.50 2.00)
CAPS=(2.0 3.0 4.0)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=6
  PROPOSAL_COUNT=18
  PREFIX="v05_route_quality_candidate_saturation_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
  NOISY_RATES=(0.25)
  BETAS=(1.25 1.50 2.00)
  CAPS=(2.0 3.0)
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=16
  CYCLES_PER_EPOCH=14
  ...
  BETAS=(0.50 0.75 1.00 1.25 1.50 2.00)
  CAPS=(2.0 3.0 4.0)
fi
...
run_case() {
  local candidate_beta="$1"
  local candidate_max="$2"
  local key_count="$3"
  local seed="$4"
  local noisy_rate="$5"
  ...
  local cap_token
  local noisy_token
  beta_token="$(token_for_float "$candidate_beta")"
  cap_token="$(token_for_float "$candidate_max")"
  noisy_token="$(token_for_float "$noisy_rate")"

  local arm="candidate-b${beta_token}-cap${cap_token}"
  local scenario="${arm}-k${key_count}-s${seed}-n${noisy_token}"
  ...
  echo "quality-candidate-saturation: ${scenario} beta=${candidate_beta} cap=${candidate_max}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
...
for key_count in "${KEY_COUNTS[@]}"; do
  for seed in "${SEEDS[@]}"; do
    for noisy_rate in "${NOISY_RATES[@]}"; do
      for candidate_beta in "${BETAS[@]}"; do
        for candidate_max in "${CAPS[@]}"; do
          run_case "$candidate_beta" "$candidate_max" "$key_count" "$seed" "$noisy_rate"
        done
      done
    done
  done
