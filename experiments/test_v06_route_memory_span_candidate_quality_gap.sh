...
"$ROOT_DIR/experiments/run_v06_route_memory_span_candidate_quality_gap.sh" --smoke

SUMMARY_CSV="$RESULTS_DIR/v06_route_memory_span_candidate_quality_gap_smoke_summary.csv"
AGG_CSV="$RESULTS_DIR/v06_route_memory_span_candidate_quality_gap_smoke_aggregate.csv"
...
awk -F, '
  ...
  END {
    if (rows < 5) {
      printf "expected at least 5 span candidate-quality rows, got %d\n", rows > "/dev/stderr"
      exit 3
    }
    if (weak_recall < 0.99) {
...
awk -F, '
  NR == 1 {
    for (i = 1; i <= NF; i++) idx[$i] = i
    required_count = split("rows weak_all_recall weak_all_top1 keyshape_all_top1 weak_correct_key_share keyshape_correct_key_share weak_key_entropy keyshape_key_entropy weak_top_key_consistency keyshape_top_key_consistency keyshape_span_exact_delta routing_trigger_rate_mean active_jump_rate_mean", required, " ")
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        printf "missing span candidate-quality aggregate column: %s\n", required[i] > "/dev/stderr"
        exit 8
      }
    }
    next
...
echo "v06 route-memory span candidate-quality gap smoke passed"
