...
while IFS=$'\t' read -r benchmark_family dataset_uri source_hash result_uri provenance_hash routing_trigger_rate active_jump_rate; do
  ((evidence_rows += 1))
...
if [[ "$real_evidence_format_ready" == "1" &&
      ...
      "$local_dataset_uri_rows" -eq "$benchmark_families" &&
      "$local_result_uri_rows" -eq "$benchmark_families" &&
...
if [[ "$real_evidence_format_ready" == "1" && "$artifact_verifier_ready" == "0" ]]; then
  if [[ "$local_dataset_uri_rows" -ne "$benchmark_families" ||
        "$local_result_uri_rows" -ne "$benchmark_families" ]]; then
...
{
  echo "benchmark_scope,benchmark_families,evidence_source,real_evidence_format_ready,evidence_rows,local_dataset_uri_rows,local_result_uri_rows,source_hash_verified_rows,provenance_hash_verified_rows,artifact_verifier_ready,real_external_benchmark_verified,action,routing_trigger_rate,active_jump_rate"
  printf "route-memory-v08g,%d,%s,%d,%d,%d,%d,%d,%d,%d,%d,%s,%.6f,%.6f\n" \
    "$benchmark_families" \
    "$evidence_source" \
...

{
  ...
  printf "real-evidence-format,%s,format_ready=%d\n" \
    ...
    "$real_evidence_format_ready"
  printf "local-artifacts,%s,dataset_rows=%d result_rows=%d\n" \
