...
awk -F, '
  NR == 1 {
    ...
    split("scenario route_credit_learning fixture_query_byte_acc route_credit_correct_mean route_credit_wrong_mean route_credit_gap route_credit_rewarded_rate route_credit_slashed_rate route_credit_top1_rate route_credit_qacc", required, " ")
  ...
  }
  {
    ...
    if (scenario == "credit_off") {
      off_seen = 1
      if (learning != 0) {
        printf "expected credit_off learning=0, got %d\n", learning > "/dev/stderr"
        exit 3
      }
    } else if (scenario == "credit_on") {
      on_seen = 1
      if (learning != 1) {
        printf "expected credit_on learning=1, got %d\n", learning > "/dev/stderr"
        exit 4
      }
