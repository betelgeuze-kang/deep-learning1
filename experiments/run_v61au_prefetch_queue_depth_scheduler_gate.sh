...
PREFIX="v61au_prefetch_queue_depth_scheduler_gate"
RUN_ID="${V61AU_RUN_ID:-gate_001}"
RUN_DIR="$RESULTS_DIR/$PREFIX/$RUN_ID"
SUMMARY_CSV="$RESULTS_DIR/${PREFIX}_summary.csv"
DECISION_CSV="$RESULTS_DIR/${PREFIX}_decision.csv"

if [[ "${V61AU_REUSE_EXISTING:-0}" == "1" && -s "$SUMMARY_CSV" && -s "$RUN_DIR/sha256_manifest.csv" ]]; then
  echo "v61au_prefetch_queue_depth_scheduler_gate_dir: $RUN_DIR"
...
V61AT_REUSE_EXISTING=1 "$ROOT_DIR/experiments/run_v61at_prefetch_overlap_admission_gate.sh" >/dev/null
...
def token_index_from_schedule(schedule_id):
    match = re.search(r"v61ac_schedule_(\d+)_", schedule_id)
    if not match:
        raise SystemExit(f"cannot parse token index from schedule id: {schedule_id}")
    return int(match.group(1))


v61at_dir = results / "v61at_prefetch_overlap_admission_gate" / "gate_001"
v61as_dir = results / "v61as_hotset_reuse_admission_gate" / "gate_001"
v61z_dir = results / "v61z_hotset_direct_io_replay" / "replay_001"
v61at_summary = read_csv(results / "v61at_prefetch_overlap_admission_gate_summary.csv")[0]
if v61at_summary.get("v61at_prefetch_overlap_admission_gate_ready") != "1":
    raise SystemExit("v61au requires v61at_prefetch_overlap_admission_gate_ready=1")
if v61at_summary.get("steady_state_prefetch_overlap_ready") != "1":
    raise SystemExit("v61au requires steady_state_prefetch_overlap_ready=1")

for src, rel in [
    (results / "v61at_prefetch_overlap_admission_gate_summary.csv", "source_v61at/v61at_prefetch_overlap_admission_gate_summary.csv"),
    (results / "v61at_prefetch_overlap_admission_gate_decision.csv", "source_v61at/v61at_prefetch_overlap_admission_gate_decision.csv"),
    (v61at_dir / "prefetch_overlap_token_rows.csv", "source_v61at/prefetch_overlap_token_rows.csv"),
    (v61at_dir / "prefetch_overlap_window_rows.csv", "source_v61at/prefetch_overlap_window_rows.csv"),
    (v61at_dir / "runtime_gap_rows.csv", "source_v61at/runtime_gap_rows.csv"),
    (v61at_dir / "sha256_manifest.csv", "source_v61at/sha256_manifest.csv"),
    (results / "v61as_hotset_reuse_admission_gate_summary.csv", "source_v61as/v61as_hotset_reuse_admission_gate_summary.csv"),
    (v61as_dir / "hotset_reuse_page_rows.csv", "source_v61as/hotset_reuse_page_rows.csv"),
...
read_p95_ms = float(v61at_summary["ssd_read_latency_ms_p95_per_page"])
compute_window_ms = float(v61at_summary["token_page_kernel_compute_window_ms"])
min_slack = float(v61at_summary["min_steady_state_overlap_slack_ms"])
page_bytes = int(page_rows[0]["page_bytes"])
...
for token_index in range(len(token_rows)):
    ...
    scheduler_token_rows.append(
        {
            "scheduler_token_id": f"v61au_scheduler_token_{token_index:04d}",
            "prefetch_token_id": token["prefetch_token_id"],
    ...
    for local_slot, page in enumerate(token_pages):
        ...
        issue_rows.append(
            {
                "prefetch_issue_id": f"v61au_prefetch_issue_{len(issue_rows):04d}",
                "scheduler_token_id": f"v61au_scheduler_token_{token_index:04d}",
                "reuse_page_id": page["reuse_page_id"],
                ...
                "prefetch_issue_status": issue_status,
                "actual_async_prefetch_execution_ready": "0",
                "checkpoint_payload_bytes_committed_to_repo": "0",
                "actual_model_generation_ready": "0",
                "production_latency_claim_ready": "0",
...
queue_depth_rows.append(
    {
        "queue_depth_profile_id": "v61au_sampled_steady_state_qd4",
        "configured_prefetch_queue_depth": str(configured_queue_depth),
...
deadline_rows.extend(
    [
        {
            "requirement_id": "v61at-prefetch-overlap-input",
            "status": "pass",
            "actual": v61at_summary["steady_state_prefetch_overlap_ready"],
            "required": "1",
            "reason": "steady-state overlap evidence is bound",
        },
        {
            "requirement_id": "steady-state-queue-depth",
            ...
            "reason": "configured queue depth covers sampled steady-state cold-fill fanout",
        },
        {
            "requirement_id": "steady-state-deadline",
            ...
            "reason": "all steady-state cold-fill pages complete before the target-token deadline",
        },
        {
        ...
        },
        {
            ...
            "actual": "0",
            "required": "1",
            "reason": "v61au is a scheduler admission ledger, not io_uring execution",
        },
        {
            ...
            "actual": "0",
            "required": "0",
            "reason": "v61au emits scheduling rows only",
        },
    ]
)

write_csv(run_dir / "prefetch_scheduler_token_rows.csv", list(scheduler_token_rows[0].keys()), scheduler_token_rows)
write_csv(run_dir / "prefetch_scheduler_issue_rows.csv", list(issue_rows[0].keys()), issue_rows)
...
metric = {
    "metric_id": "v61au_prefetch_queue_depth_scheduler_metrics",
    "model_id": model_id,
    ...
    "steady_state_deadline_met_rows": str(steady_deadline_met_rows),
    "steady_state_deadline_miss_rows": str(steady_deadline_miss_rows),
    ...
    "steady_state_scheduler_ready": str(steady_scheduler_ready),
    "bootstrap_scheduler_ready": str(bootstrap_scheduler_ready),
    "prefetch_scheduler_admission_ready": str(prefetch_scheduler_admission_ready),
    "queue_depth_control_ready": "1",
    "actual_async_prefetch_execution_ready": "0",
    "actual_io_uring_execution_ready": "0",
    "registered_buffers_ready": "0",
...
write_csv(run_dir / "prefetch_scheduler_metric_rows.csv", list(metric.keys()), [metric])
write_csv(summary_csv, list(metric.keys())[1:], [{k: v for k, v in metric.items() if k != "metric_id"}])

runtime_gap_rows = [
    ("v61at-prefetch-overlap-input", "ready", "v61at steady-state overlap evidence is bound"),
    ("steady-state-queue-depth", "ready" if max_steady_required_depth <= configured_queue_depth else "blocked", f"max_steady_state_required_queue_depth={max_steady_required_depth}"),
    ("steady-state-deadline", "ready" if steady_scheduler_ready else "blocked", f"steady_state_deadline_met_rows={steady_deadline_met_rows}/{steady_issue_rows}"),
    ("bootstrap-prefetch-scheduler", "blocked", "bootstrap cold fill has no previous compute window"),
    ("actual-async-prefetch-execution", "blocked", "io_uring or equivalent async prefetch is not executed by v61au"),
    ("registered-buffer-prefetch", "blocked", "registered buffers are not allocated by v61au"),
    ("full-runtime-admission", "blocked", "sampled scheduler admission is not complete runtime admission"),
...
decision_rows = [
    {"gate": "v61at-prefetch-overlap-input", "status": "pass", "reason": "v61at steady-state overlap evidence is ready"},
    {"gate": "steady-state-queue-depth", "status": "pass" if max_steady_required_depth <= configured_queue_depth else "blocked", "reason": f"max_steady_state_required_queue_depth={max_steady_required_depth}"},
    {"gate": "steady-state-deadline", "status": "pass" if steady_scheduler_ready else "blocked", "reason": f"steady_state_deadline_met_rows={steady_deadline_met_rows}/{steady_issue_rows}"},
    ...
    {"gate": "actual-async-prefetch-execution", "status": "blocked", "reason": "v61au does not execute io_uring prefetch"},
    {"gate": "full-runtime-admission", "status": "blocked", "reason": "sampled scheduler admission is not full runtime admission"},
...
boundary = f"""# v61au Prefetch Queue-Depth Scheduler Gate Boundary

This gate consumes v61at sampled prefetch-overlap evidence and turns the
steady-state cold-fill pages into queue-depth/deadline scheduler rows.
...
- steady_state_deadline_met_rows={steady_deadline_met_rows}
- steady_state_deadline_miss_rows={steady_deadline_miss_rows}
...
- prior_token_compute_window_ms={fmt(compute_window_ms)}
- min_deadline_slack_ms={fmt(min_slack)}
- steady_state_scheduler_ready={steady_scheduler_ready}
- bootstrap_scheduler_ready={bootstrap_scheduler_ready}
- prefetch_scheduler_admission_ready={prefetch_scheduler_admission_ready}
- queue_depth_control_ready=1
- actual_async_prefetch_execution_ready=0
- actual_io_uring_execution_ready=0
- registered_buffers_ready=0
...
Allowed wording: sampled steady-state queue-depth/deadline scheduler admission
for the v61at hotset replay.

Blocked wording: actual io_uring prefetch execution, registered-buffer
prefetch, bootstrap cold-start solved, full runtime admission, actual Mixtral
generation, production latency, near-frontier quality, or release readiness.
"""
(run_dir / "V61AU_PREFETCH_QUEUE_DEPTH_SCHEDULER_GATE_BOUNDARY.md").write_text(boundary, encoding="utf-8")

manifest = {
    "artifact": "v61au_prefetch_queue_depth_scheduler_gate",
...
(run_dir / "v61au_prefetch_queue_depth_scheduler_gate_manifest.json").write_text(
