...
CYCLES_PER_EPOCH=10
PROPOSAL_COUNT=22
PREFIX="v05_route_quality_source_norm"
KEY_COUNTS=(64 128)
SEEDS=(1 2 3)
NOISY_RATES=(0.25)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=6
  PROPOSAL_COUNT=18
  PREFIX="v05_route_quality_source_norm_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
  NOISY_RATES=(0.25)
elif [[ "$MODE" == "full" ]]; then
...
compute_metrics() {
  ...
  awk -F, '
    BEGIN {
      name_count = split("fixture_query_byte_acc route_quality_apply_active route_quality_source_normalization_active route_quality_source_ranking_delta_mean route_quality_selected_raw_rate route_quality_selected_keyshape_rate route_quality_selected_noisy_rate route_quality_retry_raw_proxy_mean route_quality_retry_keyshape_proxy_mean route_quality_retry_noisy_proxy_mean route_quality_retry_raw_norm_proxy_mean route_quality_retry_keyshape_norm_proxy_mean route_quality_retry_noisy_norm_proxy_mean route_quality_retry_raw_delta_mean route_quality_retry_keyshape_delta_mean route_quality_retry_noisy_delta_mean route_quality_selected_raw_qacc route_quality_selected_keyshape_qacc route_quality_selected_noisy_qacc route_quality_candidate_weight_correct_mean route_quality_candidate_weight_wrong_mean route_quality_candidate_weight_gap route_quality_candidate_best_correct_rate route_hint_candidate_lookup_count route_hint_value_read_distance_mean routing_trigger_rate active_jump_rate", names, " ")
    }
    NR == 1 {
...
emit_aggregate() {
  awk -F, '
    ...
    BEGIN {
      metric_count = split("qacc route_quality_source_normalization_active route_quality_selected_raw_rate route_quality_selected_keyshape_rate route_quality_selected_noisy_rate route_quality_retry_raw_proxy_mean route_quality_retry_keyshape_proxy_mean route_quality_retry_noisy_proxy_mean route_quality_retry_raw_norm_proxy_mean route_quality_retry_keyshape_norm_proxy_mean route_quality_retry_noisy_norm_proxy_mean route_quality_selected_raw_qacc route_quality_selected_keyshape_qacc route_quality_selected_noisy_qacc route_quality_candidate_weight_correct_mean route_quality_candidate_weight_wrong_mean route_quality_candidate_weight_gap route_quality_candidate_best_correct_rate route_quality_source_ranking_delta_mean lookup_count read_distance routing_trigger_rate active_jump_rate", metrics, " ")
    }
    NR == 1 {
      for (i = 1; i <= NF; i++) idx[$i] = i
      next
...
run_case() {
  ...
  echo "quality-source-norm: ${scenario} apply=${quality_apply} beta=${quality_beta} channel=${channel_weight} norm=${normalization}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
    ...
    --route-quality-source-normalization "$normalization" \
    --route-quality-source-norm-eps 1e-6 \
...
run_arm_set() {
  ...
  run_case proxy-off "$key_count" "$seed" "$noisy_rate" none 0.0 -0.1 none
  run_case channel-sign-none "$key_count" "$seed" "$noisy_rate" source-ranking 0.10 -0.1 none
  run_case channel-sign-center "$key_count" "$seed" "$noisy_rate" source-ranking 0.10 -0.1 center
  run_case channel-sign-zscore "$key_count" "$seed" "$noisy_rate" source-ranking 0.10 -0.1 zscore
}
