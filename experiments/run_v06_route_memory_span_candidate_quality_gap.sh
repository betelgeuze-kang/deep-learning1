...
PREFIX="v06_route_memory_span_candidate_quality_gap"
KEY_COUNT=32
VALUE_LEN=5
EPOCHS=6
CYCLES_PER_EPOCH=20
...
if [[ "$MODE" == "smoke" ]]; then
  PREFIX="v06_route_memory_span_candidate_quality_gap_smoke"
elif [[ "$MODE" == "full" ]]; then
  KEY_COUNT=64
  VALUE_LEN=8
  EPOCHS=8
...
run_case() {
  ...
  echo "v06-span-candidate-quality-gap: ${label}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$FIXTURE" \
    --N "$N_BYTES" \
    --epochs "$EPOCHS" \
...
FIXTURE="$TMP_DIR/span_candidate_quality_gap.txt"
...
run_case weak-hybrid-safe 0.25 0.75 16 insertion hybrid-safe
run_case weak-keyshape 0.25 0.75 16 key-shape none

awk -F, '
  NR == 1 {
    for (i = 1; i <= NF; i++) idx[$i] = i
    print "rows,clean_qacc,weak_qacc,base_qacc,hybrid_qacc,keyshape_qacc,weak_span_exact,base_span_exact,hybrid_span_exact,keyshape_span_exact,weak_all_recall,weak_all_top1,keyshape_all_top1,weak_correct_key_share,keyshape_correct_key_share,weak_key_entropy,keyshape_key_entropy,weak_top_key_consistency,keyshape_top_key_consistency,weak_coherent_wrong_top_key,keyshape_span_exact_delta,routing_trigger_rate_mean,active_jump_rate_mean"
    next
  }
  {
    rows++
  ...
  END {
    ...
    printf "%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
      rows,
      qacc["clean-route-code-span"],
      qacc["weak-k16"],
      qacc["weak-base-default"],
      qacc["weak-hybrid-safe"],
      qacc["weak-keyshape"],
      span_exact["weak-k16"],
      span_exact["weak-base-default"],
      span_exact["weak-hybrid-safe"],
      ...
      top_consistency["weak-k16"],
      top_consistency["weak-keyshape"],
