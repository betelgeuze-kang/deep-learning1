...
FIXTURE="$TMP_DIR/v06_span_boundary_fixture.txt"
CSV="$RESULTS_DIR/v06_route_memory_span_boundary.csv"
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing h6 span-boundary column: " required[i], 2)
      }
    }
    next
  }
  { last = $0 }
  END {
    if (last == "") die("no h6 span-boundary data row", 3)
    split(last, row, FS)
    if (metric("kv_record_count") != 2) {
      die("expected two multi-byte KV records", 4)
    }
    if (metric("kv_query_count") != 2 || metric("route_hint_query_count") != 2) {
      die("current h5 route-memory should expose one first-byte query per key", 5)
    }
    if (metric("kv_query_hit_rate") < 0.999999 || metric("route_hint_applied_rate") < 0.999999) {
      die("expected exact first-byte KV route hints to hit and apply", 6)
    }
    if (metric("route_hint_value_read_distance_mean") <= 0) {
      die("value-bearing route read distance should be populated", 7)
    }
    if (metric("routing_trigger_rate") != 0 || metric("active_jump_rate") != 0) {
      die("h6 span boundary must not activate jump-neighbor routing", 8)
    }
  }
' "$CSV"

echo "v06 route-memory span boundary smoke passed"
