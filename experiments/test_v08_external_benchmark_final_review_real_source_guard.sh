...
awk -F, '
  ...
  NR == 1 {
    for (i = 1; i <= NF; i++) idx[$i] = i
    required_count = split("final_review_source review_rows review_hash_verified_rows local_final_review_artifact_rows local_reviewer_identity_rows local_reviewer_conflict_rows real_source_declared_rows non_fixture_declared_rows final_review_verified real_external_benchmark_verified action routing_trigger_rate active_jump_rate", required, " ")
    for (i = 1; i <= required_count; i++) {
...
awk -F, '
  ...
  END {
    if (rows != 11) die("expected v08 final review real-source guard decision rows", 23)
  }
' "$DECISION_CSV"
