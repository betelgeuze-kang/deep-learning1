...
CYCLES_PER_EPOCH=12
PROPOSAL_COUNT=24
PREFIX="v05_route_source_credit_retry_source"
KEY_COUNTS=(128)
SEEDS=(1 2)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_retry_source_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=20
...
printf 'scenario,arm,fallback_source,retry_source,source_filter_mode,key_count,seed,qacc,fallback_recall,fallback_qacc,source_gap,noisy_mean,noisy_slashed,noisy_selected,selected_fallback,strength_mean,source_filter_filtered,source_filter_abstain,source_retry_used,source_retry_success,lookup_count,read_distance,routing_trigger_rate,active_jump_rate\n' >"$SUMMARY_CSV"
...
run_case() {
  ...
  echo "retry-source: ${scenario} source=${fallback_source} retry=${retry_source} filter=${filter_mode} threshold=${filter_threshold}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
...
run_arm_set() {
  ...
  run_case "noisy-filter" "$key_count" "$seed" noisy-route-code off negative-credit 0.0
  run_case "retry-raw" "$key_count" "$seed" noisy-route-code raw-key negative-credit 0.0
  run_case "retry-keyshape" "$key_count" "$seed" noisy-route-code key-shape negative-credit 0.0
  run_case "retry-unfiltered" "$key_count" "$seed" noisy-route-code raw-key off 0.0
}
