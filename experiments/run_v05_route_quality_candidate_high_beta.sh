...
CYCLES_PER_EPOCH=10
PROPOSAL_COUNT=22
PREFIX="v05_route_quality_candidate_high_beta"
KEY_COUNTS=(128 256)
SEEDS=(1 2 3)
NOISY_RATES=(0.25 0.50)
ARMS=(
  "candidate-b3p00-cap3p0:3.00:3.0"
  "candidate-b4p00-cap4p0:4.00:4.0"
  "candidate-b4p00-cap6p0:4.00:6.0"
  "candidate-b5p00-cap4p0:5.00:4.0"
  "candidate-b5p00-cap6p0:5.00:6.0"
  "candidate-b5p00-cap8p0:5.00:8.0"
)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=6
  PROPOSAL_COUNT=18
  PREFIX="v05_route_quality_candidate_high_beta_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
  NOISY_RATES=(0.25)
  ARMS=(
...
run_case() {
  ...
  echo "quality-candidate-high-beta: ${scenario} beta=${candidate_beta} cap=${candidate_max}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
