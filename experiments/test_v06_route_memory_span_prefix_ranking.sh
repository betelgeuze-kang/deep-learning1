...
"$ROOT_DIR/experiments/run_v06_route_memory_span_prefix_ranking.sh" --smoke

SUMMARY_CSV="$RESULTS_DIR/v06_route_memory_span_prefix_ranking_smoke_summary.csv"
AGG_CSV="$RESULTS_DIR/v06_route_memory_span_prefix_ranking_smoke_aggregate.csv"
...
awk -F, '
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        printf "missing span candidate-quality summary column: %s\n", required[i] > "/dev/stderr"
        exit 2
      }
    }
    next
  }
  {
    ...
    if (scenario == "weak-span-prefix") {
      saw_span_prefix = 1
      span_prefix_recall = $idx["span_all_recall"] + 0
    }
  ...
  END {
    if (rows < 5) {
      printf "expected at least 5 span prefix-ranking rows, got %d\n", rows > "/dev/stderr"
      exit 3
    }
    if (saw_span_prefix != 1) {
      printf "missing weak-span-prefix row\n" > "/dev/stderr"
      exit 4
    }
    if (weak_recall < 0.99) {
    ...
    if (span_prefix_recall < 0.99) {
      printf "expected span-prefix all-span recall to stay restored, got %.6f\n", span_prefix_recall > "/dev/stderr"
      exit 6
    }
...
awk -F, '
  NR == 1 {
    for (i = 1; i <= NF; i++) idx[$i] = i
    required_count = split("rows weak_all_recall weak_all_top1 span_prefix_all_top1 keyshape_all_top1 span_prefix_qacc_delta span_prefix_span_exact_delta keyshape_span_exact_delta routing_trigger_rate_mean active_jump_rate_mean", required, " ")
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        printf "missing span prefix-ranking aggregate column: %s\n", required[i] > "/dev/stderr"
        exit 10
      }
    }
    next
...
echo "v06 route-memory span prefix-ranking smoke passed"
