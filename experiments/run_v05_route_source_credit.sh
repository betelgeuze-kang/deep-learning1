...
CYCLES_PER_EPOCH=20
PROPOSAL_COUNT=30
PREFIX="v05_route_source_credit"
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_smoke"
elif [[ "$MODE" == "full" ]]; then
  KEY_COUNT=128
  EPOCHS=40
  CYCLES_PER_EPOCH=20
...
printf 'scenario,route_source_credit_learning,route_source_credit_score_weight,route_source_credit_eta_reward,route_source_credit_eta_slash,route_source_credit_decay,route_source_credit_clip,route_corrupt_preserve_correct,route_corrupt_candidate_rate,fixture_query_byte_acc,route_source_credit_size,route_source_credit_primary_mean,route_source_credit_fallback_mean,route_source_credit_gap,route_source_credit_primary_slashed_rate,route_source_credit_fallback_rewarded_rate,route_hint_candidate_lookup_count,route_hint_value_read_distance_mean,routing_trigger_rate,active_jump_rate,route_primary_recall,route_fallback_used_rate,route_fallback_recall,route_fallback_qacc,route_fallback_success_rate,route_fallback_hi_acc,route_fallback_lo_acc\n' >"$SUMMARY_CSV"
...
compute_metrics() {
  ...
  awk -F, '
    BEGIN {
      split("fixture_query_byte_acc route_source_credit_size route_source_credit_primary_mean route_source_credit_fallback_mean route_source_credit_gap route_source_credit_primary_slashed_rate route_source_credit_fallback_rewarded_rate route_hint_candidate_lookup_count route_hint_value_read_distance_mean routing_trigger_rate active_jump_rate route_primary_recall route_fallback_used_rate route_fallback_recall route_fallback_qacc route_fallback_success_rate route_fallback_hi_acc route_fallback_lo_acc", names, " ")
    }
    NR == 1 {
...
run_case() {
  ...
  echo "route source credit: ${scenario} learning=${source_learning} preserve=${preserve_correct} corrupt=${corrupt_rate}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
  ...
  printf '%s,%d,1.000000,0.050000,0.100000,0.000000,2.000000,%d,%.6f,%s\n' \
    "$scenario" \
    "$source_learning" \
    "$preserve_correct" \
    "$corrupt_rate" \
...
run_smoke() {
  run_case "source-off-remove" 0 0 0.25
  run_case "source-on-remove" 1 0 0.25
  run_case "source-on-preserve" 1 1 0.25
}

run_full() {
  local preserve_correct
  local corrupt_rate
  local source_learning

  for preserve_correct in 1 0; do
    for corrupt_rate in 0.10 0.25 0.50; do
      for source_learning in 0 1; do
        run_case "src${source_learning}-p${preserve_correct}-cr${corrupt_rate//./p}" \
          "$source_learning" "$preserve_correct" "$corrupt_rate"
      done
    done
  done
}
