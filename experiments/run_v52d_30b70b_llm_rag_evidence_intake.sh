...
for spec in system_specs:
    schema_rows.extend(
        [
            ...
            {"system_id": spec["system_id"], "artifact": "model_identity.json", "field": "model_repository", "required": 1, "rule": "public model repository identifier; placeholders rejected"},
            {"system_id": spec["system_id"], "artifact": "model_identity.json", "field": "model_revision", "required": 1, "rule": "exact model revision/commit; placeholders rejected"},
            ...
            {"system_id": spec["system_id"], "artifact": "model_identity.json", "field": "quantization", "required": 1, "rule": "exact quantization or none; placeholders rejected"},
            {"system_id": spec["system_id"], "artifact": "model_identity.json", "field": "runtime", "required": 1, "rule": "exact runtime implementation; placeholders rejected"},
            {"system_id": spec["system_id"], "artifact": "model_identity.json", "field": "runtime_version", "required": 1, "rule": "exact runtime version; placeholders rejected"},
            {"system_id": spec["system_id"], "artifact": "model_identity.json", "field": "hardware", "required": 1, "rule": "measured hardware identifier; placeholders rejected"},
            ...
            {"system_id": spec["system_id"], "artifact": "model_identity.json", "field": "non_fixture_declared", "required": 1, "rule": "must be 1 for real D/E baseline intake"},
            ...
            {"system_id": spec["system_id"], "artifact": "llm_rag_answer_rows.csv", "field": "prompt_template_sha256", "required": 1, "rule": "sha256:<64 hex> over the exact prompt template"},
            {"system_id": spec["system_id"], "artifact": "llm_rag_answer_rows.csv", "field": "context_budget", "required": 1, "rule": "positive integer context budget"},
            {"system_id": spec["system_id"], "artifact": "llm_rag_answer_rows.csv", "field": "retrieval_budget", "required": 1, "rule": "positive integer retrieval budget"},
            {"system_id": spec["system_id"], "artifact": "llm_rag_answer_rows.csv", "field": "seed", "required": 1, "rule": "integer generation seed"},
            ...
            {"system_id": spec["system_id"], "artifact": "llm_rag_resource_rows.csv", "field": "peak_memory_mb", "required": 1, "rule": "positive measured peak memory, evaluated separately from answer quality"},
            {"system_id": spec["system_id"], "artifact": "llm_rag_resource_rows.csv", "field": "evaluator_version", "required": 1, "rule": "exact evaluator version; placeholders rejected"},
            {"system_id": spec["system_id"], "artifact": "llm_rag_resource_rows.csv", "field": "evaluator_artifact_sha256", "required": 1, "rule": "sha256:<64 hex> over evaluator artifact"},
        ]
    )
...
for spec in system_specs:
    identity_templates[spec["system_id"]] = {
        ...
        "model_revision": "exact commit or immutable revision",
        "parameter_count_b": None,
        "size_class": spec["size_class"],
        "runner": "llama.cpp|vllm|transformers|tgi|sglang|other",
        "runtime": "llama.cpp|vllm|transformers|tgi|sglang|other",
        ...
        "hardware": "measured hardware identifier",
...
def validate_system(spec):
    ...
    for field in ["model_repository", "model_revision", "quantization", "runtime", "runtime_version", "hardware"]:
        if not is_nonplaceholder_text(identity.get(field, "")):
            errors.append(f"identity-{field.replace('_', '-')}-placeholder-or-missing")
    ...
    for row in answers:
        ...
        if not is_sha256(row.get("prompt_template_sha256", "")):
            row_errors.append("answer-prompt-template-sha256-invalid")
        int_value(row, "context_budget", row_errors, minimum=1)
        int_value(row, "retrieval_budget", row_errors, minimum=1)
        int_value(row, "seed", row_errors, minimum=0)
    ...
    for row in resources:
        ...
        int_value(row, "retrieved_span_rows", row_errors, minimum=1)
        int_value(row, "peak_memory_mb", row_errors, minimum=1)
        if not is_nonplaceholder_text(row.get("evaluator_version", "")):
            row_errors.append("resource-evaluator-version-placeholder-or-missing")
        if not is_sha256(row.get("evaluator_artifact_sha256", "")):
            row_errors.append("resource-evaluator-artifact-sha256-invalid")
        if row.get("external_api_used") != "0":
...
(run_dir / "V52D_30B70B_LLM_RAG_BOUNDARY.md").write_text(
    ...
    "- `model_identity.json` with the expected system ID, model repository, exact revision, quantization, runtime/version, hardware, parameter-count class, open-weight license URI, sha256-bound model artifact identity, and `non_fixture_declared=1`.\n"
    "- `llm_rag_answer_rows.csv` with one answer row for every v50 query ID plus prompt template hash, context budget, retrieval budget, seed, raw answer/citation output hashes, and generation transcript hashes.\n"
    "- `llm_rag_citation_rows.csv` with source-span-bound citations against the v50 source span registry.\n"
    "- `llm_rag_resource_rows.csv` with measured runtime/resource rows, peak memory, evaluator version/artifact hash, and `external_api_used=0`.\n\n"
