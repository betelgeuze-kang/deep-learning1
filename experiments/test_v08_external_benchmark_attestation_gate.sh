...
awk -F, '
  ...
  }
  {
    ...
    if ($idx["gate"] == "local-attestation-artifacts" && $idx["status"] != "blocked") die("local attestation artifacts should block by default", 24)
    if ($idx["gate"] == "nonlocal-attestation-artifacts" && $idx["status"] != "blocked") die("nonlocal attestation artifacts should block by default", 25)
  ...
  END {
    if (rows != 11) die("expected v08 attestation decision rows", 28)
  }
' "$DECISION_CSV"
