...
CYCLES_PER_EPOCH=16
PROPOSAL_COUNT=30
PREFIX="v05_route_source_credit_learned_source_scale"

if [[ "$MODE" == "smoke" ]]; then
  EPOCHS=8
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_learned_source_scale_smoke"
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=28
  CYCLES_PER_EPOCH=20
  PROPOSAL_COUNT=30
...
compute_metrics() {
  ...
  awk -F, '
    BEGIN {
      split("fixture_query_byte_acc key_region_route_decode_acc route_key_unique_count route_signature_collision_rate route_primary_recall route_fallback_used_rate route_fallback_recall route_fallback_qacc route_fallback_success_rate route_source_credit_size route_source_credit_primary_mean route_source_credit_fallback_mean route_source_credit_gap route_source_credit_primary_slashed_rate route_source_credit_fallback_rewarded_rate route_source_credit_selected_fallback_rate route_source_credit_strength_mean route_hint_candidate_lookup_count route_hint_value_read_distance_mean routing_trigger_rate active_jump_rate", names, " ")
    }
    NR == 1 {
...
run_case() {
  ...
  echo "learned source scale: ${scenario} arm=${arm} keys=${key_count} seed=${seed} fallback=${fallback_source} apply=${source_apply_mode} keep=${keep_prob} aux_noise=${aux_noise_rate}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
...
run_arm_set() {
  ...
  run_case "clean-off-k${key_count}-s${seed}" clean-off "$key_count" "$seed" off off 0 1.0 0.0
  run_case "mid-off-k${key_count}-s${seed}" mid-off "$key_count" "$seed" off off 0 0.5 0.25
  run_case "weak-off-k${key_count}-s${seed}" weak-off "$key_count" "$seed" off off 0 0.25 0.75
  run_case "weak-fallback-ledger-k${key_count}-s${seed}" weak-fallback-ledger "$key_count" "$seed" key-shape ranking-strength 1 0.25 0.75
}
