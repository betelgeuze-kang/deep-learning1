...
PREFIX="v52p_30b_open_weight_llm_rag_v53e_1000"
RUN_ID="${V52P_RUN_ID:-measured_001}"
RUN_DIR="$RESULTS_DIR/$PREFIX/$RUN_ID"
SUMMARY_CSV="$RESULTS_DIR/${PREFIX}_summary.csv"
DECISION_CSV="$RESULTS_DIR/${PREFIX}_decision.csv"
MODEL_ID="${V52P_OLLAMA_MODEL:-qwen2.5:32b-instruct}"

if [[ "${V52P_REUSE_EXISTING:-0}" == "1" && -s "$SUMMARY_CSV" && -s "$RUN_DIR/sha256_manifest.csv" ]]; then
  echo "v52p_30b_open_weight_llm_rag_v53e_1000_dir: $RUN_DIR"
...
if [[ "${V52P_REUSE_EXISTING:-1}" != "1" || ! -s "$RESULTS_DIR/v53e_canary_query_scale_1000_summary.csv" ]]; then
...
identity = {
    ...
    "quantization": "ollama-library-qwen2.5-32b-instruct-local-artifact",
    "model_artifact_uri": str(manifest_path),
    "model_artifact_sha256": model_artifact_sha256,
    "open_weight_license_uri": "https://huggingface.co/Qwen/Qwen2.5-32B-Instruct/blob/main/LICENSE",
...
def build_prompt(query, span):
    ...
    return (
        "You are baseline D: a 32B open-weight LLM with one RAG source span. "
...
for idx, query in enumerate(query_rows, start=1):
    ...
    answer_id = f"v52p_D_{query['query_id']}"
    answer_rows.append(
        {
            "answer_id": answer_id,
            "system_id": "D",
...
write_csv(run_dir / "d_answer_rows.csv", list(answer_rows[0].keys()), answer_rows)
write_csv(run_dir / "d_citation_rows.csv", list(citation_rows[0].keys()), citation_rows)
write_csv(run_dir / "d_retrieval_rows.csv", list(retrieval_rows[0].keys()), retrieval_rows)
write_csv(run_dir / "d_abstain_rows.csv", list(abstain_rows[0].keys()), abstain_rows)
write_csv(run_dir / "d_wrong_answer_guard_rows.csv", list(wrong_guard_rows[0].keys()), wrong_guard_rows)
write_csv(run_dir / "d_resource_rows.csv", list(resource_rows[0].keys()), resource_rows)
...
write_csv(run_dir / "d_system_metric_rows.csv", list(metric_rows[0].keys()), metric_rows)
...
decision_rows = [
    ("d-v53e-1000-open-weight-llm-rag", "pass", "D emits 1000 answer/citation/abstain/guard/resource rows over v53e"),
    ("same-frozen-query-set-as-abgh", "pass", "D uses the same full frozen v53e 1000-row query set"),
    ("same-source-manifest-as-abgh", "pass", "D uses the same v53e source manifest"),
    ("ollama-open-weight-generation", "pass", f"{model_id} generated all D response rows locally"),
    ("no-external-network", "pass", "D run uses local Ollama and no external API"),
    ("v52-d-absorb-ready", "pass", "D v53e measured packet can be absorbed into a later v52 registry update"),
    ("70b-real-row", "blocked", "E 70B LLM+RAG row is still missing"),
    ("v52-full-baseline-war", "blocked", "full v52 still needs E and optional F handling"),
    ("real-release-package", "blocked", "v52p D measured packet is not a release package"),
]
write_csv(decision_csv, ["gate", "status", "reason"], [{"gate": gate, "status": status, "reason": reason} for gate, status, reason in decision_rows])

(run_dir / "V52P_30B_OPEN_WEIGHT_LLM_RAG_V53E_BOUNDARY.md").write_text(
    "# v52p 30B Open-Weight LLM + RAG v53e Boundary\n\n"
    "This is a real local Ollama baseline-D measured packet over the full frozen v53e 1000-query canary set. "
    ...
    "- route_memory_store_used=0\n"
    "- compact_routehint_used=0\n\n"
    "Still blocked: E 70B real evidence directory, optional F handling, full v52, v59 full replay, and release claims.\n",
    encoding="utf-8",
)

manifest_out = {
    "manifest_scope": "v52p-30b-open-weight-llm-rag-v53e-1000",
...
(run_dir / "v52p_30b_open_weight_llm_rag_v53e_1000_manifest.json").write_text(json.dumps(manifest_out, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
artifact_rels = [
    ...
    "d_answer_rows.csv",
    "d_citation_rows.csv",
    "d_retrieval_rows.csv",
    "d_abstain_rows.csv",
    "d_wrong_answer_guard_rows.csv",
    "d_resource_rows.csv",
...
print(f"v52p_30b_open_weight_llm_rag_v53e_1000_dir: {run_dir}")
