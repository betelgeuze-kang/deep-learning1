...
if summary.get("v59b_dependency_blocker_ready") == "1":
    expected_blocked = {
        ...
        "release_requirement_ready_rows": "2",
        "release_requirement_blocked_rows": "9",
        "allowed_limited_claim_rows": "1",
        "forbidden_claim_rows": "6",
    ...
    for field, value in expected_blocked.items():
        if summary.get(field) != value:
            raise SystemExit(f"v60b dependency-blocker path {field}: expected {value}, got {summary.get(field)}")

    decisions = {row["gate"]: row["status"] for row in read_csv(decision_csv)}
    for gate in ["v59b-dependency-blocker-input", "candidate-preflight-audit", "candidate-chain-hash-binding"]:
        if decisions.get(gate) != "pass":
            raise SystemExit(f"v60b dependency-blocker path gate should pass: {gate}")
    for gate in [
        ...
        if decisions.get(gate) != "blocked":
            raise SystemExit(f"v60b dependency-blocker path gate should remain blocked: {gate}")
    ...
    for rel in required_files:
        ...
        if not path.is_file() or path.stat().st_size == 0:
            raise SystemExit(f"missing v60b dependency-blocker path artifact: {rel}")

    requirements = read_csv(run_dir / "release_preflight_requirement_rows.csv")
    if len(requirements) != 11:
        raise SystemExit("v60b dependency-blocker path should list eleven requirements")
    ready = {row["requirement"] for row in requirements if row["ready"] == "1"}
    if ready != {"one_command_candidate_entrypoint", "candidate_artifact_hashes"}:
        raise SystemExit(f"v60b dependency-blocker ready requirement mismatch: {ready}")
    claims = {row["claim_id"]: row["status"] for row in read_csv(run_dir / "release_preflight_claim_rows.csv")}
    if claims.get("v59b_dependency_blocker_recorded") != "allowed_limited":
        raise SystemExit("v60b should allow only dependency-blocker wording on blocker path")
    if claims.get("candidate_chain_replay_ready") != "forbidden":
        raise SystemExit("v60b should forbid candidate-chain replay wording on blocker path")

    stage_audit = read_csv(run_dir / "stage_release_audit_rows.csv")
    if len(stage_audit) != 12:
        raise SystemExit("v60b dependency-blocker path should audit 12 target stages")
    if any(row["candidate_ready"] != "0" or row["full_ready"] != "0" or row["release_acceptable"] != "0" for row in stage_audit):
        raise SystemExit("v60b dependency-blocker path should keep all target stages unready")

    manifest = json.loads((run_dir / "v60b_release_preflight_candidate_audit_manifest.json").read_text(encoding="utf-8"))
    if manifest.get("v59b_dependency_blocker_ready") != 1 or manifest.get("release_requirement_blocked_rows") != 9:
        raise SystemExit("v60b dependency-blocker manifest mismatch")
    sha_rows = {row["path"]: row["sha256"] for row in read_csv(run_dir / "sha256_manifest.csv")}
    for rel in required_files:
        if rel == "sha256_manifest.csv":
            continue
        if sha_rows.get(rel) != sha256(run_dir / rel):
            raise SystemExit(f"v60b dependency-blocker sha256 mismatch: {rel}")
    boundary = (run_dir / "V60B_RELEASE_PREFLIGHT_CANDIDATE_AUDIT_BOUNDARY.md").read_text(encoding="utf-8")
    for snippet in [
        "candidate replay or dependency blocker",
        "v59b_dependency_blocker_ready=1",
        "replayable missing-dependency blocker",
        "Do not publish v1.0 release readiness",
    ]:
        if snippet not in boundary:
            raise SystemExit(f"v60b dependency-blocker boundary missing {snippet}")
    sys.exit(0)
