...
CYCLES_PER_EPOCH=16
PROPOSAL_COUNT=30
PREFIX="v05_route_source_credit_source_aware_aggregation"

if [[ "$MODE" == "smoke" ]]; then
  EPOCHS=8
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_source_aware_aggregation_smoke"
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=28
  CYCLES_PER_EPOCH=20
  PROPOSAL_COUNT=30
...
compute_metrics() {
  ...
  awk -F, '
    BEGIN {
      split("fixture_query_byte_acc key_region_route_decode_acc route_primary_recall route_fallback_used_rate route_fallback_recall route_fallback_qacc route_fallback_success_rate route_candidate_top1_rate route_candidate_rank_mean route_hint_correct_value_vote_share_mean route_hint_vote_entropy_mean route_hint_vote_margin_mean route_source_credit_gap route_source_credit_noisy_mean route_source_credit_noisy_slashed_rate route_noisy_source_selected_rate route_source_credit_selected_fallback_rate route_source_credit_strength_mean route_agg_policy_vote_rate route_agg_policy_weighted_rate route_hint_candidate_lookup_count route_hint_value_read_distance_mean routing_trigger_rate active_jump_rate", names, " ")
    }
    NR == 1 {
...
run_case() {
  ...
  local lowconf_agg="$6"
  local highconf_agg="$7"
  local source_apply_mode="$8"
  ...
  echo "source-aware aggregation: ${scenario} source=${fallback_source} agg=${route_hint_agg} low=${lowconf_agg} high=${highconf_agg} apply=${source_apply_mode}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
    ...
    --route-lowconf-agg "$lowconf_agg" \
    --route-highconf-agg "$highconf_agg" \
...
run_arm_set() {
  ...
  run_case "raw-vote" "$key_count" "$seed" raw-key vote vote weighted-vote off
  run_case "raw-source-aware" "$key_count" "$seed" raw-key weighted-vote weighted-vote weighted-vote ranking-strength
  run_case "raw-gated-safe" "$key_count" "$seed" raw-key confidence-gated weighted-vote weighted-vote off

  run_case "keyshape-vote" "$key_count" "$seed" key-shape vote vote weighted-vote off
  run_case "keyshape-source-aware" "$key_count" "$seed" key-shape weighted-vote weighted-vote weighted-vote ranking-strength
  run_case "keyshape-gated-safe" "$key_count" "$seed" key-shape confidence-gated weighted-vote weighted-vote off

  run_case "noisy-vote" "$key_count" "$seed" noisy-route-code vote vote weighted-vote off
  run_case "noisy-source-aware" "$key_count" "$seed" noisy-route-code weighted-vote weighted-vote weighted-vote ranking-strength
  run_case "noisy-gated-safe" "$key_count" "$seed" noisy-route-code confidence-gated weighted-vote weighted-vote ranking-strength
}
