...
awk -F, '
  ...
  }
  {
    ...
    if ($idx["gate"] == "source-import" && $idx["status"] != "blocked") die("source import should block before real external benchmark", 25)
    if ($idx["gate"] == "real-external-benchmark" && $idx["status"] != "blocked") die("real external benchmark should remain blocked", 25)
  }
  END {
    if (rows != 14) die("expected v08 final review remote-review decision rows", 26)
  }
' "$DECISION_CSV"
