...
if [[ "$MODE" == "smoke" ]]; then
  PREFIX="v03_route_hint_kv_hash_route_code_fallback_persistence_smoke"
  EPOCHS=10
elif [[ "$MODE" == "full" ]]; then
  PREFIX="v03_route_hint_kv_hash_route_code_fallback_persistence"
  EPOCHS=16
else
  PREFIX="v03_route_hint_kv_hash_route_code_fallback_persistence"
  EPOCHS=12
...
run_dmv02() {
  ...
  echo "route-code fallback persistence: lo=${lo_mult} ttl=${ttl} (pc=${preserve_correct}, cr=${corrupt_rate})" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
    ...
    --route-fallback-persist-cycles "$ttl" \
...
get_clean_reference() {
  ...
  if [[ ! -f "$cache_file" ]]; then
    ...
    run_dmv02 "$fixture" "$csv_path" 7.5 0 1 0.0
...
append_summary() {
  ...
  awk -F, \
    ...
    END {
      ...
      printf "%s,%.6f,%.6f,%d,%.6f,%.6f,%.6f", scenario, hi_mult, lo_mult, ttl, qacc, clean_reference, clean_reference - qacc
...
run_case() {
  ...
  run_dmv02 "$fixture" "$csv_path" "$lo_mult" "$ttl" 0 "$CORRUPT_RATE"
  append_summary "$scenario" "$lo_mult" "$ttl" "$clean_reference" "$csv_path"
}

run_case "lo7p5_ttl0" 7.5 0
run_case "lo7p5_ttl3" 7.5 3
run_case "lo10_ttl0" 10.0 0
run_case "lo10_ttl3" 10.0 3
if [[ "$MODE" != "smoke" ]]; then
  run_case "lo7p5_ttl1" 7.5 1
  run_case "lo10_ttl1" 10.0 1
fi
if [[ "$MODE" == "full" ]]; then
  run_case "lo7p5_ttl5" 7.5 5
  run_case "lo10_ttl5" 10.0 5
fi
