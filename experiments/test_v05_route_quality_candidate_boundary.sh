...
SUMMARY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_boundary_smoke_summary.csv"
AGG_CSV="$ROOT_DIR/results/v05_route_quality_candidate_boundary_smoke_aggregate.csv"
BY_KEY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_boundary_smoke_by_key_noise.csv"

"$ROOT_DIR/experiments/run_v05_route_quality_candidate_boundary.sh" --smoke
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing candidate-boundary column: " required[i], 2)
      }
    }
    next
  }
  {
    ...
    if (metric("route_quality_apply_active") != 1 ||
        metric("route_quality_candidate_weight_beta") < 2.0) {
      die("candidate-boundary should keep high-beta candidate-weight active: " arm, 4)
    }
    if (qacc < 0 || qacc > 1 || factor < 0.5 || factor > cap ||
        factor_p90 < 0.5 || factor_p90 > cap ||
        factor_max < 0.5 || factor_max > cap) {
      die("candidate-boundary factor/qacc out of range: " arm, 5)
    }
    if (entropy < 0 || top_share < 0 || top_share > 1) {
      die("candidate-boundary concentration metric out of range: " arm, 6)
    }
    ...
    if (metric("routing_trigger_rate") != 0 || metric("active_jump_rate") != 0) {
      die("candidate-boundary must not activate jump-neighbor routing: " arm, 8)
    }
    if (metric("route_quality_selected_noisy_rate") > 0.25) {
      die("candidate-boundary selected noisy source too often: " arm, 9)
    }
    if (metric("route_quality_candidate_weight_factor_gap") <= 0) {
      die("candidate-boundary should preserve positive factor gap: " arm, 10)
    }
    if (metric("route_wrong_hint_strength_mean") < 0 ||
        metric("route_correct_hint_strength_mean") < 0) {
      die("candidate-boundary strength metric should be non-negative: " arm, 11)
    }
  ...
  END {
    ...
    required_arms_count = split("candidate-b2p00-cap3p0 candidate-b2p50-cap3p0 candidate-b3p00-cap4p0", required_arms, " ")
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing aggregate candidate-boundary column: " required[i], 20)
      }
    }
    next
  }
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing by-key candidate-boundary column: " required[i], 30)
      }
    }
    next
  }
...
echo "route quality candidate-boundary smoke passed"
