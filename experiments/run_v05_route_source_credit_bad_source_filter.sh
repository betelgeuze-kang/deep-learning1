...
CYCLES_PER_EPOCH=12
PROPOSAL_COUNT=24
PREFIX="v05_route_source_credit_bad_source_filter"
KEY_COUNTS=(128)
SEEDS=(1 2)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_bad_source_filter_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=20
...
printf 'scenario,arm,fallback_source,source_filter_mode,key_count,seed,qacc,fallback_recall,fallback_qacc,source_gap,noisy_mean,noisy_slashed,noisy_selected,selected_fallback,strength_mean,source_filter_filtered,source_filter_abstain,lookup_count,read_distance,routing_trigger_rate,active_jump_rate\n' >"$SUMMARY_CSV"
...
run_case() {
  ...
  local filter_mode="$5"
  local filter_threshold="$6"
  local source_credit_learning=1
  ...
  echo "bad-source filter: ${scenario} source=${fallback_source} filter=${filter_mode} threshold=${filter_threshold}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
...
run_arm_set() {
  ...
  run_case "off-control" "$key_count" "$seed" off off 0.0
  run_case "raw-filter" "$key_count" "$seed" raw-key negative-credit 0.0
  run_case "keyshape-filter" "$key_count" "$seed" key-shape negative-credit 0.0
  run_case "noisy-unfiltered" "$key_count" "$seed" noisy-route-code off 0.0
  run_case "noisy-filter" "$key_count" "$seed" noisy-route-code negative-credit 0.0
  run_case "noisy-abstain" "$key_count" "$seed" noisy-route-code negative-credit 0.05
}
