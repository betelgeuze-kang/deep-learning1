...
CYCLES_PER_EPOCH=12
PROPOSAL_COUNT=24
PREFIX="v05_route_source_credit_source_aware_scale"
KEY_COUNTS=(64 128)
SEEDS=(1 2 3)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_source_aware_scale_smoke"
  KEY_COUNTS=(64 128)
  SEEDS=(1 2)
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=20
...
run_case() {
  ...
  echo "source-aware scale: ${scenario} source=${fallback_source} agg=${route_hint_agg} apply=${source_apply_mode}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
...
run_arm_set() {
  ...
  run_case "raw-vote" "$key_count" "$seed" raw-key vote off
  run_case "raw-source-aware" "$key_count" "$seed" raw-key weighted-vote ranking-strength

  run_case "keyshape-vote" "$key_count" "$seed" key-shape vote off
  run_case "keyshape-source-aware" "$key_count" "$seed" key-shape weighted-vote ranking-strength

  run_case "noisy-vote" "$key_count" "$seed" noisy-route-code vote off
  run_case "noisy-source-aware" "$key_count" "$seed" noisy-route-code weighted-vote ranking-strength
}
