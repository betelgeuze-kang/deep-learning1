...
SUMMARY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_auto_basis_smoke_summary.csv"
AGG_CSV="$ROOT_DIR/results/v05_route_quality_candidate_auto_basis_smoke_aggregate.csv"
BY_KEY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_auto_basis_smoke_by_key_noise.csv"

"$ROOT_DIR/experiments/run_v05_route_quality_candidate_hybrid_guardrail.sh" --auto-smoke
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing candidate-auto-basis column: " required[i], 2)
      }
    }
    next
  }
  {
    ...
    if (metric("key_count") != 128 || metric("seed") != 1) {
      die("auto smoke should run key_count=128 seed=1", 3)
    }
    ...
    if (metric("route_quality_apply_active") != 1 ||
      ...
      die("auto smoke should keep candidate-weight beta=8 active: " arm, 6)
    }
    if (metric("route_quality_candidate_weight_factor_max") > 8.0) {
      die("auto smoke factor max exceeded clamp: " arm, 7)
    }
    ...
    if (metric("routing_trigger_rate") != 0 || metric("active_jump_rate") != 0) {
      die("auto basis must not activate jump-neighbor routing: " arm, 10)
    }
    if (metric("route_quality_selected_noisy_rate") > 0.25) {
      die("auto basis selected noisy source too often: " arm, 11)
    }
    rows++
    seen[arm] = 1
  }
  END {
    ...
    if (!seen["base-default"] || !seen["hybrid-m0p25"] ||
        !seen["auto-f6p0-t0p72"]) {
      die("auto smoke missing required arm", 13)
    }
  }
' "$SUMMARY_CSV"
...
awk -F, '
  ...
  NR == 1 {
    required_count = split("arm candidate_basis basis_mix rows qacc_mean qacc_std route_quality_candidate_weight_factor_gap_mean route_quality_candidate_weight_factor_max_mean route_quality_candidate_weight_auto_hybrid_rate_mean route_quality_selected_noisy_rate_mean lookup_count_mean read_distance_mean routing_trigger_rate_mean active_jump_rate_mean", required, " ")
    for (i = 1; i <= NF; i++) idx[$i] = i
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing aggregate candidate-auto-basis column: " required[i], 20)
      }
    }
    next
  }
  ...
  END {
    if (rows != 3) {
      die("expected 3 aggregate rows, found " rows, 22)
    }
  }
' "$AGG_CSV"
...
awk -F, '
  ...
  NR == 1 {
    required_count = split("arm candidate_basis basis_mix key_count noisy_source_rate rows qacc_mean qacc_std factor_gap_mean factor_max_mean top_share_mean entropy_mean auto_hybrid_rate_mean wrong_strength_mean selected_noisy_rate_mean active_jump_rate_mean", required, " ")
    for (i = 1; i <= NF; i++) idx[$i] = i
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing by-key candidate-auto-basis column: " required[i], 30)
      }
    }
    next
  }
...
echo "route quality candidate-auto-basis smoke passed"
