...
expected = {
    ...
    "unblocker_matrix_rows": "9",
    "ready_unblocker_operator_surfaces": "9",
    ...
    "runtime_admission_operator_bundle_file_rows": "5",
    "runtime_admission_operator_command_rows_ready": "3",
...
required_files = [
    ...
    "source_v61cv/complete_source_runtime_admission_operator_bundle_file_rows.csv",
    ...
    "source_v53v/complete_source_review_return_acceptance_requirement_rows.csv",
...
if len(matrix) != 9 or len(execution) != 9 or len(files) != 3:
...
if sum(1 for row in matrix if row["current_ready"] == "1") != 3:
    raise SystemExit("v61ck should mark the first three unblockers ready")
if sum(1 for row in matrix if row["current_ready"] == "0") != 6:
    raise SystemExit("v61ck should leave six unblockers blocked")
...
for snippet in [
    "unblocker_matrix_rows=9",
    "ready_unblocker_operator_surfaces=9/9",
    "remaining_materialization_queue_rows=0",
    "remaining_unverified_bytes=0",
    "checkpoint_materialization_promotion_rows=59",
    "ready_checkpoint_materialization_shard_rows=59",
