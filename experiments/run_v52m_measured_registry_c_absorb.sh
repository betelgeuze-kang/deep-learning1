...
PREFIX="v52m_measured_registry_c_absorb"
RUN_ID="${V52M_RUN_ID:-registry_001}"
...
if [[ "${V52M_REUSE_EXISTING:-1}" != "1" || ! -s "$RESULTS_DIR/v52i_abgh_same_query_measured_1000_summary.csv" ]]; then
  "$ROOT_DIR/experiments/run_v52i_abgh_same_query_measured_1000.sh" >/dev/null
fi
if [[ "${V52M_REUSE_EXISTING:-1}" != "1" || ! -s "$RESULTS_DIR/v52l_7b14b_local_model_rag_v53e_1000_summary.csv" ]]; then
  V52L_REUSE_EXISTING=1 "$ROOT_DIR/experiments/run_v52l_7b14b_local_model_rag_v53e_1000.sh" >/dev/null
fi
if [[ "${V52M_REUSE_EXISTING:-1}" != "1" || ! -s "$RESULTS_DIR/v52c_7b14b_local_model_rag_evidence_intake_summary.csv" ]]; then
  "$ROOT_DIR/experiments/run_v52c_7b14b_local_model_rag_evidence_intake.sh" >/dev/null
fi
if [[ "${V52M_REUSE_EXISTING:-1}" != "1" || ! -s "$RESULTS_DIR/v52d_30b70b_llm_rag_evidence_intake_summary.csv" ]]; then
  "$ROOT_DIR/experiments/run_v52d_30b70b_llm_rag_evidence_intake.sh" >/dev/null
fi
if [[ "${V52M_REUSE_EXISTING:-1}" != "1" || ! -s "$RESULTS_DIR/v52e_100b_plus_hosted_llm_rag_optional_intake_summary.csv" ]]; then
...
if int(v52l_summary.get("c_v53e_absorb_ready", "0")) != 1:
    raise SystemExit("v52m requires v52l with c_v53e_absorb_ready=1")
if int(v52l_summary.get("same_query_set_as_v52i_abgh", "0")) != 1:
    raise SystemExit("v52m requires v52l bound to the same query set as v52i")
...
for system_id in ["A", "B", "C", "D", "E", "F", "G", "H"]:
    ...
    blocking_reason = ""
    if system_id == "C":
        adapter_status = "measured-local-v52l"
    elif measured:
        adapter_status = "measured-local-v52i"
    elif optional:
    ...
    baseline_rows.append(
        {
            ...
            "external_model_used": int(system_id == "C" and measured),
...
decision_rows = [
    ("v52m-measured-registry-c-absorb", "pass", "A/B/C/G/H measured rows are absorbed into the v52 measured registry"),
    ("same-query-source-local-systems", "pass", "A/B/C/G/H share v53e query IDs and source manifest"),
    ("local-answer-citation-resource-rows", "pass", "A/B/C/G/H have answer/citation/abstain/guard/resource rows"),
    ...
    ("v52-full-baseline-war", "blocked", "full v52 requires D/E and optional F handling"),
    ("real-release-package", "blocked", "v52m measured registry is not a release package"),
]
write_csv(decision_csv, ["gate", "status", "reason"], [{"gate": gate, "status": status, "reason": reason} for gate, status, reason in decision_rows])

(run_dir / "V52M_MEASURED_REGISTRY_C_ABSORB_BOUNDARY.md").write_text(
    "# v52m Measured Registry C Absorb Boundary\n\n"
    "This absorbs the v52i A/B/G/H local measured packet plus the v52l C measured packet into a v52 measured registry. "
    ...
    "Do not publish 30B-150B comparison claims or C performance wins from this local measured registry.\n",
    encoding="utf-8",
)

manifest = {
    "manifest_scope": "v52m-measured-registry-c-absorb",
...
(run_dir / "v52m_measured_registry_c_absorb_manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
print(f"v52m_measured_registry_c_absorb_dir: {run_dir}")
