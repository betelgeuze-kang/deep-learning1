...
PREFIX="v06_route_memory_span_prefix_ranking"
KEY_COUNT=32
VALUE_LEN=5
EPOCHS=6
CYCLES_PER_EPOCH=20
...
if [[ "$MODE" == "smoke" ]]; then
  PREFIX="v06_route_memory_span_prefix_ranking_smoke"
elif [[ "$MODE" == "full" ]]; then
  KEY_COUNT=64
  VALUE_LEN=8
  EPOCHS=8
    ...
    NR == 1 {
      ...
      for (i = 1; i <= required_count; i++) {
        if (!(required[i] in idx)) {
          printf "missing span candidate-quality metric column: %s\n", required[i] > "/dev/stderr"
          exit 2
        }
      }
      next
...
run_case() {
  ...
  echo "v06-span-prefix-ranking: ${label}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$FIXTURE" \
    --N "$N_BYTES" \
    --epochs "$EPOCHS" \
...
FIXTURE="$TMP_DIR/span_prefix_ranking.txt"
...
awk -F, '
  NR == 1 {
    for (i = 1; i <= NF; i++) idx[$i] = i
    print "rows,clean_qacc,weak_qacc,span_prefix_qacc,base_qacc,keyshape_qacc,weak_span_exact,span_prefix_span_exact,base_span_exact,keyshape_span_exact,weak_all_recall,weak_all_top1,span_prefix_all_top1,keyshape_all_top1,weak_correct_key_share,span_prefix_correct_key_share,keyshape_correct_key_share,weak_key_entropy,span_prefix_key_entropy,keyshape_key_entropy,weak_coherent_wrong_top_key,span_prefix_coherent_wrong_top_key,span_prefix_qacc_delta,span_prefix_span_exact_delta,keyshape_span_exact_delta,routing_trigger_rate_mean,active_jump_rate_mean"
    next
  }
  {
    rows++
    ...
    top_consistency[scenario] = $idx["top_key_consistency"] + 0
  ...
  END {
    if (rows < 5) {
      printf "invalid span candidate-quality rows\n" > "/dev/stderr"
      exit 4
    }
    printf "%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
      rows,
      ...
      qacc["weak-span-prefix"] - qacc["weak-k16"],
      span_exact["weak-span-prefix"] - span_exact["weak-k16"],
...
echo "wrote $SUMMARY_CSV" >&2
echo "wrote $AGG_CSV" >&2
