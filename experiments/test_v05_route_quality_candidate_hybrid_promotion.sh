...
SUMMARY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_hybrid_promotion_smoke_summary.csv"
AGG_CSV="$ROOT_DIR/results/v05_route_quality_candidate_hybrid_promotion_smoke_aggregate.csv"
BY_KEY_CSV="$ROOT_DIR/results/v05_route_quality_candidate_hybrid_promotion_smoke_by_key_noise.csv"

"$ROOT_DIR/experiments/run_v05_route_quality_candidate_hybrid_guardrail.sh" --promotion-smoke
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing candidate-hybrid-promotion column: " required[i], 2)
      }
    }
    next
  }
  {
    ...
    if (metric("key_count") != 128 || metric("seed") != 1) {
      die("promotion smoke should run key_count=128 seed=1", 3)
    }
    ...
    if (metric("route_quality_apply_active") != 1 ||
      ...
      die("promotion smoke should keep candidate-weight beta=8 active: " arm, 6)
    }
    if (metric("route_quality_candidate_weight_factor_max") > 8.0) {
      die("promotion smoke factor max exceeded clamp: " arm, 7)
    }
    if (metric("lookup_count") <= 0 || metric("read_distance") <= 0) {
      die("value-bearing route path should remain populated: " arm, 8)
    }
    if (metric("routing_trigger_rate") != 0 || metric("active_jump_rate") != 0) {
      die("promotion smoke must not activate jump-neighbor routing: " arm, 9)
    }
    if (metric("route_quality_selected_noisy_rate") > 0.25) {
      die("promotion smoke selected noisy source too often: " arm, 10)
    }
    rows++
    seen[arm] = 1
  }
  END {
    ...
    if (!seen["base-default"] || !seen["hybrid-m0p25"]) {
      die("promotion smoke missing base or hybrid arm", 12)
    }
  }
' "$SUMMARY_CSV"
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing aggregate candidate-hybrid-promotion column: " required[i], 20)
      }
    }
    next
  }
  ...
  END {
    if (rows != 2) {
      die("expected 2 aggregate rows, found " rows, 22)
    }
  }
' "$AGG_CSV"
...
awk -F, '
  ...
  NR == 1 {
    ...
    for (i = 1; i <= required_count; i++) {
      if (!(required[i] in idx)) {
        die("missing by-key candidate-hybrid-promotion column: " required[i], 30)
      }
    }
    next
  }
...
echo "route quality candidate-hybrid-promotion smoke passed"
