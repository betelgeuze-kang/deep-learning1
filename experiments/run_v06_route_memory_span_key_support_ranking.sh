...
PREFIX="v06_route_memory_span_key_support_ranking"
KEY_COUNT=32
VALUE_LEN=5
EPOCHS=6
CYCLES_PER_EPOCH=20
...
if [[ "$MODE" == "smoke" ]]; then
  PREFIX="v06_route_memory_span_key_support_ranking_smoke"
elif [[ "$MODE" == "full" ]]; then
  KEY_COUNT=64
  VALUE_LEN=8
  EPOCHS=8
    ...
    NR == 1 {
      ...
      for (i = 1; i <= required_count; i++) {
        if (!(required[i] in idx)) {
          printf "missing span key-support metric column: %s\n", required[i] > "/dev/stderr"
          exit 2
        }
      }
      next
...
run_case() {
  ...
  echo "v06-span-key-support-ranking: ${label}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$FIXTURE" \
    --N "$N_BYTES" \
    --epochs "$EPOCHS" \
...
FIXTURE="$TMP_DIR/span_key_support_ranking.txt"
...
run_case weak-span-key-support 0.25 0.75 16 span-key-support none
...
awk -F, '
  NR == 1 {
    for (i = 1; i <= NF; i++) idx[$i] = i
    print "rows,clean_qacc,weak_qacc,span_prefix_qacc,span_key_support_qacc,base_qacc,keyshape_qacc,weak_span_exact,span_prefix_span_exact,span_key_support_span_exact,base_span_exact,keyshape_span_exact,weak_all_recall,weak_all_top1,span_prefix_all_top1,span_key_support_all_top1,keyshape_all_top1,weak_correct_key_share,span_prefix_correct_key_share,span_key_support_correct_key_share,keyshape_correct_key_share,weak_key_entropy,span_prefix_key_entropy,span_key_support_key_entropy,keyshape_key_entropy,weak_coherent_wrong_top_key,span_prefix_coherent_wrong_top_key,span_key_support_coherent_wrong_top_key,span_key_support_qacc_delta,span_key_support_span_exact_delta,keyshape_span_exact_delta,routing_trigger_rate_mean,active_jump_rate_mean"
    next
  }
  {
    rows++
  ...
  END {
    if (rows < 6) {
      printf "invalid span key-support rows\n" > "/dev/stderr"
      exit 4
    }
