...
awk -F, '
  ...
  }
  {
    ...
    if ($idx["gate"] == "teacher-label-contract" && $idx["status"] != "blocked") {
      die("teacher-label contract should block distillation", 24)
    }
