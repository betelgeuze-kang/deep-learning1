...
slice_file_rows = [
    ...
    file_row("v61-ssd-moe-runtime-roadmap", "v61/one_token_path.json", "one-token-runtime-policy", "include"),
...
slice_verification_rows = [
    ...
    command_row("v61-ssd-moe-runtime-roadmap", "tools/verify_artifact.py v61-one-token v61/one_token_path.json --v61aa-summary results/v61aa_hotset_tensor_slice_verifier_summary.csv --v61ab-summary results/v61ab_hotset_tensor_tile_quant_probe_summary.csv", "one-token runtime policy keeps SSD-resident runtime claims blocked until milestones 1-6 prove out", "local-smoke"),
...
blocker_required_artifact_rows = [
    ...
    required_artifact_row(
        "de-30b70b-baselines-missing",
        "d-answer-citation-raw-output",
        "V52D_30B_LLM_RAG_EVIDENCE_DIR/{llm_rag_answer_rows.csv,llm_rag_citation_rows.csv,ollama_generation_transcript_rows.csv}",
        "csv-set",
        "one D raw answer/citation row per frozen query with prompt/context/retrieval budget, seed, transcript sha256, source-span-bound citations, fixture_rows=0, measured_registry_candidate=1",
        "V52D_30B_LLM_RAG_EVIDENCE_DIR=<D_DIR> V52D_70B_LLM_RAG_EVIDENCE_DIR=<E_DIR> experiments/test_v52d_30b70b_llm_rag_evidence_intake.sh",
        "D raw answer/citation outputs replay on the frozen query/source surface",
    ),
    required_artifact_row(
        "de-30b70b-baselines-missing",
        "d-resource-evaluator-manifest",
        "V52D_30B_LLM_RAG_EVIDENCE_DIR/{llm_rag_resource_rows.csv,sha256_manifest.csv}",
        "csv-set",
        "D resource rows and evaluator version/artifact hashes for the same frozen query set, fixture_rows=0, measured_registry_candidate=1",
        "V52D_30B_LLM_RAG_EVIDENCE_DIR=<D_DIR> V52D_70B_LLM_RAG_EVIDENCE_DIR=<E_DIR> experiments/test_v52d_30b70b_llm_rag_evidence_intake.sh",
        "D resource/evaluator manifest replays on the frozen query/source surface",
    ),
    required_artifact_row(
        "de-30b70b-baselines-missing",
        "e-model-identity",
    ...
    required_artifact_row(
        "de-30b70b-baselines-missing",
        "e-answer-citation-raw-output",
        "V52D_70B_LLM_RAG_EVIDENCE_DIR/{llm_rag_answer_rows.csv,llm_rag_citation_rows.csv,ollama_generation_transcript_rows.csv}",
        "csv-set",
        "one E raw answer/citation row per frozen query with prompt/context/retrieval budget, seed, transcript sha256, source-span-bound citations, fixture_rows=0, measured_registry_candidate=1",
        "V52D_30B_LLM_RAG_EVIDENCE_DIR=<D_DIR> V52D_70B_LLM_RAG_EVIDENCE_DIR=<E_DIR> experiments/test_v52d_30b70b_llm_rag_evidence_intake.sh",
        "E raw answer/citation outputs replay on the frozen query/source surface",
    ),
    required_artifact_row(
        "de-30b70b-baselines-missing",
        "e-resource-evaluator-manifest",
        "V52D_70B_LLM_RAG_EVIDENCE_DIR/{llm_rag_resource_rows.csv,sha256_manifest.csv}",
        "csv-set",
        "E resource rows and evaluator version/artifact hashes for the same frozen query set, fixture_rows=0, measured_registry_candidate=1",
        "V52D_30B_LLM_RAG_EVIDENCE_DIR=<D_DIR> V52D_70B_LLM_RAG_EVIDENCE_DIR=<E_DIR> experiments/test_v52d_30b70b_llm_rag_evidence_intake.sh",
        "E resource/evaluator manifest replays on the frozen query/source surface",
    ),
    required_artifact_row(
        "external-human-label-evidence-missing",
    ...
    required_artifact_row(
        ...
        "csv",
        "per-(blind_response_id, blind_eval_id, blind_system_id) latency_ns, memory_peak_bytes, and resource_trace_sha256 rows kept separate from answer quality",
        "V58C_BLIND_RESPONSE_EVIDENCE_DIR=<BLIND_RESPONSE_DIR> experiments/test_v58c_blind_response_evidence_intake.sh",
    ...
    required_artifact_row(
        ...
        "review-return",
        "per-(blind_response_id, blind_eval_id, blind_system_id) human blind review rows with reviewer_blinded, reviewer_independent, conflict_disclosed, source_span_exactness, unsupported_abstention_correctness, and resource columns forbidden",
    ...
    required_artifact_row(
        ...
        "review-return",
        "per-(blind_response_id, blind_eval_id, blind_system_id) adjudication rows with reviewer_a_id, reviewer_b_id, reviewer decisions, adjudicator_id, adjudicated_decision, inter_rater_agree, and resource columns forbidden",
...
def return_template_for_artifact(row):
    ...
    headers = {
        ...
        "d-answer-citation-raw-output": "system_id,query_id,same_query_set_id,prompt_template_sha256,context_budget,retrieval_budget,seed,answer_row_path,citation_row_path,raw_answer,raw_citation,raw_output_sha256,generation_transcript_sha256,answer_rows_sha256,citation_rows_sha256,fixture_rows,measured_registry_candidate,non_fixture_declared,approval_reference",
        "d-resource-evaluator-manifest": "system_id,query_id,same_query_set_id,resource_row_path,latency_ns,raw_prompt_context_bytes,retrieved_span_rows,peak_memory_mb,evaluator_version,evaluator_artifact_sha256,resource_rows_sha256,fixture_rows,measured_registry_candidate,non_fixture_declared,approval_reference",
        "e-answer-citation-raw-output": "system_id,query_id,same_query_set_id,prompt_template_sha256,context_budget,retrieval_budget,seed,answer_row_path,citation_row_path,raw_answer,raw_citation,raw_output_sha256,generation_transcript_sha256,answer_rows_sha256,citation_rows_sha256,fixture_rows,measured_registry_candidate,non_fixture_declared,approval_reference",
        "e-resource-evaluator-manifest": "system_id,query_id,same_query_set_id,resource_row_path,latency_ns,raw_prompt_context_bytes,retrieved_span_rows,peak_memory_mb,evaluator_version,evaluator_artifact_sha256,resource_rows_sha256,fixture_rows,measured_registry_candidate,non_fixture_declared,approval_reference",
        ...
        "v58-blind-response-rows": "blind_response_id,blind_eval_id,blind_system_id,response_text,citation_source_span_id,abstained,output_sha256,latency_ns,memory_peak_bytes,cost_usd,model_run_id,credential_redacted,resource_trace_sha256,latency_memory_excluded_from_quality_score",
        "v58-run-identity-rows": "blind_system_id,source_system_id,model_or_architecture_id,corpus_id,context_budget,retrieval_budget,prompt_template_sha256,source_manifest_sha256,external_api_used",
        "v58-query-split-rows": "query_id,repo_id,split_name,unseen_repository,frozen_query_packet_sha256,source_manifest_sha256",
        "v58-resource-rows": "blind_response_id,blind_eval_id,blind_system_id,latency_ns,memory_peak_bytes,resource_trace_sha256,latency_memory_excluded_from_quality_score",
        "v58-human-review-rows": "blind_response_id,blind_eval_id,blind_system_id,reviewer_id,reviewer_pool_id,reviewer_blinded,reviewer_independent,conflict_disclosed,answer_correctness,citation_correctness,abstain_correctness,source_span_exactness,unsupported_abstention_correctness,unseen_repository_split_id,latency_memory_excluded_from_quality_score,policy_score,review_decision,review_sha256",
        "v58-adjudication-rows": "blind_response_id,blind_eval_id,blind_system_id,reviewer_a_id,reviewer_b_id,reviewer_a_decision,reviewer_b_decision,adjudicated_decision,inter_rater_agree,adjudicator_id,adjudicator_pool_id,adjudicator_independent,adjudication_sha256",
...
pm_retrieval_leakage_guard_rows = [
    leakage_guard_row("source-span-id", "source span ID", "source_span_id;span_id;source_span_row_id;span_row_id"),
    leakage_guard_row("source-path", "source path", "source_path;source_file_path;file_path;repo_path;path;parsed_path"),
    leakage_guard_row("source-line", "source line", "source_line;source_line_start;source_line_end;line;start_line;end_line;line_start;line_end;parsed_line"),
    leakage_guard_row("source-file-hash", "source file hash", "source_file_hash;source_file_sha256;source_sha256;file_sha256;content_sha256;sha256;blob_sha256;git_blob_sha;source_git_blob_sha"),
    leakage_guard_row("query-source-direct-binding", "query ID and direct source row binding", "query_id;case_id;source_row_id;source_case_id;source_query_id;query_source_id;source_binding_id"),
    leakage_guard_row("expected-behavior", "expected behavior", "expected_behavior;expected_answer;expected_answer_sha256;expected_citation;expected_output;gold_answer;gold_citation", evaluator_only_required=False),
    leakage_guard_row("expected-label", "expected label", "negative_or_abstain;audit_type;expected_label;gold_label;target_label"),
]
