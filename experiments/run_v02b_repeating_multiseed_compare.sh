...
run_mode() {
  ...
  case "$mode" in
    control)
      csv_path="$RESULTS_DIR/v02b_text_off_seed${seed}.csv"
      mode_args=(--lambda-b 0 --eta-b 0)
      CONTROL_FILES+=("$csv_path")
      ;;
    weak)
      csv_path="$RESULTS_DIR/v02b_text_lv0_lb010_eb002_seed${seed}.csv"
  ...
  echo "repeating-text: ${mode} seed=${seed} -> $(basename "$csv_path")"
...
echo
echo "repeating-text multiseed summary (default proposal count, seeds 1..5)"
