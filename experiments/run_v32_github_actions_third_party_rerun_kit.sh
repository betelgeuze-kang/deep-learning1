...
runbook.write_text(
    "\n".join(
        [
            ...
            "- The workflow can run either from a PR targeting `main` or from `workflow_dispatch` after it exists on the default branch.",
            "",
            "PR path:",
            "",
            "```bash",
            ...
            "",
            "```bash",
            "V18_THIRD_PARTY_RERUN_DIR=/path/to/downloaded/results/github_actions_third_party_rerun/<return_id>/third_party_return \\",
