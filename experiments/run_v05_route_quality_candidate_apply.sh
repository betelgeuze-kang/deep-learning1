...
CYCLES_PER_EPOCH=10
PROPOSAL_COUNT=22
PREFIX="v05_route_quality_candidate_apply"
KEY_COUNTS=(64 128)
SEEDS=(1 2 3)
NOISY_RATES=(0.25)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=6
  PROPOSAL_COUNT=18
  PREFIX="v05_route_quality_candidate_apply_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
  NOISY_RATES=(0.25)
elif [[ "$MODE" == "full" ]]; then
...
compute_metrics() {
  ...
  awk -F, '
    BEGIN {
      name_count = split("fixture_query_byte_acc route_quality_apply_active route_quality_source_ranking_delta_mean route_quality_candidate_weight_beta route_quality_candidate_weight_factor_mean route_quality_candidate_weight_factor_correct_mean route_quality_candidate_weight_factor_wrong_mean route_quality_candidate_weight_factor_gap route_quality_candidate_weight_correct_mean route_quality_candidate_weight_wrong_mean route_quality_candidate_weight_gap route_quality_candidate_best_correct_rate route_quality_selected_raw_rate route_quality_selected_keyshape_rate route_quality_selected_noisy_rate route_quality_selected_raw_qacc route_hint_candidate_lookup_count route_hint_value_read_distance_mean routing_trigger_rate active_jump_rate", names, " ")
    }
    NR == 1 {
...
emit_aggregate() {
  awk -F, '
    ...
    BEGIN {
      metric_count = split("qacc route_quality_apply_active route_quality_source_ranking_delta_mean route_quality_candidate_weight_beta route_quality_candidate_weight_factor_mean route_quality_candidate_weight_factor_correct_mean route_quality_candidate_weight_factor_wrong_mean route_quality_candidate_weight_factor_gap route_quality_candidate_weight_correct_mean route_quality_candidate_weight_wrong_mean route_quality_candidate_weight_gap route_quality_candidate_best_correct_rate route_quality_selected_raw_rate route_quality_selected_keyshape_rate route_quality_selected_noisy_rate route_quality_selected_raw_qacc lookup_count read_distance routing_trigger_rate active_jump_rate", metrics, " ")
    }
    NR == 1 {
      for (i = 1; i <= NF; i++) idx[$i] = i
      printf "arm,rows"
...
run_case() {
  ...
  local candidate_min="$5"
  local candidate_max="$6"
  local key_count="$7"
  local seed="$8"
  local noisy_rate="$9"
  ...
  echo "quality-candidate-apply: ${scenario} apply=${quality_apply} source_beta=${source_beta} candidate_beta=${candidate_beta}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
    ...
    --route-quality-candidate-weight-min "$candidate_min" \
...
for key_count in "${KEY_COUNTS[@]}"; do
  for seed in "${SEEDS[@]}"; do
    for noisy_rate in "${NOISY_RATES[@]}"; do
      run_case "proxy-off" "none" "0.0" "0.0" "0.5" "2.0" "$key_count" "$seed" "$noisy_rate"
      run_case "source-ranking" "source-ranking" "0.10" "0.0" "0.5" "2.0" "$key_count" "$seed" "$noisy_rate"
      run_case "candidate-b0p10" "candidate-weight" "0.0" "0.10" "0.5" "2.0" "$key_count" "$seed" "$noisy_rate"
      run_case "candidate-b0p25" "candidate-weight" "0.0" "0.25" "0.5" "2.0" "$key_count" "$seed" "$noisy_rate"
      run_case "candidate-b0p50" "candidate-weight" "0.0" "0.50" "0.5" "2.0" "$key_count" "$seed" "$noisy_rate"
    done
  done
done
