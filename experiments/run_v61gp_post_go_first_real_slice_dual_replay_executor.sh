#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RESULTS_DIR="$ROOT_DIR/results"
PREFIX="v61gp_post_go_first_real_slice_dual_replay_executor"
RUN_ID="${V61GP_RUN_ID:-replay_001}"
RUN_DIR="$RESULTS_DIR/$PREFIX/$RUN_ID"
SUMMARY_CSV="$RESULTS_DIR/${PREFIX}_summary.csv"
DECISION_CSV="$RESULTS_DIR/${PREFIX}_decision.csv"
EXECUTE_REPLAY="${V61GP_EXECUTE_REPLAY:-0}"

if [[ "${V61GP_REUSE_EXISTING:-0}" == "1" && -s "$SUMMARY_CSV" && -s "$RUN_DIR/sha256_manifest.csv" ]]; then
  echo "v61gp_post_go_first_real_slice_dual_replay_executor_dir: $RUN_DIR"
  echo "summary: $SUMMARY_CSV"
  echo "decision: $DECISION_CSV"
  exit 0
fi

rm -rf "$RUN_DIR"
mkdir -p "$RUN_DIR"

V61GO_REUSE_EXISTING=1 "$ROOT_DIR/experiments/run_v61go_post_gn_first_real_slice_operator_input_materializer.sh" >/dev/null

python3 - "$ROOT_DIR" "$RUN_DIR" "$SUMMARY_CSV" "$DECISION_CSV" "$RUN_ID" "$EXECUTE_REPLAY" <<'PY'
import csv
import hashlib
import json
import os
import shutil
import subprocess
import sys
from datetime import datetime, timezone
from pathlib import Path

root = Path(sys.argv[1]).resolve()
run_dir = Path(sys.argv[2])
summary_csv = Path(sys.argv[3])
decision_csv = Path(sys.argv[4])
run_id = sys.argv[5]
execute_replay = int((sys.argv[6].strip() or "0") == "1")
results = root / "results"
prefix = "v61gp_post_go_first_real_slice_dual_replay_executor"
package_dir = run_dir / "first_real_slice_dual_replay_executor"
package_dir.mkdir(parents=True, exist_ok=True)

GO_PREFIX = "v61go_post_gn_first_real_slice_operator_input_materializer"
GJ_PREFIX = "v61gj_post_gi_operator_input_receiver"
EXPECTED_ACK = "operator-confirmed-real-external-review-and-generation-return"
NONFINAL_TOKENS = ["replace_with", "template", "fixture", "synthetic", "dry run", "sample", "example"]


def sha256(path):
    h = hashlib.sha256()
    with path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            h.update(chunk)
    return "sha256:" + h.hexdigest()


def read_csv(path):
    with path.open(newline="", encoding="utf-8") as handle:
        return list(csv.DictReader(handle))


def write_csv(path, fieldnames, rows):
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames, lineterminator="\n")
        writer.writeheader()
        writer.writerows(rows)


def copy_source(source_id, src, folder):
    dst = run_dir / folder / src.name
    dst.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(src, dst)
    return {
        "source_id": source_id,
        "path": dst.relative_to(run_dir).as_posix(),
        "bytes": str(dst.stat().st_size),
        "sha256": sha256(dst),
        "metadata_only": "1",
    }


def as_int(row, key):
    try:
        return int(row.get(key, "0") or "0")
    except ValueError:
        return 0


def is_inside(child, parent):
    try:
        child.resolve().relative_to(parent.resolve())
        return True
    except ValueError:
        return False


def has_nonfinal_text(value):
    lowered = str(value).lower()
    return any(token in lowered for token in NONFINAL_TOKENS)


def run_receiver(run_label, operator_root, output_root, execute_assembly):
    env = os.environ.copy()
    env.update({
        "V61GJ_RUN_ID": run_label,
        "V61GJ_OPERATOR_INPUT_ROOT": str(operator_root),
        "V61GJ_OUTPUT_ROOT": str(output_root) if output_root is not None else "",
        "V61GI_OUTPUT_ROOT": str(output_root) if output_root is not None else "",
        "V61GJ_EXECUTE_ASSEMBLY": "1" if execute_assembly else "0",
        "V61GJ_REUSE_EXISTING": "0",
    })
    proc = subprocess.run(
        [str(root / "experiments" / "run_v61gj_post_gi_operator_input_receiver.sh")],
        env=env,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    )
    (run_dir / f"{run_label}_stdout.txt").write_text(proc.stdout, encoding="utf-8")
    (run_dir / f"{run_label}_stderr.txt").write_text(proc.stderr, encoding="utf-8")
    return proc


source_paths = {
    "v61go_summary": results / f"{GO_PREFIX}_summary.csv",
    "v61go_decision": results / f"{GO_PREFIX}_decision.csv",
}
for label, path in source_paths.items():
    if not path.is_file():
        raise SystemExit(f"missing v61gp source {label}: {path}")

source_rows = [copy_source(label, path, "source_v61go") for label, path in source_paths.items()]
write_csv(run_dir / "first_real_slice_dual_replay_executor_source_rows.csv", list(source_rows[0].keys()), source_rows)

go = read_csv(source_paths["v61go_summary"])[0]
if go.get("v61go_post_gn_first_real_slice_operator_input_materializer_ready") != "1":
    raise SystemExit("v61gp requires v61go ready")

operator_root_raw = os.environ.get("V61GP_OPERATOR_INPUT_ROOT", os.environ.get("V61GI_OPERATOR_INPUT_ROOT", "")).strip()
output_root_raw = os.environ.get("V61GP_OUTPUT_ROOT", os.environ.get("V61GI_OUTPUT_ROOT", "")).strip()
operator_root = Path(operator_root_raw).expanduser().resolve() if operator_root_raw else None
output_root = Path(output_root_raw).expanduser().resolve() if output_root_raw else None
operator_root_supplied = int(operator_root is not None)
operator_root_exists = int(operator_root is not None and operator_root.is_dir())
operator_root_outside_repo = int(operator_root is not None and not is_inside(operator_root, root))
output_root_supplied = int(output_root is not None)
output_root_exists = int(output_root is not None and output_root.exists())
output_root_outside_repo = int(output_root is not None and not is_inside(output_root, root))

ack = os.environ.get("V61GP_EXTERNAL_RETURN_AUTHORITY_ACK", "").strip()
ack_statement = os.environ.get("V61GP_EXTERNAL_RETURN_AUTHORITY_STATEMENT", "").strip()
external_real_ack_ready = int(
    ack == EXPECTED_ACK
    and len(ack_statement) >= 80
    and not has_nonfinal_text(ack_statement)
)

preflight_executed = 0
preflight_exit_code = "not-run"
preflight_summary = {}
if operator_root_exists and operator_root_outside_repo:
    proc = run_receiver(f"{run_id}_preflight", operator_root, None, False)
    preflight_executed = 1
    preflight_exit_code = str(proc.returncode)
    if proc.returncode != 0:
        raise SystemExit(f"v61gp receiver preflight failed: {proc.returncode}")
    preflight_summary_path = results / f"{GJ_PREFIX}_summary.csv"
    preflight_summary = read_csv(preflight_summary_path)[0]
    for label, path in {
        "v61gj_preflight_summary": results / f"{GJ_PREFIX}_summary.csv",
        "v61gj_preflight_decision": results / f"{GJ_PREFIX}_decision.csv",
        "v61gj_preflight_rows": results / GJ_PREFIX / f"{run_id}_preflight" / "operator_input_receiver_preflight_rows.csv",
        "v61gj_preflight_receipt_rows": results / GJ_PREFIX / f"{run_id}_preflight" / "operator_input_receiver_receipt_rows.csv",
    }.items():
        if not path.is_file():
            raise SystemExit(f"missing v61gp preflight source {label}: {path}")
        source_rows.append(copy_source(label, path, "source_v61gj_preflight"))
    write_csv(run_dir / "first_real_slice_dual_replay_executor_source_rows.csv", list(source_rows[0].keys()), source_rows)
else:
    (run_dir / f"{run_id}_preflight_stdout.txt").write_text("", encoding="utf-8")
    (run_dir / f"{run_id}_preflight_stderr.txt").write_text("preflight-not-executed\n", encoding="utf-8")

operator_input_preflight_ready = as_int(preflight_summary, "operator_input_preflight_ready")
operator_input_receipt_ready = as_int(preflight_summary, "operator_input_receipt_ready")
ready_operator_input_rows = as_int(preflight_summary, "ready_operator_input_rows")
preflight_assembly_admitted = as_int(preflight_summary, "assembly_admitted")

replay_admitted = int(
    operator_input_preflight_ready
    and operator_input_receipt_ready
    and ready_operator_input_rows >= 12
    and output_root_supplied
    and output_root_outside_repo
    and external_real_ack_ready
)
replay_executed = 0
replay_exit_code = "not-run"
assembly_summary = {}
if execute_replay and replay_admitted:
    proc = run_receiver(f"{run_id}_assembly", operator_root, output_root, True)
    replay_executed = 1
    replay_exit_code = str(proc.returncode)
    if proc.returncode != 0:
        raise SystemExit(f"v61gp replay execution failed: {proc.returncode}")
    assembly_summary_path = results / f"{GJ_PREFIX}_summary.csv"
    assembly_summary = read_csv(assembly_summary_path)[0]
    for label, path in {
        "v61gj_assembly_summary": results / f"{GJ_PREFIX}_summary.csv",
        "v61gj_assembly_decision": results / f"{GJ_PREFIX}_decision.csv",
        "v61gj_assembly_stage_rows": results / GJ_PREFIX / f"{run_id}_assembly" / "operator_input_receiver_stage_rows.csv",
        "v61gj_assembly_command_rows": results / GJ_PREFIX / f"{run_id}_assembly" / "operator_input_receiver_command_rows.csv",
        "v61gj_assembly_source_rows": results / GJ_PREFIX / f"{run_id}_assembly" / "operator_input_receiver_source_rows.csv",
    }.items():
        if not path.is_file():
            raise SystemExit(f"missing v61gp assembly source {label}: {path}")
        source_rows.append(copy_source(label, path, "source_v61gj_assembly"))
    write_csv(run_dir / "first_real_slice_dual_replay_executor_source_rows.csv", list(source_rows[0].keys()), source_rows)
else:
    (run_dir / f"{run_id}_assembly_stdout.txt").write_text("", encoding="utf-8")
    (run_dir / f"{run_id}_assembly_stderr.txt").write_text("assembly-not-executed\n", encoding="utf-8")

counter_source = assembly_summary if replay_executed else {}
assembly_admitted = as_int(counter_source, "assembly_admitted")
assembly_executed = as_int(counter_source, "assembly_executed")
real_external_review_return_rows = as_int(counter_source, "real_external_review_return_rows")
real_adjudication_rows = as_int(counter_source, "real_adjudication_rows")
slice_answer_review_accepted_rows = as_int(counter_source, "slice_answer_review_accepted_rows")
real_generation_result_artifacts = as_int(counter_source, "real_generation_result_artifacts")
accepted_generation_result_artifacts = as_int(counter_source, "accepted_generation_result_artifacts")
generation_result_accepted_rows = as_int(counter_source, "generation_result_accepted_rows")
accepted_answer_rows = as_int(counter_source, "accepted_answer_rows")
accepted_citation_rows = as_int(counter_source, "accepted_citation_rows")
accepted_latency_rows = as_int(counter_source, "accepted_latency_rows")
row_acceptance_ready = as_int(counter_source, "row_acceptance_ready")
generation_execution_admission_ready = as_int(counter_source, "generation_execution_admission_ready")
generation_result_row_acceptance_ready = as_int(counter_source, "generation_result_row_acceptance_ready")
dual_external_return_real_ready = as_int(counter_source, "dual_external_return_real_ready")
real_return_replay_admission_ready = as_int(counter_source, "real_return_replay_admission_ready")
generation_acceptance_closure_ready = as_int(counter_source, "generation_acceptance_closure_ready")
authority_bound_replay_admission_ready = as_int(counter_source, "authority_bound_replay_admission_ready")
actual_model_generation_ready = as_int(counter_source, "actual_model_generation_ready")

root_rows = [
    {
        "root_id": "operator-input-root",
        "path": str(operator_root) if operator_root else "",
        "supplied": str(operator_root_supplied),
        "exists": str(operator_root_exists),
        "outside_repo": str(operator_root_outside_repo),
        "role": "final operator input root",
    },
    {
        "root_id": "output-root",
        "path": str(output_root) if output_root else "",
        "supplied": str(output_root_supplied),
        "exists": str(output_root_exists),
        "outside_repo": str(output_root_outside_repo),
        "role": "external dual replay output root",
    },
]
write_csv(run_dir / "first_real_slice_dual_replay_root_rows.csv", list(root_rows[0].keys()), root_rows)

target_rows = [
    {"target_id": "real-external-review-return", "current_value": str(real_external_review_return_rows), "target_condition": ">0", "ready": str(int(real_external_review_return_rows > 0))},
    {"target_id": "answer-review-accepted-rows", "current_value": str(slice_answer_review_accepted_rows), "target_condition": ">0", "ready": str(int(slice_answer_review_accepted_rows > 0))},
    {"target_id": "adjudication-rows", "current_value": str(real_adjudication_rows), "target_condition": ">0", "ready": str(int(real_adjudication_rows > 0))},
    {"target_id": "real-generation-result-artifacts", "current_value": str(real_generation_result_artifacts), "target_condition": ">0", "ready": str(int(real_generation_result_artifacts > 0))},
    {"target_id": "accepted-generation-result-artifacts", "current_value": str(accepted_generation_result_artifacts), "target_condition": ">0", "ready": str(int(accepted_generation_result_artifacts > 0))},
    {"target_id": "generation-result-accepted-rows", "current_value": str(generation_result_accepted_rows), "target_condition": ">0", "ready": str(int(generation_result_accepted_rows > 0))},
    {"target_id": "row-acceptance-ready", "current_value": str(row_acceptance_ready), "target_condition": "1", "ready": str(row_acceptance_ready)},
    {"target_id": "generation-execution-admission-ready", "current_value": str(generation_execution_admission_ready), "target_condition": "1", "ready": str(generation_execution_admission_ready)},
    {"target_id": "dual-external-return-real-ready", "current_value": str(dual_external_return_real_ready), "target_condition": "1", "ready": str(dual_external_return_real_ready)},
    {"target_id": "real-return-replay-admission-ready", "current_value": str(real_return_replay_admission_ready), "target_condition": "1", "ready": str(real_return_replay_admission_ready)},
    {"target_id": "generation-acceptance-closure-ready", "current_value": str(generation_acceptance_closure_ready), "target_condition": "1", "ready": str(generation_acceptance_closure_ready)},
    {"target_id": "actual-model-generation-ready", "current_value": str(actual_model_generation_ready), "target_condition": "1", "ready": str(actual_model_generation_ready)},
]
write_csv(run_dir / "first_real_slice_dual_replay_target_rows.csv", list(target_rows[0].keys()), target_rows)

stage_rows = [
    {"stage_id": "01-v61go-source", "status": "ready", "evidence": "v61go ready"},
    {"stage_id": "02-operator-input-root", "status": "ready" if operator_root_exists and operator_root_outside_repo else "blocked", "evidence": f"supplied={operator_root_supplied}; exists={operator_root_exists}; outside_repo={operator_root_outside_repo}"},
    {"stage_id": "03-receiver-preflight", "status": "ready" if operator_input_preflight_ready else "blocked", "evidence": f"preflight_executed={preflight_executed}; ready_rows={ready_operator_input_rows}; receipt={operator_input_receipt_ready}"},
    {"stage_id": "04-output-root", "status": "ready" if output_root_supplied and output_root_outside_repo else "blocked", "evidence": f"supplied={output_root_supplied}; outside_repo={output_root_outside_repo}"},
    {"stage_id": "05-external-real-ack", "status": "ready" if external_real_ack_ready else "blocked", "evidence": f"ack_ready={external_real_ack_ready}"},
    {"stage_id": "06-replay-admitted", "status": "ready" if replay_admitted else "blocked", "evidence": f"replay_admitted={replay_admitted}"},
    {"stage_id": "07-replay-executed", "status": "ready" if replay_executed else "blocked", "evidence": f"replay_executed={replay_executed}; exit_code={replay_exit_code}"},
    {"stage_id": "08-dual-return-counters", "status": "ready" if dual_external_return_real_ready else "blocked", "evidence": f"row_acceptance_ready={row_acceptance_ready}; generation_result_row_acceptance_ready={generation_result_row_acceptance_ready}"},
    {"stage_id": "09-actual-generation-full-claim", "status": "blocked", "evidence": f"actual_model_generation_ready={actual_model_generation_ready}"},
]
write_csv(run_dir / "first_real_slice_dual_replay_stage_rows.csv", list(stage_rows[0].keys()), stage_rows)

command_rows = [
    {"command_id": "01-verify-dual-replay-executor-package", "ready_to_run_now": "1", "command": "results/v61gp_post_go_first_real_slice_dual_replay_executor/replay_001/first_real_slice_dual_replay_executor/VERIFY_FIRST_REAL_SLICE_DUAL_REPLAY_EXECUTOR.sh", "purpose": "verify this replay executor package"},
    {"command_id": "02-print-ready-commands", "ready_to_run_now": "1", "command": "results/v61gp_post_go_first_real_slice_dual_replay_executor/replay_001/first_real_slice_dual_replay_executor/READY_NOW_COMMANDS.sh", "purpose": "show guarded replay command"},
    {"command_id": "03-preflight-final-input", "ready_to_run_now": str(int(operator_root_exists and operator_root_outside_repo)), "command": "V61GP_OPERATOR_INPUT_ROOT=<operator-input-root> ./experiments/run_v61gp_post_go_first_real_slice_dual_replay_executor.sh", "purpose": "run v61gj preflight with output root withheld"},
    {"command_id": "04-execute-dual-replay", "ready_to_run_now": str(replay_admitted), "command": "V61GP_EXECUTE_REPLAY=1 V61GP_OPERATOR_INPUT_ROOT=<operator-input-root> V61GP_OUTPUT_ROOT=<external-output-root> V61GP_EXTERNAL_RETURN_AUTHORITY_ACK=operator-confirmed-real-external-review-and-generation-return V61GP_EXTERNAL_RETURN_AUTHORITY_STATEMENT=<final-external-statement> ./experiments/run_v61gp_post_go_first_real_slice_dual_replay_executor.sh", "purpose": "execute v61gj assembly/replay only after real external return acknowledgement"},
    {"command_id": "05-check-dual-replay-opened", "ready_to_run_now": str(int(real_return_replay_admission_ready and generation_acceptance_closure_ready)), "command": "results/v61gp_post_go_first_real_slice_dual_replay_executor/replay_001/first_real_slice_dual_replay_executor/CHECK_DUAL_REPLAY_OPENED.py", "purpose": "assert subset dual replay counters opened"},
]
write_csv(run_dir / "first_real_slice_dual_replay_command_rows.csv", list(command_rows[0].keys()), command_rows)

for rel, src in [
    ("FIRST_REAL_SLICE_DUAL_REPLAY_ROOT_ROWS.csv", run_dir / "first_real_slice_dual_replay_root_rows.csv"),
    ("FIRST_REAL_SLICE_DUAL_REPLAY_TARGET_ROWS.csv", run_dir / "first_real_slice_dual_replay_target_rows.csv"),
    ("FIRST_REAL_SLICE_DUAL_REPLAY_STAGE_ROWS.csv", run_dir / "first_real_slice_dual_replay_stage_rows.csv"),
    ("FIRST_REAL_SLICE_DUAL_REPLAY_COMMAND_ROWS.csv", run_dir / "first_real_slice_dual_replay_command_rows.csv"),
]:
    shutil.copy2(src, package_dir / rel)

(package_dir / "CHECK_DUAL_REPLAY_OPENED.py").write_text(
    "\n".join([
        "#!/usr/bin/env python3",
        "import csv",
        "from pathlib import Path",
        "",
        f"SUMMARY = Path({str(summary_csv)!r})",
        "",
        "with SUMMARY.open(newline='', encoding='utf-8') as handle:",
        "    row = next(csv.DictReader(handle))",
        "required_positive = [",
        "    'real_external_review_return_rows',",
        "    'real_adjudication_rows',",
        "    'slice_answer_review_accepted_rows',",
        "    'real_generation_result_artifacts',",
        "    'accepted_generation_result_artifacts',",
        "    'generation_result_accepted_rows',",
        "]",
        "required_ready = [",
        "    'row_acceptance_ready',",
        "    'generation_execution_admission_ready',",
        "    'generation_result_row_acceptance_ready',",
        "    'dual_external_return_real_ready',",
        "    'real_return_replay_admission_ready',",
        "    'generation_acceptance_closure_ready',",
        "    'authority_bound_replay_admission_ready',",
        "]",
        "errors = []",
        "for key in required_positive:",
        "    if int(row.get(key, '0') or '0') <= 0:",
        "        errors.append(f'{key}={row.get(key)}')",
        "for key in required_ready:",
        "    if row.get(key) != '1':",
        "        errors.append(f'{key}={row.get(key)}')",
        "if errors:",
        "    raise SystemExit('dual replay remains blocked: ' + ';'.join(errors))",
        "print('subset dual replay opened')",
        "",
    ]),
    encoding="utf-8",
)
(package_dir / "CHECK_DUAL_REPLAY_OPENED.py").chmod(0o755)

(package_dir / "VERIFY_FIRST_REAL_SLICE_DUAL_REPLAY_EXECUTOR.sh").write_text(
    "\n".join([
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "DIR=\"$(cd \"$(dirname \"${BASH_SOURCE[0]}\")\" && pwd)\"",
        "test -s \"$DIR/FIRST_REAL_SLICE_DUAL_REPLAY_ROOT_ROWS.csv\"",
        "test -s \"$DIR/FIRST_REAL_SLICE_DUAL_REPLAY_TARGET_ROWS.csv\"",
        "test -s \"$DIR/FIRST_REAL_SLICE_DUAL_REPLAY_STAGE_ROWS.csv\"",
        "test -s \"$DIR/FIRST_REAL_SLICE_DUAL_REPLAY_COMMAND_ROWS.csv\"",
        "test -s \"$DIR/FIRST_REAL_SLICE_DUAL_REPLAY_EXECUTOR_MANIFEST.json\"",
        "test -x \"$DIR/CHECK_DUAL_REPLAY_OPENED.py\"",
        "if find \"$DIR\" -type f \\( -name '*.safetensors' -o -name '*.gguf' -o -name '*.bin' -o -name '*.pt' -o -name '*.pth' \\) | grep -q .; then",
        "  echo 'payload-like file in v61gp package' >&2",
        "  exit 1",
        "fi",
        "",
    ]),
    encoding="utf-8",
)
(package_dir / "VERIFY_FIRST_REAL_SLICE_DUAL_REPLAY_EXECUTOR.sh").chmod(0o755)

(package_dir / "READY_NOW_COMMANDS.sh").write_text(
    "\n".join([
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "echo 'Preflight only:'",
        "echo 'V61GP_OPERATOR_INPUT_ROOT=<operator-input-root> ./experiments/run_v61gp_post_go_first_real_slice_dual_replay_executor.sh'",
        "echo 'Execute only after real external return acknowledgement:'",
        "echo 'V61GP_EXECUTE_REPLAY=1 V61GP_OPERATOR_INPUT_ROOT=<operator-input-root> V61GP_OUTPUT_ROOT=<external-output-root> V61GP_EXTERNAL_RETURN_AUTHORITY_ACK=operator-confirmed-real-external-review-and-generation-return V61GP_EXTERNAL_RETURN_AUTHORITY_STATEMENT=<final-external-statement> ./experiments/run_v61gp_post_go_first_real_slice_dual_replay_executor.sh'",
        "echo 'Then check:'",
        "echo 'results/v61gp_post_go_first_real_slice_dual_replay_executor/replay_001/first_real_slice_dual_replay_executor/CHECK_DUAL_REPLAY_OPENED.py'",
        "",
    ]),
    encoding="utf-8",
)
(package_dir / "READY_NOW_COMMANDS.sh").chmod(0o755)

contains_real_external_evidence = int(
    real_external_review_return_rows > 0
    or real_generation_result_artifacts > 0
    or dual_external_return_real_ready
)

summary = {
    "v61gp_post_go_first_real_slice_dual_replay_executor_ready": 1,
    "v61go_post_gn_first_real_slice_operator_input_materializer_ready": 1,
    "contains_real_external_evidence": contains_real_external_evidence,
    "operator_input_root_supplied": operator_root_supplied,
    "operator_input_root_exists": operator_root_exists,
    "operator_input_root_outside_repo": operator_root_outside_repo,
    "output_root_supplied": output_root_supplied,
    "output_root_exists": output_root_exists,
    "output_root_outside_repo": output_root_outside_repo,
    "external_real_ack_ready": external_real_ack_ready,
    "preflight_executed": preflight_executed,
    "preflight_exit_code": preflight_exit_code,
    "ready_operator_input_rows": ready_operator_input_rows,
    "operator_input_receipt_ready": operator_input_receipt_ready,
    "operator_input_preflight_ready": operator_input_preflight_ready,
    "preflight_assembly_admitted": preflight_assembly_admitted,
    "replay_admitted": replay_admitted,
    "replay_requested": execute_replay,
    "replay_executed": replay_executed,
    "replay_exit_code": replay_exit_code,
    "assembly_admitted": assembly_admitted,
    "assembly_executed": assembly_executed,
    "real_external_review_return_rows": real_external_review_return_rows,
    "real_adjudication_rows": real_adjudication_rows,
    "slice_answer_review_accepted_rows": slice_answer_review_accepted_rows,
    "real_generation_result_artifacts": real_generation_result_artifacts,
    "accepted_generation_result_artifacts": accepted_generation_result_artifacts,
    "generation_result_accepted_rows": generation_result_accepted_rows,
    "accepted_answer_rows": accepted_answer_rows,
    "accepted_citation_rows": accepted_citation_rows,
    "accepted_latency_rows": accepted_latency_rows,
    "row_acceptance_ready": row_acceptance_ready,
    "generation_execution_admission_ready": generation_execution_admission_ready,
    "generation_result_row_acceptance_ready": generation_result_row_acceptance_ready,
    "dual_external_return_real_ready": dual_external_return_real_ready,
    "real_return_replay_admission_ready": real_return_replay_admission_ready,
    "generation_acceptance_closure_ready": generation_acceptance_closure_ready,
    "authority_bound_replay_admission_ready": authority_bound_replay_admission_ready,
    "actual_model_generation_ready": actual_model_generation_ready,
    "target_rows": len(target_rows),
    "ready_target_rows": sum(row["ready"] == "1" for row in target_rows),
    "command_rows": len(command_rows),
    "ready_command_rows": sum(row["ready_to_run_now"] == "1" for row in command_rows),
    "blocked_command_rows": sum(row["ready_to_run_now"] == "0" for row in command_rows),
    "stage_rows": len(stage_rows),
    "ready_stage_rows": sum(row["status"] == "ready" for row in stage_rows),
    "blocked_stage_rows": sum(row["status"] == "blocked" for row in stage_rows),
    "source_file_rows": len(source_rows),
    "checkpoint_payload_bytes_downloaded_by_v61gp": 0,
    "checkpoint_payload_bytes_committed_to_repo": 0,
    "route_jump_rows": 0,
}

manifest = {
    "artifact": prefix,
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "expected_external_ack": EXPECTED_ACK,
    "summary": summary,
    "checkpoint_payload_bytes_committed_to_repo": 0,
}
(package_dir / "FIRST_REAL_SLICE_DUAL_REPLAY_EXECUTOR_MANIFEST.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
(package_dir / "FIRST_REAL_SLICE_DUAL_REPLAY_EXECUTOR.md").write_text(
    "\n".join([
        "# v61gp first real slice dual replay executor",
        "",
        f"- operator_input_preflight_ready={operator_input_preflight_ready}",
        f"- external_real_ack_ready={external_real_ack_ready}",
        f"- replay_admitted={replay_admitted}",
        f"- replay_executed={replay_executed}",
        f"- dual_external_return_real_ready={dual_external_return_real_ready}",
        f"- real_return_replay_admission_ready={real_return_replay_admission_ready}",
        f"- generation_acceptance_closure_ready={generation_acceptance_closure_ready}",
        f"- actual_model_generation_ready={actual_model_generation_ready}",
        "",
        "This gate withholds output-root assembly unless the explicit real external return acknowledgement and execute flag are present.",
        "",
    ]),
    encoding="utf-8",
)

package_files = sorted(path for path in package_dir.rglob("*") if path.is_file())
package_file_rows = []
for path in package_files:
    payload_like = int(path.suffix.lower() in {".safetensors", ".gguf", ".bin", ".pt", ".pth"})
    package_file_rows.append({
        "path": path.relative_to(run_dir).as_posix(),
        "bytes": str(path.stat().st_size),
        "sha256": sha256(path),
        "metadata_only": str(int(not payload_like)),
        "payload_like": str(payload_like),
    })
write_csv(run_dir / "first_real_slice_dual_replay_executor_package_file_rows.csv", list(package_file_rows[0].keys()), package_file_rows)
summary["package_file_rows"] = len(package_file_rows)
summary["metadata_only_package_file_rows"] = sum(row["metadata_only"] == "1" for row in package_file_rows)
summary["payload_like_package_file_rows"] = sum(row["payload_like"] == "1" for row in package_file_rows)
write_csv(summary_csv, list(summary.keys()), [summary])
write_csv(run_dir / f"{prefix}_summary.csv", list(summary.keys()), [summary])

decision_rows = [
    {"gate": "source-v61go-ready", "status": "pass", "evidence": "v61go ready"},
    {"gate": "operator-input-root", "status": "pass" if operator_root_exists and operator_root_outside_repo else "blocked", "evidence": f"supplied={operator_root_supplied}; exists={operator_root_exists}; outside_repo={operator_root_outside_repo}"},
    {"gate": "receiver-preflight", "status": "pass" if operator_input_preflight_ready else "blocked", "evidence": f"operator_input_preflight_ready={operator_input_preflight_ready}"},
    {"gate": "output-root", "status": "pass" if output_root_supplied and output_root_outside_repo else "blocked", "evidence": f"supplied={output_root_supplied}; outside_repo={output_root_outside_repo}"},
    {"gate": "external-real-ack", "status": "pass" if external_real_ack_ready else "blocked", "evidence": f"external_real_ack_ready={external_real_ack_ready}"},
    {"gate": "replay-admitted", "status": "pass" if replay_admitted else "blocked", "evidence": f"replay_admitted={replay_admitted}"},
    {"gate": "replay-executed", "status": "pass" if replay_executed else "blocked", "evidence": f"replay_executed={replay_executed}; exit_code={replay_exit_code}"},
    {"gate": "row-acceptance", "status": "pass" if row_acceptance_ready else "blocked", "evidence": f"row_acceptance_ready={row_acceptance_ready}"},
    {"gate": "dual-external-return-real", "status": "pass" if dual_external_return_real_ready else "blocked", "evidence": f"dual_external_return_real_ready={dual_external_return_real_ready}"},
    {"gate": "real-return-replay-admission", "status": "pass" if real_return_replay_admission_ready else "blocked", "evidence": f"real_return_replay_admission_ready={real_return_replay_admission_ready}"},
    {"gate": "generation-acceptance-closure", "status": "pass" if generation_acceptance_closure_ready else "blocked", "evidence": f"generation_acceptance_closure_ready={generation_acceptance_closure_ready}"},
    {"gate": "actual-generation", "status": "blocked", "evidence": f"actual_model_generation_ready={actual_model_generation_ready}"},
    {"gate": "zero-repo-checkpoint-payload", "status": "pass", "evidence": "checkpoint_payload_bytes_committed_to_repo=0"},
]
write_csv(decision_csv, list(decision_rows[0].keys()), decision_rows)
write_csv(run_dir / f"{prefix}_decision.csv", list(decision_rows[0].keys()), decision_rows)

boundary = "\n".join([
    "# V61GP Post-GO First Real Slice Dual Replay Executor",
    "",
    "- v61gp_post_go_first_real_slice_dual_replay_executor_ready=1",
    f"- operator_input_preflight_ready={operator_input_preflight_ready}",
    f"- external_real_ack_ready={external_real_ack_ready}",
    f"- replay_admitted={replay_admitted}",
    f"- replay_executed={replay_executed}",
    f"- real_external_review_return_rows={real_external_review_return_rows}",
    f"- generation_result_accepted_rows={generation_result_accepted_rows}",
    f"- dual_external_return_real_ready={dual_external_return_real_ready}",
    f"- real_return_replay_admission_ready={real_return_replay_admission_ready}",
    f"- generation_acceptance_closure_ready={generation_acceptance_closure_ready}",
    f"- actual_model_generation_ready={actual_model_generation_ready}",
    "- checkpoint_payload_bytes_committed_to_repo=0",
    "",
    "This gate can execute the subset replay, but the default path keeps it blocked until real external acknowledgement and execution are explicit.",
    "",
])
(run_dir / "V61GP_POST_GO_FIRST_REAL_SLICE_DUAL_REPLAY_EXECUTOR_BOUNDARY.md").write_text(boundary, encoding="utf-8")

top_manifest = {
    "artifact": prefix,
    "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    "expected_external_ack": EXPECTED_ACK,
    "summary": summary,
    "decisions": decision_rows,
    "checkpoint_payload_bytes_downloaded_by_v61gp": 0,
    "checkpoint_payload_bytes_committed_to_repo": 0,
    "route_jump_rows": 0,
}
(run_dir / f"{prefix}_manifest.json").write_text(json.dumps(top_manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")

hash_rows = []
for path in sorted(p for p in run_dir.rglob("*") if p.is_file() and p.name != "sha256_manifest.csv"):
    hash_rows.append({
        "path": path.relative_to(run_dir).as_posix(),
        "bytes": str(path.stat().st_size),
        "sha256": sha256(path),
    })
write_csv(run_dir / "sha256_manifest.csv", list(hash_rows[0].keys()), hash_rows)

print(f"v61gp_post_go_first_real_slice_dual_replay_executor_dir: {run_dir}")
print(f"summary: {summary_csv}")
print(f"decision: {decision_csv}")
PY
