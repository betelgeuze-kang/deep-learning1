...
capture_script.write_text(
    "\n".join(
        [
            ...
            'CAPTURE_TIMEOUT_SECONDS="${CAPTURE_TIMEOUT_SECONDS:-900}"',
            'echo "capture_command_started: timeout_seconds=$CAPTURE_TIMEOUT_SECONDS"',
            "set +e",
            'timeout --preserve-status "$CAPTURE_TIMEOUT_SECONDS" bash -lc "$COMMAND" >"$STDOUT_PATH" 2>"$STDERR_PATH"',
            "EXIT_CODE=$?",
            "set -e",
            'echo "capture_command_finished: exit_code=$EXIT_CODE"',
