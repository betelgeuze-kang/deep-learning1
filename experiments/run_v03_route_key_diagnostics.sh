...
run_suite_for_dataset() {
  ...
  echo "route-key diagnostics: ${dataset_label} joint-code probe"
  run_case "$RESULTS_DIR/v03_route_key_${dataset_label}_joint_probe.csv" \
    "${dataset_args[@]}" "${BASE_ARGS[@]}" "${JOINT_PROBE_ARGS[@]}"

  echo "route-key diagnostics: ${dataset_label} state-code cycle probe"
  run_case "$RESULTS_DIR/v03_route_key_${dataset_label}_state_cycle_probe.csv" \
    "${dataset_args[@]}" "${BASE_ARGS[@]}" "${STATE_PROBE_ARGS[@]}"

  echo "route-key diagnostics: ${dataset_label} joint-code guarded jump"
  run_case "$RESULTS_DIR/v03_route_key_${dataset_label}_joint_guard.csv" \
    "${dataset_args[@]}" "${BASE_ARGS[@]}" "${JOINT_GUARD_ARGS[@]}"

  echo "route-key diagnostics: ${dataset_label} state-code cycle guarded jump"
  run_case "$RESULTS_DIR/v03_route_key_${dataset_label}_state_cycle_guard.csv" \
    "${dataset_args[@]}" "${BASE_ARGS[@]}" "${STATE_GUARD_ARGS[@]}"

  echo "route-key diagnostics: ${dataset_label} state-code epoch guarded jump"
  run_case "$RESULTS_DIR/v03_route_key_${dataset_label}_state_epoch_guard.csv" \
...
print_suite_for_dataset() {
  ...
  print_final_summary "${dataset_label}-joint-probe" \
    "$RESULTS_DIR/v03_route_key_${dataset_label}_joint_probe.csv"
  print_last10_summary "${dataset_label}-joint-probe" \
    "$RESULTS_DIR/v03_route_key_${dataset_label}_joint_probe.csv"

  print_final_summary "${dataset_label}-state-cycle-probe" \
    "$RESULTS_DIR/v03_route_key_${dataset_label}_state_cycle_probe.csv"
  print_last10_summary "${dataset_label}-state-cycle-probe" \
    "$RESULTS_DIR/v03_route_key_${dataset_label}_state_cycle_probe.csv"

  print_final_summary "${dataset_label}-joint-guard" \
    "$RESULTS_DIR/v03_route_key_${dataset_label}_joint_guard.csv"
  print_last10_summary "${dataset_label}-joint-guard" \
    "$RESULTS_DIR/v03_route_key_${dataset_label}_joint_guard.csv"

  print_final_summary "${dataset_label}-state-cycle-guard" \
    "$RESULTS_DIR/v03_route_key_${dataset_label}_state_cycle_guard.csv"
  print_last10_summary "${dataset_label}-state-cycle-guard" \
    "$RESULTS_DIR/v03_route_key_${dataset_label}_state_cycle_guard.csv"

  print_final_summary "${dataset_label}-state-epoch-guard" \
    "$RESULTS_DIR/v03_route_key_${dataset_label}_state_epoch_guard.csv"
  print_last10_summary "${dataset_label}-state-epoch-guard" \
    "$RESULTS_DIR/v03_route_key_${dataset_label}_state_epoch_guard.csv"
}

# Diagnostic-only route-key comparison: keep the existing parameters and
# surface the current hard route-key / state-vs-anchor diagnostics block in
# the CSV schema alongside the prediction metrics.
