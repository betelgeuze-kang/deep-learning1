...
CYCLES_PER_EPOCH=10
PROPOSAL_COUNT=22
PREFIX="v05_route_quality_candidate_default"
KEY_COUNTS=(64 128 256)
SEEDS=(1 2 3)
NOISY_RATES=(0.10 0.25 0.50)
ARMS=(
  "proxy-off:none:0.0:0.0:2.0"
  "candidate-default:candidate-weight:0.0:8.0:8.0"
  "source-candidate-default:source-candidate:0.10:8.0:8.0"
)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=6
  PROPOSAL_COUNT=18
  PREFIX="v05_route_quality_candidate_default_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
  NOISY_RATES=(0.25)
elif [[ "$MODE" == "full" ]]; then
...
compute_metrics() {
  ...
  awk -F, '
    BEGIN {
      name_count = split("fixture_query_byte_acc route_quality_apply_active route_quality_source_ranking_beta route_quality_source_ranking_delta_mean route_quality_candidate_weight_beta route_quality_candidate_weight_factor_mean route_quality_candidate_weight_factor_correct_mean route_quality_candidate_weight_factor_wrong_mean route_quality_candidate_weight_factor_gap route_quality_candidate_weight_factor_p90 route_quality_candidate_weight_factor_max route_quality_candidate_weight_entropy_mean route_quality_candidate_weight_top_share_mean route_quality_candidate_weight_correct_mean route_quality_candidate_weight_wrong_mean route_quality_candidate_weight_gap route_quality_candidate_best_correct_rate route_quality_selected_raw_rate route_quality_selected_keyshape_rate route_quality_selected_noisy_rate route_quality_selected_raw_qacc route_wrong_hint_strength_mean route_correct_hint_strength_mean route_hint_candidate_lookup_count route_hint_value_read_distance_mean routing_trigger_rate active_jump_rate", names, " ")
    }
    NR == 1 {
...
emit_aggregate() {
  awk -F, '
    ...
    BEGIN {
      metric_count = split("qacc route_quality_apply_active route_quality_source_ranking_beta route_quality_source_ranking_delta_mean route_quality_candidate_weight_beta route_quality_candidate_weight_factor_mean route_quality_candidate_weight_factor_correct_mean route_quality_candidate_weight_factor_wrong_mean route_quality_candidate_weight_factor_gap route_quality_candidate_weight_factor_p90 route_quality_candidate_weight_factor_max route_quality_candidate_weight_entropy_mean route_quality_candidate_weight_top_share_mean route_quality_candidate_weight_correct_mean route_quality_candidate_weight_wrong_mean route_quality_candidate_weight_gap route_quality_candidate_best_correct_rate route_quality_selected_raw_rate route_quality_selected_keyshape_rate route_quality_selected_noisy_rate route_quality_selected_raw_qacc route_wrong_hint_strength_mean route_correct_hint_strength_mean lookup_count read_distance routing_trigger_rate active_jump_rate", metrics, " ")
    }
    NR == 1 {
      for (i = 1; i <= NF; i++) idx[$i] = i
      printf "arm,rows"
  ...
  awk -F, '
    NR == 1 {
      for (i = 1; i <= NF; i++) idx[$i] = i
      print "arm,key_count,noisy_source_rate,rows,qacc_mean,qacc_std,factor_gap_mean,factor_p90_mean,factor_max_mean,top_share_mean,entropy_mean,wrong_strength_mean,selected_noisy_rate_mean,active_jump_rate_mean"
      next
    }
    {
      ...
      sum_p90[key] += $idx["route_quality_candidate_weight_factor_p90"] + 0
    ...
    END {
      for (i = 1; i <= key_count_seen; i++) {
        ...
        printf "%s,%s,%s,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
          parts[1], parts[2], parts[3], count[key], mean_q,
          sqrt(var_q < 0 ? 0 : var_q), sum_gap[key] / count[key],
          sum_p90[key] / count[key], sum_fmax[key] / count[key],
          sum_top[key] / count[key], sum_entropy[key] / count[key],
...
run_case() {
  ...
  echo "quality-candidate-default: ${scenario} apply=${quality_apply} source_beta=${source_beta} candidate_beta=${candidate_beta} cap=${candidate_max}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
  ...
  for seed in "${SEEDS[@]}"; do
    for noisy_rate in "${NOISY_RATES[@]}"; do
      for arm_spec in "${ARMS[@]}"; do
        IFS=: read -r arm quality_apply source_beta candidate_beta candidate_max <<<"$arm_spec"
        run_case "$arm" "$quality_apply" "$source_beta" "$candidate_beta" \
          "$candidate_max" "$key_count" "$seed" "$noisy_rate"
      done
    done
  done
done
