...
PREFIX="v61ac_hotset_token_budget_replay"
RUN_ID="${V61AC_RUN_ID:-replay_001}"
RUN_DIR="$RESULTS_DIR/$PREFIX/$RUN_ID"
SUMMARY_CSV="$RESULTS_DIR/${PREFIX}_summary.csv"
DECISION_CSV="$RESULTS_DIR/${PREFIX}_decision.csv"

if [[ "${V61AC_REUSE_EXISTING:-0}" == "1" && -s "$SUMMARY_CSV" && -s "$RUN_DIR/sha256_manifest.csv" ]]; then
  echo "v61ac_hotset_token_budget_replay_dir: $RUN_DIR"
...
v61ab_dir = results / "v61ab_hotset_tensor_tile_quant_probe" / "probe_001"
...
v61ab_summary = read_csv(results / "v61ab_hotset_tensor_tile_quant_probe_summary.csv")[0]
if v61x_summary.get("v61x_hotset_runtime_replay_manifest_ready") != "1":
    raise SystemExit("v61ac requires v61x_hotset_runtime_replay_manifest_ready=1")
if v61x_summary.get("source_bound_replay_binding_ready") != "1":
    raise SystemExit("v61ac requires source_bound_replay_binding_ready=1")
if v61z_summary.get("direct_io_replay_ready") != "1":
    raise SystemExit("v61ac requires direct_io_replay_ready=1")
if v61ab_summary.get("v61ab_hotset_tensor_tile_quant_probe_ready") != "1":
    raise SystemExit("v61ac requires v61ab_hotset_tensor_tile_quant_probe_ready=1")
...
for src, rel in [
    ...
    (results / "v61ab_hotset_tensor_tile_quant_probe_summary.csv", "source_v61ab/v61ab_hotset_tensor_tile_quant_probe_summary.csv"),
    (v61ab_dir / "hotset_tensor_tile_probe_rows.csv", "source_v61ab/hotset_tensor_tile_probe_rows.csv"),
    (v61ab_dir / "hotset_tensor_tile_quant_metric_rows.csv", "source_v61ab/hotset_tensor_tile_quant_metric_rows.csv"),
    (v61ab_dir / "sha256_manifest.csv", "source_v61ab/sha256_manifest.csv"),
]:
    copy(src, rel)
...
for sample_id, rows in tiles_by_remote_sample.items():
    ...
    if len(rows) != tiles_per_page:
        raise SystemExit(f"v61ac expects {tiles_per_page} tiles for {sample_id}")

direct_p50_ms = float(v61z_summary["direct_io_read_latency_ms_p50"])
direct_p95_ms = float(v61z_summary["direct_io_read_latency_ms_p95"])
ssd_read_bytes_per_token = active_page_reads_per_token * page_bytes
if str(ssd_read_bytes_per_token) != v61z_summary["ssd_read_bytes_per_token"]:
    raise SystemExit("v61ac ssd_read_bytes_per_token disagrees with v61z")
token_p50_ms = direct_p50_ms * active_page_reads_per_token
token_p95_ms = direct_p95_ms * active_page_reads_per_token
tile_values_per_token = active_page_reads_per_token * tiles_per_page * int(metric["tile_bf16_value_rows"]) // int(metric["tensor_tile_probe_rows"])
q8_error_budget_mean = float(metric["q8_abs_error_mean"]) * active_page_reads_per_token * tiles_per_page
q4_error_budget_mean = float(metric["q4_abs_error_mean"]) * active_page_reads_per_token * tiles_per_page
...
for query_index, workload in enumerate(workload_rows):
    ...
    for page_index, direct in enumerate(selected_direct):
        ...
        schedule_id = f"v61ac_schedule_{query_index:04d}_{page_index:02d}"
        ...
        for tile in tile_set:
            ...
            tile_binding_rows.append(
                {
                    "tile_binding_id": f"v61ac_tile_binding_{query_index:04d}_{page_index:02d}_{tile['tile_index_in_slice']}",
                    "schedule_id": schedule_id,
                    "workload_binding_id": workload["workload_binding_id"],
                    "query_id": workload["query_id"],
    ...
    budget_rows.append(
        {
            "token_budget_id": f"v61ac_token_budget_{query_index:04d}",
            "workload_binding_id": workload["workload_binding_id"],
            "query_id": workload["query_id"],
            "query_family": workload["query_family"],
            ...
            "direct_io_latency_ms_p50_per_page": fmt(direct_p50_ms),
            "direct_io_latency_ms_p95_per_page": fmt(direct_p95_ms),
            "token_direct_io_latency_ms_p50": fmt(token_p50_ms),
            "token_direct_io_latency_ms_p95": fmt(token_p95_ms),
            ...
            "hotset_token_budget_replay_ready": str(token_ready),
            "checkpoint_payload_bytes_committed_to_repo": "0",
...
metric_rows = [
    {
        "metric_id": "v61ac_hotset_token_budget_metrics",
        ...
        "q8_abs_error_budget_mean_per_token": fmt(q8_error_budget_mean),
        "q4_abs_error_budget_mean_per_token": fmt(q4_error_budget_mean),
        "hotset_token_budget_replay_ready": str(budget_ready),
        "checkpoint_payload_bytes_committed_to_repo": "0",
        "full_checkpoint_materialization_ready": "0",
        "full_safetensors_page_hash_binding_ready": "0",
        "actual_model_generation_ready": "0",
...
decision_rows = [
    ...
    {"gate": "v61z-direct-io-latency-input", "status": "pass", "reason": "direct-I/O latency metrics are present"},
    {"gate": "v61ab-numeric-tile-input", "status": "pass", "reason": "finite BF16/q8/q4 numeric tile probes are present"},
    {"gate": "hotset-token-budget-replay", "status": "pass" if budget_ready else "blocked", "reason": f"{finite_budget_rows}/{len(budget_rows)} token budget rows are ready"},
...
write_csv(run_dir / "hotset_token_budget_rows.csv", list(budget_rows[0].keys()), budget_rows)
write_csv(run_dir / "hotset_token_budget_page_schedule_rows.csv", list(schedule_rows[0].keys()), schedule_rows)
write_csv(run_dir / "hotset_token_budget_tile_binding_rows.csv", list(tile_binding_rows[0].keys()), tile_binding_rows)
write_csv(run_dir / "hotset_token_budget_metric_rows.csv", list(metric_rows[0].keys()), metric_rows)
write_csv(run_dir / "runtime_gap_rows.csv", ["gap", "status", "evidence"], runtime_gap_rows)

summary = {
    "v61ac_hotset_token_budget_replay_ready": str(budget_ready),
    ...
    "v61ab_hotset_tensor_tile_quant_probe_ready": v61ab_summary["v61ab_hotset_tensor_tile_quant_probe_ready"],
    "model_id": model_id,
    "source_bound_workload_binding_rows": str(len(workload_rows)),
    "token_budget_rows": str(len(budget_rows)),
    "token_page_schedule_rows": str(schedule_count),
...
manifest = {
    "artifact": "v61ac_hotset_token_budget_replay",
    "created_at": datetime.now(timezone.utc).isoformat(),
    "model_id": model_id,
    "run_dir": str(run_dir),
    "v61ac_hotset_token_budget_replay_ready": budget_ready,
    ...
    "token_direct_io_latency_ms_p50": token_p50_ms,
    "token_direct_io_latency_ms_p95": token_p95_ms,
    "checkpoint_payload_bytes_committed_to_repo": 0,
    "actual_model_generation_ready": 0,
    "blocked_claims": [
...
(run_dir / "v61ac_hotset_token_budget_replay_manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")

boundary = f"""# v61ac Hotset Token Budget Replay Boundary

This artifact binds v61x source-bound workload rows, v61z sampled direct-I/O
latency evidence, and v61ab sampled BF16/q8/q4 numeric tile probes into a
bounded per-token hotset budget replay. It does not execute Mixtral generation.
...
- q8_abs_error_budget_mean_per_token={fmt(q8_error_budget_mean)}
- q4_abs_error_budget_mean_per_token={fmt(q4_error_budget_mean)}
- hotset_token_budget_replay_ready={budget_ready}
- checkpoint_payload_bytes_committed_to_repo=0
...
This is bounded budget replay evidence over sampled real checkpoint pages. It
is not full Mixtral checkpoint materialization, full page-hash coverage, real
Mixtral generation, near-frontier quality, production latency, or release
evidence.
"""
(run_dir / "V61AC_HOTSET_TOKEN_BUDGET_BOUNDARY.md").write_text(boundary, encoding="utf-8")
