...
CYCLES_PER_EPOCH=12
PROPOSAL_COUNT=24
PREFIX="v05_route_source_credit_evidence_quality"
KEY_COUNTS=(128)
SEEDS=(1 2)
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CYCLES_PER_EPOCH=8
  PROPOSAL_COUNT=20
  PREFIX="v05_route_source_credit_evidence_quality_smoke"
  KEY_COUNTS=(128)
  SEEDS=(1)
elif [[ "$MODE" == "full" ]]; then
  EPOCHS=20
...
printf 'scenario,arm,prior_mode,retry_tiebreak,retry_priorities,key_count,seed,qacc,fallback_recall,fallback_qacc,source_gap,noisy_slashed,source_retry_used,source_retry_success,retry_raw_selected,retry_keyshape_selected,retry_noisy_selected,retry_raw_mean,retry_keyshape_mean,retry_noisy_mean,retry_raw_rewarded,retry_keyshape_rewarded,retry_noisy_slashed,lookup_count,read_distance,routing_trigger_rate,active_jump_rate\n' >"$SUMMARY_CSV"
...
compute_metrics() {
  ...
  awk -F, '
    BEGIN {
      split("fixture_query_byte_acc route_fallback_recall route_fallback_qacc route_source_credit_gap route_source_credit_noisy_slashed_rate route_source_retry_used_rate route_source_retry_success_rate route_source_retry_raw_selected_rate route_source_retry_keyshape_selected_rate route_source_retry_noisy_selected_rate route_source_retry_raw_mean route_source_retry_keyshape_mean route_source_retry_noisy_mean route_source_retry_raw_rewarded_rate route_source_retry_keyshape_rewarded_rate route_source_retry_noisy_slashed_rate route_hint_candidate_lookup_count route_hint_value_read_distance_mean routing_trigger_rate active_jump_rate", names, " ")
    }
    NR == 1 {
...
run_case() {
  ...
  local retry_tiebreak="$7"
  local retry_priorities_csv="$8"
  local retry_priorities_label="$9"
  local retry_candidates_csv="${10}"
  ...
  echo "evidence-quality: ${scenario} prior=${prior_mode} tiebreak=${retry_tiebreak} candidates=${retry_candidates_csv}" >&2
  "$BUILD_DIR/dmv02" \
    --input "$fixture" \
    --N "$n" \
    --epochs "$EPOCHS" \
...
run_arm_set() {
  ...
  run_case source-order "$key_count" "$seed" none 1.0 0 source-order \
    "raw-key:0.0,key-shape:0.0,noisy-route-code:0.0" \
    "raw-key:0.0+key-shape:0.0+noisy-route-code:0.0" \
    raw-key,key-shape,noisy-route-code
  run_case static-keyshape-prior "$key_count" "$seed" static 1.0 0 source-prior \
    "key-shape:0.2,raw-key:0.0,noisy-route-code:-1.0" \
    "key-shape:0.2+raw-key:0.0+noisy-route-code:-1.0" \
    raw-key,key-shape,noisy-route-code
  run_case warmup-keyshape-prior "$key_count" "$seed" warmup 1.0 "$EPOCHS" source-prior \
    "key-shape:0.2,raw-key:0.0,noisy-route-code:-1.0" \
    "key-shape:0.2+raw-key:0.0+noisy-route-code:-1.0" \
    raw-key,key-shape,noisy-route-code
  run_case raw-quality-evidence "$key_count" "$seed" none 1.0 0 source-order \
    "raw-key:0.0,noisy-route-code:0.0" \
    "raw-key:0.0+noisy-route-code:0.0" \
    raw-key,noisy-route-code
  run_case keyshape-quality-evidence "$key_count" "$seed" none 1.0 0 source-order \
