...
PREFIX="v61ad_kv_weight_token_budget_replay"
RUN_ID="${V61AD_RUN_ID:-replay_001}"
RUN_DIR="$RESULTS_DIR/$PREFIX/$RUN_ID"
SUMMARY_CSV="$RESULTS_DIR/${PREFIX}_summary.csv"
DECISION_CSV="$RESULTS_DIR/${PREFIX}_decision.csv"

if [[ "${V61AD_REUSE_EXISTING:-0}" == "1" && -s "$SUMMARY_CSV" && -s "$RUN_DIR/sha256_manifest.csv" ]]; then
  echo "v61ad_kv_weight_token_budget_replay_dir: $RUN_DIR"
...
if v61ac_summary.get("v61ac_hotset_token_budget_replay_ready") != "1":
    raise SystemExit("v61ad requires v61ac_hotset_token_budget_replay_ready=1")
if v61m_summary.get("kv_cache_policy_ready") != "1":
    raise SystemExit("v61ad requires kv_cache_policy_ready=1")
if v61m_summary.get("host_ram_kv_spill_enabled") != "0":
    raise SystemExit("v61ad requires host RAM KV spill disabled")
...
for src, rel in [
    ...
    (v61ac_dir / "runtime_gap_rows.csv", "source_v61ac/runtime_gap_rows.csv"),
...
ssd_read_bytes_per_token = int(v61ac_summary["ssd_read_bytes_per_token"])
...
for token in token_rows:
    for profile in kv_profiles:
        token_ready = int(token["hotset_token_budget_replay_ready"])
        ...
        combined_rows.append(
            {
                "combined_budget_id": f"v61ad_{token['token_budget_id']}_{profile['profile_id']}",
                ...
                "token_direct_io_latency_ms_p50": token["token_direct_io_latency_ms_p50"],
                "token_direct_io_latency_ms_p95": token["token_direct_io_latency_ms_p95"],
...
metric_rows = [
    {
        "metric_id": "v61ad_kv_weight_token_budget_metrics",
...
decision_rows = [
    {"gate": "v61ac-hotset-token-budget-input", "status": "pass", "reason": "source-bound hotset token-budget replay is ready"},
    {"gate": "v61m-kv-cache-policy-input", "status": "pass", "reason": "KV policy is ready with host RAM spill disabled"},
    {"gate": "combined-kv-weight-token-budget", "status": "pass" if budget_ready else "blocked", "reason": f"{combined_ready_rows}/{combined_count} combined rows pass"},
...
summary = {
    "v61ad_kv_weight_token_budget_replay_ready": str(budget_ready),
...
manifest = {
    "artifact": "v61ad_kv_weight_token_budget_replay",
    "created_at": datetime.now(timezone.utc).isoformat(),
    "model_id": model_id,
    "run_dir": str(run_dir),
    "v61ad_kv_weight_token_budget_replay_ready": budget_ready,
...
(run_dir / "v61ad_kv_weight_token_budget_replay_manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")

boundary = f"""# v61ad KV + Weight Token Budget Replay Boundary

This artifact binds v61ac sampled hotset token-budget rows to the v61m
KV-cache residency/eviction policy. It verifies that sampled SSD weight reads
and deterministic VRAM-hot/NVMe-cold KV residency fit the local budget shape
without host RAM KV spill. It does not execute Mixtral generation.
...
This is bounded budget replay evidence over sampled real checkpoint pages and
deterministic KV policy rows. It is not full Mixtral checkpoint materialization,
full page-hash coverage, real Mixtral generation, near-frontier quality,
production latency, or release evidence.
"""
(run_dir / "V61AD_KV_WEIGHT_TOKEN_BUDGET_BOUNDARY.md").write_text(boundary, encoding="utf-8")
