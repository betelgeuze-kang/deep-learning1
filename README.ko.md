...
- 현재 브랜치 `codex/route-memory-local-energy-policy`는 h10-q remote teacher-source live-network import checkpoint와 h11-b PC RouteLM artifact verification checkpoint까지 최신입니다.
...
- v08-l은 현재 external-benchmark evidence boundary입니다. Adapter, evidence schema, supplied CSV import, comparison delta, real-evidence format, artifact hash verification, benchmark authenticity, execution-output, independent attestation, attestor identity, lower-chain HTTPS hash-attested artifact, final-review mechanics까지 닫혔습니다. 하지만 local/fixture row는 계속 non-publishable이고 `real_external_benchmark_verified=0`입니다. final-review guard는 local `file://` review artifact의 declaration flag rewrite 우회를 막고, HTTPS hash-attested review artifact는 non-local review evidence로 카운트하되 lower-chain benchmark artifact가 하나라도 local이면 publish를 막습니다. lower-chain과 final-review가 모두 remote-style HTTPS hash-attested인 fixture도 real source import verifier가 붙기 전까지 `source_import_verified=0`, action `external-benchmark-source-import-missing`에서 막습니다.
...
- 최신 검증 스택은 `bash -n experiments/*.sh`, `git diff --check`, focused h10-k/h10-l/h10-m/h10-n/h10-o/h10-p/h10-q scorer/source/content/fetch-attestation/runtime-fetcher/live-network-import/distillation tests, focused v08 lower-chain remote-artifact 및 v08-l final-review/import/real-source-guard/remote-review-guard/remote-full-source-import-guard tests, focused h11-b verifier/import tests, h11 readiness/import tests, `bash experiments/test_v09_gpu_backend_closure.sh`입니다.
...
- Real external benchmark source/result/review evidence가 v08-f부터 v08-l까지 통과하고 source/result/execution/attestation/identity artifact와 final-review artifact가 모두 non-local이며 real source-import verification path까지 있어야 v0.8 external comparison을 publish할 수 있습니다.
...
- 현재 검증 checkpoint는 h6-t/u/v/w/x/y, h10-a/b/c/d/e/f/g/h/i/j/k/l/m/n/o/p/q, h7-b가 h7 goal closure에 포함되어 있고, v08-b/v08-c/v08-d/v08-e/v08-f/v08-g/v08-h/v08-i/v08-j/v08-k/v08-l adapter/evidence/import/comparison/real-evidence/artifact-verifier/authenticity/execution/attestation/attestor-identity/final-review/readiness와 v08 lower-chain remote-artifact path, v08-l real-source/remote-review/remote-full source-import guard, h11-a prototype readiness/import, h11-b artifact verifier/import, h9-g measured-speed gate/import가 h9 quick closure에 연결된 상태입니다. HIP parity는 optional extended check로 남아 있습니다.
