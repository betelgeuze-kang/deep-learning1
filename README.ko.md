...
- 현재 브랜치 `codex/route-memory-local-energy-policy`는 h10-q remote teacher-source live-network import checkpoint, v08-z external benchmark source-acquisition checkpoint, h11-b PC RouteLM artifact verification checkpoint까지 최신입니다.
...
- v08-z가 현재 external-benchmark source-acquisition/intake boundary입니다. v08-m부터 v08-w까지는 source-import contract, live verifier/review, authoritative review, public registry, live registry query, fetch/cache, live-registry network proof, real verification, official source authority를 연결하고, v08-x는 result/leaderboard authority layer, v08-y는 publication-package layer입니다. v08-z는 RULER, LongBench, codebase retrieval, real document QA에 대해 official benchmark source landing/dataset/card/split/license/metric URI와 sha256 package를 별도 acquisition gate로 요구합니다. Supplied fixture acquisition row는 `external_benchmark_source_acquisition_review_ready=1`까지 갈 수 있지만 `external-benchmark-source-acquisition-fixture-only`로 막히고, supplied non-fixture acquisition package도 `external_benchmark_source_acquisition_ready=1`까지만 올라가며 source import/content/result/review/publication chain이 연결되기 전까지 `real_external_benchmark_verified=0`입니다.
...
- 최신 검증 스택은 현재 touched v08 script `bash -n`, v08 source-import live-registry-query/fetcher/network-proof/real-verification/official-authority focused test, v08-x result-authority focused test, v08-y publication-package focused test, v08-z source-acquisition focused test, comparison guard test, 그리고 v08-z까지 포함한 full h9 quick closure입니다.
...
- Real external benchmark source/result/review/publication evidence가 v08-f부터 v08-z까지 통과하고 source/result/execution/attestation/identity artifact와 final-review artifact가 모두 non-local이며 official source-acquisition URI/hash package, live source-import verifier evidence, independent live review, authoritative source-import review, public-registry/source-import authority evidence, non-fixture runner-owned live registry query proof, non-fixture live registry fetch/cache proof, non-fixture live registry network proof, non-placeholder official real-verification record, official authority/trust-root record, official result-authority/leaderboard record, non-fixture publication/reproducibility/license package까지 있어야 v0.8 external comparison을 publish할 수 있습니다.
...
- v08 external benchmark readiness는 promotion gate가 열릴 때까지 외부 benchmark comparison을 defer합니다. v08-b부터 v08-l까지는 adapter/evidence/import/comparison/artifact/authenticity/execution/attestation/identity/final-review mechanics를 덮고, v08-m은 `source_import_contract_ready=1`, v08-n은 `source_import_verifier_ready=1`, v08-o는 `source_import_live_verifier_ready=1`, v08-p는 `source_import_independent_live_review_ready=1`, v08-q는 `source_import_authoritative_review_ready=1`, v08-r은 `source_import_public_registry_ready=1`, v08-s는 `source_import_live_registry_query_ready=1`, v08-t는 `source_import_live_registry_fetch_ready=1`, v08-u는 `source_import_live_registry_network_proof_ready=1`, v08-v는 `source_import_real_verification_review_ready=1`, v08-w는 `source_import_official_authority_review_ready=1`, v08-x는 `external_benchmark_result_authority_review_ready=1`, v08-y는 `external_benchmark_publication_review_ready=1`, v08-z는 `external_benchmark_source_acquisition_ready=1`까지 올릴 수 있습니다. 그래도 acquisition만으로는 source import/content/result/review/publication evidence가 아니므로 `real_external_benchmark_verified=0`이며 publish claim은 계속 막힙니다. h9-e는 CPU-canonical quick boundary에서 candidate-weight/proposal-score parity scaffold가 유지되는지 확인하며, HIP parity는 환경 의존 optional extended check입니다.
...
- 현재 검증 checkpoint는 h6-t/u/v/w/x/y, h10-a/b/c/d/e/f/g/h/i/j/k/l/m/n/o/p/q, h7-b가 h7 goal closure에 포함되어 있고, v08-b부터 v08-z readiness가 h9 quick closure에 연결되어 통과합니다. HIP parity는 optional extended check로 남아 있습니다.
