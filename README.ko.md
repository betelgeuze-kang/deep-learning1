...
- h9-a/h9-b/h9-d/h9-e는 `-DDLE_ENABLE_HIP=ON`과 `--backend hip` 뒤에 optional ROCm/HIP backend scaffold를 추가합니다. CPU가 여전히 canonical/default입니다. 첫 HIP 경계는 bounded route-quality candidate-weight factor parity와 diagnostic-only 16x16 proposal-score parity이며, h9-e는 quick closure에서 extended parity boundary를 정적으로 확인합니다. KV parsing, hash/source-credit orchestration, update acceptance, RNG, age/tick/reservoir mutation, CSV는 CPU에 남습니다. 이는 backend/parity instrumentation이지 GPU acceleration proven이나 learned routing solved가 아닙니다.
