...
- 현재 route-memory checkpoint는 h6-x와 h7-b입니다. h6-t는 `utility-w0p75` adaptive guardrail을 약한 scale smoke로 확장했고, h6-u는 chunk exact / per-offset consistency / coherent wrong-key / top1-recall gap을 별도로 읽습니다. h6-v/h6-w는 source-credit retry와 chunk guardrail을 결합해 default promotion을 막고, h6-x는 단순 local scorer 변형이 plain `span-local-energy`를 넘지 못함을 보여줍니다.
- h7-b promotion gate는 `default_promotion=0`, `status=diagnostic-only`로 통과했습니다. h6-x는 `chunk_local_safe=1`, best scorer `span-local-energy`로 통과하지만, h6-u/h6-v 기준 chunk side는 아직 `chunk_ready=0`, `combined_ready=0`이고 source-credit side는 noisy retry 선택을 `0.000000`으로 유지합니다. 현재 action은 `abstain-or-weak-hint`입니다.
- v08 external benchmark readiness는 promotion gate가 열릴 때까지 외부 benchmark comparison을 defer합니다. h9-e는 CPU-canonical quick boundary에서 candidate-weight/proposal-score parity scaffold가 유지되는지 확인하며, HIP parity는 환경 의존 optional extended check입니다.
...
- h6-t/h6-u/h6-v/h6-w는 이 정책을 promotion gate로 끌고 갑니다. Smoke 기준 h6-t의 `utility-w0p75`는 `bad_accept_rate=0.000000`으로 안전하지만 promotion하지 않습니다. h6-u chunk readout은 `chunk_exact_mean=0.156250`, `coherent_wrong_key_mean=0.828125`, `top1_recall_gap_mean=0.796875`, `keyshape_gap_mean=0.734375`로 아직 chunk-quality가 부족함을 보여줍니다. h6-v는 source-credit retry에서 `policy-source-order`, `source_qacc=0.957813`, `retry_noisy_selected=0.000000`을 고르지만, h6-w는 default promotion 대신 `abstain-or-weak-hint`를 선택합니다. h6-x는 prefix/worst-offset/margin local scorer 변형을 테스트했고, plain `span-local-energy`가 여전히 최선입니다(`qacc=0.700000`, `chunk_exact=0.531250`, `coherent_wrong=0.468750`). 따라서 chunk-ranking 경계는 아직 열려 있습니다.
- h7-a는 goal closure smoke를 추가했고, h7-b는 h6-t/u/v/w/x promotion gate를 추가합니다. 현재 h7-b 결과는 `default_promotion=0`, `status=diagnostic-only`입니다. 이는 현재 route-quality/route-memory scaffold를 instrumentation으로 닫는 것이지 learned routing이나 long-context retrieval solved가 아닙니다.
- v08은 external benchmark readiness gate입니다. 현재는 `external_benchmark_ready=0`, `action=defer-external-comparison`으로 통과합니다.
...
- 현재 검증 checkpoint는 h6-t/u/v/w/x와 h7-b가 h7 goal closure에 포함되어 있고, v08 readiness와 h9-e가 h9 quick closure에 연결된 상태입니다. HIP parity는 optional extended check로 남아 있습니다.
