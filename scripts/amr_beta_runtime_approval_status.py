...
def validate_runtime_script_path(script_path: Path, benchmark_out: str) -> list[str]:
    errors: list[str] = []
    resolved_script = script_path.expanduser().resolve()
    if is_forbidden_env_path(resolved_script):
        errors.append("out_runtime_commands_sh must not be .env-like")
    benchmark_out_text = str(benchmark_out or "").strip()
    if benchmark_out_text:
        benchmark_out_path = Path(benchmark_out_text).expanduser().resolve()
        if resolved_script == benchmark_out_path or is_relative_to(resolved_script, benchmark_out_path):
            errors.append(
                "out_runtime_commands_sh must not be inside approved benchmark output: "
                f"{resolved_script} (benchmark_out: {benchmark_out_path})"
            )
    return errors
...
def output_exists_errors(paths: dict[str, Path], *, overwrite: bool) -> list[str]:
    ...
    for name, path in paths.items():
        if path.expanduser().resolve().exists():
            errors.append(f"{name} already exists; use --overwrite: {path.expanduser().resolve()}")
    return errors
    ...
    *,
    ...
    return {
        ...
        **{
        ...
        "writes_runtime_command_script": 0,
        "runtime_commands_script": "",
        "runtime_commands_script_sha256": "",
        "runtime_commands_script_command_count": 0,
        **BLOCKED_FLAGS,
        "errors": [],
    }


def runtime_command_script_text(payload: dict) -> str:
    commands = payload["runtime_commands"]
    lines = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "",
        ...
        f"# approval_record_sha256: {payload.get('approval_record_sha256', '')}",
        f"# runtime_commands_sha256: {payload.get('runtime_commands_sha256', '')}",
        f"# approved_runtime_budget_minutes: {payload.get('approved_runtime_budget_minutes', '')}",
        f"# benchmark_out: {payload.get('benchmark_out', '')}",
...
def write_runtime_command_script(path: Path, payload: dict, overwrite: bool) -> str:
    if is_forbidden_env_path(path):
        raise ValueError("refusing .env-like runtime command script output path")
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists() and not overwrite:
        raise ValueError(f"runtime command script already exists; use --overwrite: {path}")
    text = runtime_command_script_text(payload)
...
def write_markdown(path: Path, payload: dict, overwrite: bool) -> None:
    ...
    lines = [
        ...
        f"- writes_runtime_command_script: {payload['writes_runtime_command_script']}",
        f"- runtime_commands_script: {payload['runtime_commands_script']}",
        f"- runtime_commands_script_sha256: {payload['runtime_commands_script_sha256']}",
        f"- runtime_commands_script_command_count: {payload['runtime_commands_script_command_count']}",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--out-runtime-commands-sh",
        default="",
        help="Optional executable script containing the human-approved runtime commands.",
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        runtime_script_path = (
            Path(args.out_runtime_commands_sh).expanduser().resolve()
            if args.out_runtime_commands_sh
            else None
        )
        output_errors: list[str] = []
        if runtime_script_path:
            output_paths["out_runtime_commands_sh"] = runtime_script_path
            output_errors.extend(validate_runtime_script_path(runtime_script_path, benchmark_out))
        output_errors.extend(output_exists_errors(output_paths, overwrite=args.overwrite))
        if output_errors:
            if args.json:
                print(json.dumps({**status, "errors": output_errors}, indent=2, sort_keys=True))
            for error in output_errors:
                print(error, file=sys.stderr)
            return 1
        if runtime_script_path:
            status["writes_runtime_command_script"] = 1
            status["runtime_commands_script"] = str(runtime_script_path)
            status["runtime_commands_script_command_count"] = len(commands)
            status["runtime_commands_script_sha256"] = sha256_text(runtime_command_script_text(status))
        ...
        if runtime_script_path:
            script_sha = write_runtime_command_script(runtime_script_path, status, args.overwrite)
            if script_sha != status["runtime_commands_script_sha256"]:
                raise ValueError("runtime command script sha drift after write")
        if args.json:
