...
def output_file_exists_errors(paths: dict[str, Path], *, overwrite: bool) -> list[str]:
    if overwrite:
        return []
    errors: list[str] = []
...
def build_operator_commands(
    *,
    ...
    packet_parts: list[object] = [
        "python3",
        "scripts/amr_beta_maintainer_feedback_packet.py",
        "--repo-intake",
        repo_path,
    ]
    for raw_dir in label_intake_dirs:
    ...
    if args.enforce_min_maintainers:
        packet_parts.append("--enforce-min-maintainers")
    if args.require_countable_cases:
        packet_parts.append("--require-countable-cases")
    if args.skip_verify_existing:
        packet_parts.append("--skip-verify-existing")
    packet_parts.append("--overwrite")
    progress_path = out_path / "maintainer_feedback_progress_summary.json"
    return [
        command_line(packet_parts),
        command_line(["python3", "-m", "json.tool", progress_path]),
    ]


def command_script_text(summary: dict) -> str:
    commands = summary["operator_commands"]
    lines = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "",
        ...
        f"# repo_snapshot_lock_sha256: {summary.get('repo_snapshot_lock_sha256', '')}",
        f"# feedback_sha256: {summary.get('feedback_sha256', '')}",
        f"# feedback_bundle_sha256: {summary.get('feedback_bundle_sha256', '')}",
        f"# operator_commands_sha256: {summary.get('operator_commands_sha256', '')}",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--out-commands-sh",
        default="",
        help="Optional executable shell script containing the validated operator handoff commands.",
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if command_script_path:
            ...
            output_path_errors.extend(
                output_file_exists_errors(
                    {"out_commands_sh": command_script_path},
        ...
        summary = {
            ...
            **feedback_summary,
            ...
            "operator_commands_sha256": sha256_json(operator_commands) if operator_commands else "",
        ...
        if command_script_path:
            summary["writes_operator_command_script"] = 1
            summary["operator_commands_script"] = str(command_script_path)
            summary["operator_commands_script_command_count"] = len(operator_commands)
            summary["operator_commands_script_sha256"] = sha256_text(command_script_text(summary))
        if errors:
