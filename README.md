...
Current v59 one-command progress: `experiments/test_v59b_one_command_candidate_demo.sh` verifies `examples/v1_0_architecture_challenge_candidate_demo.sh`, which assembles `results/v59b_one_command_candidate_demo/candidate_001/` from v52b-v58b candidate/intake artifacts with stage rows, gate rows, README_RESULT, boundary, manifest, and sha256 manifest. This closes one-command replay for the current candidate chain, but v59 remains blocked until the replay uses real v52-v58 measured/reviewed rows.
