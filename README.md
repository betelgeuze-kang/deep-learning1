...
- v0.3 Architecture Preview is implemented as a user-facing surface over the existing evidence stack. `scripts/audit_my_repo.sh` emits `AUDIT_REPORT.md`, JSONL/CSV findings, citation spans, RouteMemory lineage, mmap read trace, compact RouteHint rows, grounded generation rows, abstain rows, resource envelope, and `reproduce.sh`. `examples/local_codebase_intelligence_box.sh` bundles the audit report, baseline comparison note, local scaling summary, architecture trace, lineage, citations, RouteHints, generations, and hashes. `experiments/test_v0_3_architecture_preview.sh` verifies `v0_3_architecture_preview_ready=1`, `one_command_repo_audit_ready=1`, `baseline_war_ready=1`, `baseline_rows=8`, `routehint_generator_mainline_ready=1`, `raw_prompt_context_bytes=0`, `attention_blocks=0`, `transformer_blocks=0`, `oracle_prediction_used=0`, `raw_input_extractor_used=0`, while keeping `gpu_speedup_claim=deferred` and `real_release_package_ready=0`.
