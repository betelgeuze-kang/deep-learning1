...
- Treat h6-x plus h7-b as the current route-memory policy checkpoint. h6-p exposes the byte-qacc versus span-exact objective split, h6-q/h6-r show fixed guardrails are useful but regime-sensitive, h6-s calibrates a utility-style adaptive guardrail, h6-t scales that guardrail as a diagnostic, h6-u moves the readout to chunk-quality metrics, h6-v/h6-w combine chunk-quality with source-credit retry gates, and h6-x shows simple local scorer transforms do not beat plain `span-local-energy`.
...
- Do not keep increasing route strength or revive topology replacement by default. The current h7-b promotion gate blocks default promotion: h6-x reports `chunk_local_safe=1` with best scorer `span-local-energy`, but h6-u/h6-v still show `chunk_ready=0`, `combined_ready=0`, coherent wrong-key remains high, and the guardrail action is `abstain-or-weak-hint`.
...
- h6-t scales the adaptive guardrail smoke over weak/harsher degradation and keeps `utility-w0p75` safe but diagnostic (`bad_accept_rate=0.000000`, no default promotion). h6-u derives chunk-quality diagnostics: smoke `chunk_exact_mean=0.156250`, `coherent_wrong_key_mean=0.828125`, `top1_recall_gap_mean=0.796875`, `keyshape_gap_mean=0.734375`. h6-v/h6-w show source-credit retry can stay noisy-clean (`policy-source-order`, `source_qacc=0.957813`, `retry_noisy_selected=0.000000`) but chunk-quality blocks promotion, so the action is `abstain-or-weak-hint`. h6-x tests prefix/worst-offset/margin local scorer variants; none beats plain `span-local-energy` (`qacc=0.700000`, `chunk_exact=0.531250`, `coherent_wrong=0.468750`), so the chunk-ranking boundary stays open.
- h7-a adds a goal closure smoke. h7-b adds a promotion gate over h6-t/u/v/w/x and keeps `default_promotion=0`, `status=diagnostic-only`. This closes the current route-quality/route-memory scaffold as instrumentation, not learned routing or long-context retrieval solved.
- v08 adds an external benchmark readiness gate. It passes by deferring external comparison (`external_benchmark_ready=0`) until the h7 promotion gate passes.
...
- Current verification checkpoint: h6-t/u/v/w/x and h7-b are wired into the h7 goal closure, v08 readiness is wired into h9 quick closure, and HIP parity remains optional/environment-dependent.
