...
This closes the v61a-v61j SSD-resident active-sparse runtime prototype and adds v61k-v61cp real-model page evidence for Mixtral 8x22B: deterministic 2 MB SSD weight pages, aligned direct I/O reads, no full-model RAM residency audit rows, RouteHint prefetch/VRAM hot cache, CPU page-dequant-matmul numeric checks, expert routing, predictive prefetch, mixed quant planning, dense full-stream stress blockers, a logical 128B MoE active-sparse contract, a one-command demo bundle, a legally redistributable real-model page manifest, a ROCm page-dequant-matmul timing row over one 2 MiB q4-equivalent page tile, a KV-cache residency/eviction policy, a source-bound QA workload seed, checkpoint index/shard HTTP/safetensors header plus sampled page-hash probe evidence, an outside-repository local SSD checkpoint residency preflight, a real safetensors-header-derived checkpoint page map, a full page-hash sweep plan, one-command source-bound QA replay, a local checkpoint materialization identity verifier, bounded remote checkpoint page-hash samples, remote-hashed page tensor/runtime-node binding, a materialization admission/resume plan, an NVMe hotset runtime replay manifest, local sampled-hotset page materialization, sampled hotset direct-I/O replay, BF16 tensor-slice verification, bounded BF16/q8/q4 tensor-tile numeric probes, bounded source-bound hotset token-budget replay, KV+weight combined token-budget replay, a real generation admission gate, a guarded checkpoint warehouse operator bundle, a checkpoint warehouse execution preflight, a download backend fallback plan, a storage budget remediation plan, a storage profile admission matrix, a warehouse target preflight, a warehouse activation gate, a post-activation verification gate, a full page-hash execution gate, a real-model page-manifest coverage audit, a MoE coverage remote-hash expansion plan, a MoE remote-hash execution gate, a MoE remote-hash result intake gate, a sampled hotset reuse admission gate, a sampled prefetch-overlap admission gate, a sampled prefetch queue-depth scheduler gate, a sampled threaded O_DIRECT async prefetch execution probe, a current-host io_uring/registered-buffer preflight, a current-host async-I/O backend selection gate, selected-backend token runtime binding, ubuntu-1 warehouse target admission, a target-bound ubuntu-1 activation handoff package, an ubuntu-1 write sentinel activation probe, ubuntu-1 sampled hotset materialization, ubuntu-1 sampled hotset direct-I/O replay, ubuntu-1 resident BF16 tensor-slice verification, ubuntu-1 resident BF16/q8/q4 tensor-tile quant probes, ubuntu-1 source-bound token-budget replay, ubuntu-1 KV+weight token-budget replay, ubuntu-1 persistent-hotset reuse admission, ubuntu-1 sampled prefetch-overlap admission, ubuntu-1 sampled prefetch queue-depth scheduler admission, ubuntu-1 sampled threaded O_DIRECT async prefetch execution, ubuntu-1 bootstrap cold-start admission, ubuntu-1 activation target admission refresh, ubuntu-1 payload execution readiness gating, ubuntu-1 payload execution launch bundling, ubuntu-1 payload execution receipt intake, ubuntu-1 post-receipt materialization promotion gating, ubuntu-1 post-receipt verification result intake, ubuntu-1 actual generation result intake, ubuntu-1 partial checkpoint materialization witness, ubuntu-1 remaining checkpoint materialization queueing, ubuntu-1 partial page-hash witnessing, ubuntu-1 page-hash coverage ledgering, ubuntu-1 remaining page-hash execution planning, ubuntu-1 remaining page-hash operator bundling, ubuntu-1 remaining page-hash result intake, ubuntu-1 full page-hash coverage promotion gating, ubuntu-1 page-hash generation admission bridging, ubuntu-1 generation unblocker closure bundling, ubuntu-1 generation closure return intake, ubuntu-1 source-bound generation execution packeting, ubuntu-1 source-bound generation operator bundling, a zero-payload real-model page-manifest release index, a real-manifest runtime substitution gate, a real-manifest immediate-target bridge, a real-manifest runtime execution admission bridge, and a complete-source runtime admission coverage gate. The v61p preflight records 59 shard download-plan/presence rows, 281241493344 checkpoint bytes required, 315601231712 bytes required with reserve, and the current 21337460736-byte SSD budget blocker. The v61q page map records 1739 real checkpoint tensor rows, 134161 unique 2 MiB checkpoint page rows, 135841 tensor/page segment rows, and zero checkpoint payload bytes included. The v61r sweep plan records 134161 page-hash task rows, 3 sampled remote page-hash probes, 6 sampled page overlaps, and 0 verified local page hashes on the current host. The v61s replay runs `./examples/v61_ssd_resident_moe_demo.sh --source-bound-qa` with exit code 0 and 37/37 source-bound query pass rows. The v61t verifier now records 1 local existing shard, 1 size-match shard, 1 header-hash match, 1 identity-verified shard, `local_identity_verified_bytes=4932529864`, 0 sampled local page-hash matches, and `local_checkpoint_materialization_ready=0`; its target-override smoke verifies that `V61T_WAREHOUSE_ROOT` is passed into the v61p shard-presence preflight. The v61u sampler reads 16 full 2 MiB checkpoint pages by HTTP Range, records 33554432 remote payload bytes read as hashes only, and keeps `full_safetensors_page_hash_binding_ready=0`. The v61v binder maps those 16 remote-hashed pages to 16 tensor/runtime-node rows, including 15 MoE expert page bindings across 15 layers and all 8 expert indices. The v61w planner emits 59 download-resume rows, 16 sampled-priority shard rows, 15 MoE-first shard rows, and `download_resume_plan_ready=1` while keeping `materialization_admission_ready=0` on the current SSD budget. The v61x manifest binds those 16 remote-hashed pages to 16 planned NVMe hotset slots and 37 source-bound replay rows, with `hotset_manifest_ready=1`, `source_bound_replay_binding_ready=1`, and zero checkpoint payload bytes downloaded or committed. The v61y verifier materializes those 16 sampled pages outside the repository, records 33554432 persisted hotset bytes, 16/16 local hash matches, 16/16 local readback hash matches, and zero checkpoint payload bytes committed to the repository. The v61z replay reads those 16 pages with O_DIRECT, records 16/16 direct-read hash matches, 33554432 direct-I/O bytes, p50/p95 read latency 0.580768/0.956690 ms, 2784.734538 MiB/s sampled throughput, and 8388608 SSD read bytes per token. The v61aa verifier interprets those local pages as real BF16 tensor segments, records 16 tensor slices, 33550832 tensor-segment bytes, 65536 sampled BF16 values, 65536 finite values, zero NaN/Inf values, and 16 slice/page hash matches. The v61ab probe runs 128 bounded dot-tile probes over 524288 BF16 values from those sampled slices, including 120 MoE tile probes and 8 embedding tile probes, with 128/128 finite baseline/q8/q4 dot rows and q8/q4 mean absolute dot errors of 0.00113809798/0.0244754219. The v61ac replay binds those hotset pages and numeric tiles to 37 source-bound workload rows, records 37 token-budget rows, 148 active page schedule rows, 1184 tile-binding rows, 8388608 SSD read bytes/token, 131072 BF16 tile values/token, and sampled token direct-I/O p50/p95 budgets of 2.323072/3.82676 ms. The v61ad replay combines those 37 source-bound token-budget rows with five KV context profiles into 185 combined KV+weight budget rows, with 185/185 resident KV policy passes, 74/185 full-KV-in-VRAM passes, 111 rows requiring NVMe cold KV eviction, zero host RAM spill bytes, 229376 KV bytes/token, 8617984 sampled weight+new-KV bytes/token, and max 8192-context KV cold tier of 1639972864 bytes. The v61ae gate binds v61ad/v53r/v61r/v61t/v61w into 1000 real-generation candidate rows, admits 0 rows, records 1000 runtime-budget-ready rows, and keeps all 1000 rows blocked by source review, materialization, and full page-hash gates. The v61af bundle emits 59 guarded download commands, 62 operator command rows, six operator bundle files, dry-run defaults for download and full page hashing, and zero checkpoint payload bytes downloaded by v61af. The v61ag preflight records 62 operator command rows, 59 download commands, 4/4 script syntax/executable passes, one guarded dry-run probe, `huggingface_cli_available=0`, `download_execution_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61ag. The v61ah backend plan records five backend candidates, three ready backends, selects `curl-resume`, emits 59 curl-resume download plan rows, verifies backend dry-run guard readiness, and keeps zero checkpoint payload bytes downloaded or committed by v61ah. The v61ai storage plan records full-budget deficit 294263770976 bytes, raw-checkpoint deficit 259904032608 bytes, safe materialization batch rows 0, and a diagnostic no-reserve top-priority batch of 4 shards / 19478756392 bytes while keeping it non-admitted by reserve policy. The v61aj storage profile matrix records 6 profile rows, current reserve admitted shard rows 0, current no-reserve diagnostic admitted shard rows 4 / 19478756392 bytes, first full-reserve profile `full-checkpoint-exact-with-reserve`, minimum additional bytes 294263770976, recommended operator free bytes 549755813888, and zero checkpoint payload bytes downloaded or committed by v61aj. The v61ak target preflight records 3 target rows, live current target free/deficit bytes, repository-local target rejection, and zero checkpoint payload bytes downloaded or committed by v61ak. The v61al activation gate records 59 activation command rows, 0 admitted activation rows, 59 blocked activation rows, `selected_target_id=none`, `selected_backend_id=curl-resume`, and zero checkpoint payload bytes downloaded or committed by v61al. The v61bn activation admission refresh records `selected_activation_target_id=ubuntu-1-write-witness-admitted`, 59/59 target activation admitted rows, 0 payload execution rows, and zero checkpoint payload bytes downloaded or committed by v61bn. The v61bo payload execution readiness gate records `payload_execution_preflight_ready=1`, 59/59 target-bound resumable curl rows, 3 priority execution chunks, 0 payload execution rows, and zero checkpoint payload bytes downloaded or committed by v61bo. The v61bw partial page-hash witness records 2353 local page-hash rows over 4932529864 bytes from the first identity-verified ubuntu-1 shard, `partial_full_shard_page_hash_ready=1`, `full_safetensors_page_hash_binding_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61bw. The v61bx page-hash coverage ledger records 2353 verified page hashes and 131808 remaining page hashes across 59 shards, `partial_page_hash_coverage_ledger_ready=1`, `full_safetensors_page_hash_binding_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61bx. The v61by remaining page-hash execution plan skips those 2353 verified page hashes and schedules the remaining 131808 page hashes into 286 guarded chunks, with `remaining_page_hash_execution_plan_ready=1`, `full_safetensors_page_hash_binding_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61by. The v61bz operator bundle mirrors those 286 remaining chunks into seven dry-run-first operator files, records `dry_run_guard_ready=1`, `remaining_page_hash_operator_bundle_ready=1`, `page_hash_execution_ready=0`, `full_safetensors_page_hash_binding_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61bz. The v61ca result intake defines the hash-only return surface for those 131808 remaining page hashes, records accepted result rows 0, missing result rows 131808, existing verified page hash rows 2353, `full_safetensors_page_hash_binding_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61ca. The v61cb promotion gate aggregates v61ca into 59 shard-level rows, records one ready page-hash shard, 58 blocked shards, total verified page hash rows 2353/134161, `full_safetensors_page_hash_binding_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61cb. The v61cc bridge binds v61cb/v53t/v61bt into 1000 complete-source generation admission rows, records `generation_execution_admitted_rows=0`, `page_hash_blocked_rows=1000`, `review_return_blocked_rows=1000`, `generation_result_artifact_blocked_rows=1000`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61cc. The v61cd closure bundle records 3 closure phases, 11 required return artifacts, 7 operator command rows, `page_hash_return_required_rows=131808`, `human_review_required_rows=7000`, `adjudication_required_rows=1000`, `generation_result_required_artifacts=5`, `generation_unblocker_closure_ready=0`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61cd. The v61ce closure return intake rechecks v61cd/v61cb/v53t/v61bt/v61cc as three closure gates and 1000 generation admission rows, records `generation_closure_return_intake_ready=0`, `generation_execution_admitted_rows=0`, `page_hash_blocked_rows=1000`, `review_return_blocked_rows=1000`, `generation_result_artifact_blocked_rows=1000`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61ce. The v61cf execution packet binds 1000 source-bound generation packet rows, 4 prompt contract rows, 5 return artifact rows, and 6 operator command rows, records `generation_execution_ready=0`, `blocked_execution_rows=1000`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61cf. The v61cg operator bundle wraps that packet into 4 operator bundle files and 4 bundle command rows, verifies packet shape with an executable bundle verifier, records `operator_bundle_handoff_ready=1`, `generation_operator_execution_ready=0`, `actual_model_generation_ready=0`, and zero checkpoint payload bytes downloaded or committed by v61cg. The v61ch release index binds v61ao/v61cb/v61cg into 8 source artifact rows and 10 zero-payload release index files, records `redistributable_manifest_index_ready=1`, 134161 manifest pages, 135841 tensor/page segment rows, 1344/1344 MoE coverage rows, 2353/134161 verified page hashes, 131808 remaining page hashes, `actual_model_generation_ready=0`, and zero redistributed/downloaded/committed checkpoint payload bytes. The v61ci substitution gate binds v61j/v61k/v61ch into 4 logical-fixture replacement contract rows and 5 runtime substitution binding rows, records `logical_fixture_replaced_by_real_manifest_ready=1`, `zero_payload_runtime_input_ready=1`, `runtime_execution_admission_ready=0`, `actual_model_generation_ready=0`, and zero redistributed/downloaded/committed checkpoint payload bytes. It keeps full checkpoint materialization, explicit remaining page-hash execution, completed full page-hash coverage, complete-source human review return, real Mixtral generation over complete-source A-H QA, end-to-end GPU speedup, long-context quality, near-frontier quality, dense hundreds-B local-speed, production-latency, and release claims blocked.

The v61cj immediate target bridge binds v61ci/v61l/v61m/v61s into 4/4 ready immediate target rows and 3/3 ready runtime bridge rows, records real-manifest fixture replacement ready, ROCm page-kernel timing avg 0.513442 ms, KV policy ready, source-bound QA command pass 37/37, completed full page-hash coverage 0, complete-source 1000-query 0, actual generation 0, and zero checkpoint payload bytes.

The v61ck real generation unblocker operator matrix binds the current real Mixtral evidence to the remaining execution/return blockers: 59 checkpoint materialization promotion rows with 1 ready and 58 blocked shards, 286 page-hash execution chunks with 0 admitted and 286 materialization-blocked chunks, 37 real-manifest runtime execution admission candidates with 0 admitted, 131808 remaining page-hash rows, 7000 human review rows, 1000 adjudication rows, and 5 generation result artifacts. It records a 6-row unblocker matrix, 9 execution-order rows, `generation_unblocker_operator_matrix_ready=1`, keeps accepted return rows/artifacts at 0 where external returns are required, and keeps actual generation, production latency, near-frontier, and release claims blocked.
The v61cl remaining checkpoint materialization return intake consumes the v61bv 58-shard queue, defines a metadata-only return schema, preserves the one existing identity-verified shard, records `accepted_remaining_materialization_return_rows=0`, `missing_remaining_materialization_return_rows=58`, `full_checkpoint_materialization_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cl=0`, and zero checkpoint payload bytes committed to the repo while keeping full materialization, full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61cm full checkpoint materialization promotion gate consumes v61cl, aggregates 59 shard-level materialization promotion rows, records 1 ready identity-verified shard, 58 blocked shards, `promotion_missing_materialization_bytes=276308963480`, `full_checkpoint_materialization_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cm=0`, and zero checkpoint payload bytes committed to the repo while keeping full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61cn page-hash execution materialization admission gate consumes v61bz and v61cm, binds 286 remaining page-hash execution chunks to materialization promotion rows, records `admitted_page_hash_execution_chunk_rows=0`, `materialization_blocked_page_hash_execution_chunk_rows=286`, `blocked_page_hash_rows=131808`, `blocked_page_hash_bytes=276308963480`, `page_hash_execution_admission_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cn=0`, and zero checkpoint payload bytes committed to the repo while keeping page-hash execution, full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61co real manifest runtime execution admission bridge consumes v61cj/v61ci/v61cm/v61cn/v61n/v61s, maps 37 source-bound QA seed rows onto real-manifest runtime prerequisites, records `runtime_execution_candidate_rows=37`, `runtime_execution_admitted_rows=0`, `materialization_blocked_runtime_rows=37`, `page_hash_admission_blocked_runtime_rows=37`, `real_manifest_runtime_execution_admission_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61co=0`, and zero checkpoint payload bytes committed to the repo while keeping real runtime execution, actual generation, production-latency, near-frontier, and release claims blocked.
...
The v61bu ubuntu-1 partial checkpoint materialization witness consumes v61bq and v61t, records 1 live size-match shard, 1 header-hash-matched shard, 1 identity-verified shard, `local_identity_verified_bytes=4932529864`, `partial_checkpoint_materialization_witness_ready=1`, `full_checkpoint_materialization_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61bu=0`, and zero checkpoint payload bytes committed to the repo while keeping receipt-backed full materialization, full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61bv ubuntu-1 remaining checkpoint materialization queue consumes v61bp and v61bu, skips the identity-verified shard, records 58 remaining queue rows across three dry-run-first chunks, `remaining_unverified_bytes=276308963480`, `remaining_bytes_fit_current_free_space=1`, `dry_run_guard_ready=1`, `checkpoint_payload_bytes_downloaded_by_v61bv=0`, and zero checkpoint payload bytes committed to the repo while keeping explicit payload execution, receipt-backed full materialization, full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61cl ubuntu-1 remaining checkpoint materialization return intake consumes v61bv, defines the metadata-only return surface for those 58 remaining shard materialization receipts, records `accepted_remaining_materialization_return_rows=0`, `missing_remaining_materialization_return_rows=58`, `expected_remaining_materialization_bytes=276308963480`, `full_checkpoint_materialization_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cl=0`, and zero checkpoint payload bytes committed to the repo while keeping completed full checkpoint materialization, full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61cm ubuntu-1 full checkpoint materialization promotion gate consumes v61cl, promotes the return-intake state into 59 shard-level decision rows, records `ready_checkpoint_materialization_shard_rows=1`, `blocked_checkpoint_materialization_shard_rows=58`, `total_identity_verified_checkpoint_shard_rows=1`, `full_checkpoint_materialization_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cm=0`, and zero checkpoint payload bytes committed to the repo while keeping full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61cn ubuntu-1 page-hash execution materialization admission gate consumes v61bz/v61cm, admits 0/286 remaining page-hash execution chunks because the 58 required materialization returns are still missing, records `blocked_page_hash_rows=131808`, `blocked_page_hash_bytes=276308963480`, `page_hash_execution_admission_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cn=0`, and zero checkpoint payload bytes committed to the repo while keeping page-hash execution and completed full page-hash coverage blocked.
The v61co real-manifest runtime execution admission bridge converts the 37 source-bound QA seed rows into runtime admission candidates and keeps all 37 blocked by incomplete full checkpoint materialization and page-hash admission, with `checkpoint_payload_bytes_downloaded_by_v61co=0` and repo payload bytes still at zero.
...
The v61bw ubuntu-1 partial page-hash witness consumes v61bu and v61q, reads the first identity-verified shard into 2353 local page-hash rows over 4932529864 bytes, records `partial_full_shard_page_hash_ready=1`, `full_safetensors_page_hash_binding_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61bw=0`, and zero checkpoint payload bytes committed to the repo while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61bx ubuntu-1 page-hash coverage ledger consumes v61bw/v61bv/v61q, promotes the 2353 verified page hashes into a 59-shard ledger, records `remaining_page_hash_rows=131808`, `remaining_page_hash_bytes=276308963480`, `partial_page_hash_coverage_ledger_ready=1`, `full_safetensors_page_hash_binding_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61bx=0`, and zero checkpoint payload bytes committed to the repo while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61by ubuntu-1 remaining page-hash execution plan consumes v61bx/v61bv, skips the 2353 verified page hashes, schedules the remaining 131808 page hashes into 286 guarded chunks, records `remaining_page_hash_execution_plan_ready=1`, `full_safetensors_page_hash_binding_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61by=0`, and zero checkpoint payload bytes committed to the repo while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61bz ubuntu-1 remaining page-hash operator bundle consumes v61by, mirrors those 286 chunks into seven dry-run-first operator files with execute, approval, and identity-confirmation guards, records `dry_run_guard_ready=1`, `remaining_page_hash_operator_bundle_ready=1`, `page_hash_execution_ready=0`, `full_safetensors_page_hash_binding_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61bz=0`, and zero checkpoint payload bytes committed to the repo while keeping explicit remaining page-hash execution, completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61ca ubuntu-1 remaining page-hash result intake consumes v61bz, defines the hash-only return surface for the 131808 remaining page hashes, records `accepted_remaining_page_hash_result_rows=0`, `missing_remaining_page_hash_result_rows=131808`, `existing_verified_page_hash_rows=2353`, `total_verified_page_hash_rows=2353`, `full_safetensors_page_hash_binding_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61ca=0`, and zero checkpoint payload bytes committed to the repo while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61cb ubuntu-1 full page-hash coverage promotion gate consumes v61ca, aggregates 59 shard-level promotion rows, records one ready shard, 58 blocked shards, total verified page hash rows 2353/134161, `full_page_hash_coverage_promotion_ready=0`, `full_safetensors_page_hash_binding_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cb=0`, and zero checkpoint payload bytes committed to the repo while keeping completed full page-hash coverage, actual generation, production-latency, near-frontier, and release claims blocked.
The v61cc ubuntu-1 page-hash generation admission bridge consumes v61cb, v53t, and v61bt, emits 1000 complete-source generation admission bridge rows, records `generation_execution_admitted_rows=0`, `page_hash_blocked_rows=1000`, `review_return_blocked_rows=1000`, `generation_result_artifact_blocked_rows=1000`, `actual_model_generation_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cc=0`, and zero checkpoint payload bytes committed to the repo while keeping completed full page-hash coverage, complete-source review return, actual generation, production-latency, near-frontier, and release claims blocked.
