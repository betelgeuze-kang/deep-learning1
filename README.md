...
- Branch `codex/route-memory-local-energy-policy` is current through the h10-q remote teacher-source live-network import checkpoint and the h11-b PC RouteLM artifact verification checkpoint.
...
- v08-l is closed as the current external-benchmark evidence boundary. Adapter, evidence schema, supplied CSV import, comparison deltas, real-evidence format, artifact hash verification, benchmark authenticity, execution-output, independent attestation, attestor identity, lower-chain HTTPS hash-attested artifacts, and final-review mechanics are all covered. Local/fixture rows remain non-publishable with `real_external_benchmark_verified=0`; the final-review guards block local `file://` review artifacts after declaration rewrites, allow HTTPS hash-attested review artifacts to be counted as non-local review evidence, block publication when any lower-chain benchmark artifact is local, and now also block fully remote-style lower-chain plus final-review fixtures at `source_import_verified=0` with `external-benchmark-source-import-missing` until a real benchmark source import verifier exists.
...
- Latest verified command stack: `bash -n experiments/*.sh`, `git diff --check`, focused h10-k/h10-l/h10-m/h10-n/h10-o/h10-p/h10-q scorer/source/content/fetch-attestation/runtime-fetcher/live-network-import/distillation tests, focused v08 lower-chain remote-artifact and v08-l final-review/import/real-source-guard/remote-review-guard/remote-full-source-import-guard tests, focused h11-b verifier/import tests, h11 readiness/import tests, and `bash experiments/test_v09_gpu_backend_closure.sh`.
...
- Real external benchmark source/result/review evidence must pass the v08-f through v08-l chain with non-local source/result/execution/attestation/identity artifacts, non-local final-review artifacts, and a real source-import verification path before any v0.8 external comparison can be published.
...
- Current verification checkpoint: h6-t/u/v/w/x/y, h10-a/b/c/d/e/f/g/h/i/j/k/l/m/n/o/p/q, and h7-b are wired into the h7 goal closure, v08-b/v08-c/v08-d/v08-e/v08-f/v08-g/v08-h/v08-i/v08-j/v08-k/v08-l adapter/evidence/import/comparison/real-evidence/artifact-verifier/authenticity/execution/attestation/attestor-identity/final-review/readiness plus the v08 lower-chain remote-artifact path and v08-l real-source/remote-review/remote-full source-import guards, h11-a readiness/import, h11-b artifact verifier/import, and h9-g measured-speed gate/import are wired into h9 quick closure, and HIP parity remains optional/environment-dependent.
