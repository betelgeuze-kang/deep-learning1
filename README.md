...
This closes the v61a-v61j SSD-resident active-sparse runtime prototype and adds v61k-v61cr real-model page evidence for Mixtral 8x22B: deterministic 2 MB SSD weight pages, aligned direct I/O reads, no full-model RAM residency audit rows, RouteHint prefetch/VRAM hot cache, CPU page-dequant-matmul numeric checks, expert routing, predictive prefetch, mixed quant planning, a logical 128B MoE active-sparse contract, a legally redistributable real-model page manifest, ROCm page-dequant-matmul timing, KV-cache residency/eviction policy, source-bound QA workload replay, checkpoint index/shard/header/page-map evidence, sampled local hotset materialization, direct-I/O replay, BF16 tensor-slice verification, q8/q4 numeric probes, source-bound and KV+weight token-budget replay, ubuntu-1 checkpoint warehouse admission/receipt/page-hash gates, real-manifest runtime execution admission, complete-source runtime admission coverage gating, complete-source runtime admission expansion packeting, and complete-source runtime admission return intake. It keeps full checkpoint materialization, completed full safetensors page-hash coverage, complete-source human review return, complete-source runtime admission execution, actual Mixtral generation, production latency, near-frontier quality, and release claims blocked.
...
The v61ck real generation unblocker operator matrix binds the current real Mixtral evidence to the remaining execution/return blockers: 59 checkpoint materialization promotion rows with 1 ready and 58 blocked shards, 286 page-hash execution chunks with 0 admitted and 286 materialization-blocked chunks, 37 real-manifest runtime execution admission candidates with 0 admitted, 1000 complete-source runtime admission return rows with 0 accepted, 131808 remaining page-hash rows, 7000 human review rows, 1000 adjudication rows, and 5 generation result artifacts. It records a 7-row unblocker matrix, 11 execution-order rows, `generation_unblocker_operator_matrix_ready=1`, keeps accepted return rows/artifacts at 0 where external returns are required, and keeps actual generation, production latency, near-frontier, and release claims blocked.
...
The v61cr complete-source runtime admission return intake consumes v61cq and defines the five-artifact runtime admission return surface. It records `expected_runtime_admission_return_artifacts=5`, `accepted_runtime_admission_return_artifacts=0`, `missing_runtime_admission_return_artifacts=5`, `expected_runtime_admission_result_rows=1000`, `accepted_runtime_admission_result_rows=0`, `complete_source_runtime_admission_execution_ready=0`, `checkpoint_payload_bytes_downloaded_by_v61cr=0`, and zero checkpoint payload bytes committed to the repo while keeping runtime admission execution, actual generation, production-latency, near-frontier, and release claims blocked.
...
The v61cr complete-source runtime admission return intake defines that return side and keeps all five runtime admission artifacts missing by default.
