...
- Next stage: do not add more internal packaging unless a real return exposes a verifier gap. Research-first is v41 RULER NIAH row-count scale-up at fixed 4096 context length, then v42 context length expansion, then v43 LongBench v2 small slice; commercial-first is product-manual QA or incident-log QA under the same local evidence-bound QA/audit claim.
