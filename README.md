...
- v08-l is closed as the current external-benchmark evidence boundary. Adapter, evidence schema, supplied CSV import, comparison deltas, real-evidence format, artifact hash verification, benchmark authenticity, execution-output, independent attestation, attestor identity, lower-chain HTTPS hash-attested artifacts, and final-review mechanics are all covered. Local/fixture rows remain non-publishable with `real_external_benchmark_verified=0`; the final-review guards block local `file://` review artifacts after declaration rewrites, allow HTTPS hash-attested review artifacts to be counted as non-local review evidence, and still block publication when any lower-chain benchmark artifact is local.
...
- Real external benchmark source/result/review evidence must pass the v08-f through v08-l chain with non-local source/result/execution/attestation/identity artifacts plus non-local final-review artifacts before any v0.8 external comparison can be published.
