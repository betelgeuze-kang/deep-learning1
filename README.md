...
- Branch `codex/route-memory-local-energy-policy` is current through the h10-q remote teacher-source live-network import checkpoint, v08-q external benchmark source-import authoritative-review checkpoint, and the h11-b PC RouteLM artifact verification checkpoint.
...
- h7 route-memory closure is current through h10-q. The closure still keeps `default_promotion=0`, `status=diagnostic-only`, `routing_trigger_rate=0`, and `active_jump_rate=0`. The positive chunk-credit and learned local scorer results are therefore guarded diagnostic route-memory policies, not default sparse-routing policies.
- v08-q is now the current external-benchmark source-import boundary. v08-m binds non-local lower-chain source/result/execution artifacts to live-network import rows, import manifest/fetch-log/reviewer artifacts, real/non-fixture declarations, and independent source-import review, reaching `source_import_contract_ready=1`. v08-n adds runner-owned verifier/fetch-evidence replay rows that bind back to every source-import ID, manifest/fetch-log/reviewer hash, benchmark artifact URI, and verifier binary/stdout/stderr hash. v08-o separates replay from live verifier evidence. v08-p adds the independent live-review layer over those verifier rows. v08-q adds an authoritative source-import review layer: supplied non-local, hash-attested authority rows can bind source-import IDs, verifier run IDs, live-review IDs, live-review artifact hashes, verifier hashes, reviewer registry, identity, and conflict-disclosure evidence, reaching `source_import_authoritative_review_ready=1`, but still keeps `source_import_verified=0` and `real_external_benchmark_verified=0` with `external-benchmark-source-import-real-public-registry-missing` until real public registry/source-import authority evidence replaces fixtures.
...
- Latest verified command stack: `bash -n experiments/*.sh`, `git diff --check`, focused h10-k/h10-l/h10-m/h10-n/h10-o/h10-p/h10-q scorer/source/content/fetch-attestation/runtime-fetcher/live-network-import/distillation tests, focused v08 source-import/default/remote-contract/verifier-replay/live-verifier/live-review/authoritative-review tests plus lower-chain remote-artifact and v08-l final-review/import/real-source-guard/remote-review-guard/remote-full-source-import-guard tests, focused h11-b verifier/import tests, h11 readiness/import tests, and `bash experiments/test_v09_gpu_backend_closure.sh`.

Current open blockers:

- Real external teacher-label source evidence must pass the h10-j verifier, h10-m acquisition contract, h10-n content-cache verifier, h10-o fetch-attestation contract, h10-p runtime-fetcher contract, and h10-q live-network import gate, then connect a real non-fixture source import/review chain before teacher-distilled chunk retrieval can be claimed.
- Real external benchmark source/result/review evidence must pass the v08-f through v08-q chain with non-local source/result/execution/attestation/identity artifacts, non-local final-review artifacts, live source-import verifier evidence, independent live review, authoritative source-import review, and real public registry/source-import authority evidence before any v0.8 external comparison can be published.
...
- Treat h10-a through h10-q, h7-b, v08-b/v08-c/v08-d/v08-e/v08-f/v08-g/v08-h/v08-i/v08-j/v08-k/v08-l/v08-m/v08-n/v08-o/v08-p/v08-q, and h11-a/h11-b as the current route-memory policy checkpoint. h6-p exposes the byte-qacc versus span-exact objective split, h6-t/u/v/w keep promotion guarded, h6-x/y show simple local transforms and direct learned-code similarity are not enough, h10-a adds a teacher-free chunk-credit ranker, h10-d forces the fallback/retry path to exercise without selecting noisy sources, h10-e defines the teacher-label contract, h10-f collects local teacher-label harness supervision, h10-g fits a local distilled-rule learner over those labels, h10-h defines the external teacher-label ingestion schema while keeping the default source blocked, h10-i adds a supplied external teacher-label CSV import path, h10-j verifies teacher source/provenance/license/hash chains while keeping any local `file://` fixture non-real, h10-k adds a local linear learned chunk-quality scorer over the teacher-label features while keeping external source and default promotion blocked, h10-l binds learned scorer readiness to source-verified feature labels only when those feature rows match external teacher-label rows by `teacher_id/query_key/candidate_key/teacher_label/source_uri/provenance_hash`, h10-m opens the HTTPS remote teacher-source acquisition contract, h10-n verifies a supplied content cache against that remote hash manifest, h10-o verifies a supplied artifact-level fetch-attestation contract, h10-p adds a runner-owned runtime-fetcher replay contract, and h10-q separates live-network runtime evidence from real source import while still requiring non-fixture source review before real source claims. v08-b defines the external benchmark adapter manifest, v08-c defines the external benchmark evidence schema, v08-d adds a supplied-CSV import path, v08-e computes baseline-vs-route-memory deltas while requiring v08-l/v08-m/v08-n/v08-o/v08-p/v08-q real verification before publish, v08-f separates placeholder supplied evidence from real benchmark evidence, v08-g verifies benchmark source/result artifact hashes, v08-h verifies benchmark authenticity/evaluator contracts, v08-i verifies execution/evaluator-output artifacts, v08-j adds independent attestation checks over execution hashes and metrics, v08-k verifies attestor identity/provenance, v08-l verifies final-review mechanics and blocks local/remote-style review bypasses, v08-m verifies source-import contract mechanics, v08-n verifies runner-owned source-import verifier replay mechanics, v08-o verifies live verifier evidence mechanics, v08-p verifies independent live-review mechanics, and v08-q verifies authoritative source-import review mechanics while keeping `source_import_verified=0`; h11-a defines the PC RouteLM / NLG prototype readiness contract, and h11-b verifies prototype artifact/provenance hash chains while keeping local fixtures non-real.
...
- h6-t scales the adaptive guardrail smoke over weak/harsher degradation and keeps `utility-w0p75` safe but diagnostic (`bad_accept_rate=0.000000`, no default promotion). h6-u derives chunk-quality diagnostics: smoke `chunk_exact_mean=0.156250`, `coherent_wrong_key_mean=0.828125`, `top1_recall_gap_mean=0.796875`, `keyshape_gap_mean=0.734375`. h6-v/h6-w show source-credit retry can stay noisy-clean but chunk-quality blocks promotion. h6-x/y show plain `span-local-energy` remains better than local transform and route-code similarity probes. h10-a adds `span-chunk-credit` and `span-local-energy-chunk-credit`; in smoke it reaches `qacc=1.000000`, `chunk_exact=1.000000`, `coherent_wrong=0.000000`, and in the 32/64-key scale guard it keeps `chunk_exact=0.960938`, `coherent_wrong=0.000000`, `keyshape_chunk_gap=0.000000`. h10-c/d/e/f/g/h/i/j/k/l/m/n/o/p/q add joint noisy, fallback/retry, teacher-label contract, local teacher-label collection, local teacher-distillation learner, external ingestion schema, supplied-label import, real teacher-source verification, local learned chunk-quality scorer, row-bound source-verified scorer binding, remote teacher-source acquisition-contract, content-cache verifier, live-fetch attestation, runtime-fetcher, and live-network import gates. Default no-env ingestion remains blocked with `teacher_external_label_source_ready=0`; relabeled local rows without provenance and mismatched external label rows are rejected, any local `file://` source fixture remains non-real, h10-q live-network evidence still stops at missing real source import, and h10-l stays blocked with `source_verified_learned_chunk_scorer_ready=0`, `distillation_ready=0`, `status=diagnostic-only`, with `default_promotion=0`.
- h7-a adds a goal closure smoke. h7-b adds a promotion gate over h6-t/u/v/w/x/y and keeps `default_promotion=0`, `status=diagnostic-only`; h10-a/b/c/d/e/f/g/h/i/j/k/l/m/n/o/p/q are wired into the route-memory closure as later chunk-ranking/source/fallback/teacher-label/scorer/source-acquisition/content/fetch-attestation/runtime-fetcher/live-network-import smokes, not yet as a default promotion path.
- v08 adds an external benchmark readiness gate. v08-b adds an external benchmark adapter manifest for RULER, LongBench, codebase retrieval, and real document QA (`benchmark_adapter_ready=1`, `benchmark_families=4`), v08-c adds the evidence-ingestion schema (`benchmark_evidence_schema_ready=1`), v08-d can import a supplied `V08_EXTERNAL_BENCHMARK_EVIDENCE_CSV`, v08-e computes comparison deltas, v08-f blocks placeholder supplied evidence from being treated as real benchmark evidence, v08-g verifies source/result artifact hashes, v08-h verifies benchmark authenticity/evaluator contracts, v08-i verifies execution/evaluator-output artifacts, v08-j requires independent attestation of execution hashes and metric values, v08-k verifies attestor identity/provenance/conflict-disclosure artifacts, v08-l verifies final review reports against source/provenance, execution hashes, metrics, attestation IDs, reviewer identity, and conflict-disclosure artifacts, v08-m adds the source-import contract gate, v08-n adds runner-owned source-import verifier replay, v08-o adds live verifier evidence separation, v08-p adds independent live-review evidence, and v08-q adds authoritative source-import review evidence. Source/result/baseline/license evidence is still absent in the default run, so readiness still defers external comparison (`external_benchmark_ready=0`, `action=defer-external-comparison`) until promotion and real evidence exist. The supplied fixture comparison is diagnostic-only and unpublished before promotion; v08-f marks the same placeholder fixture `fixture-evidence-not-real-benchmark`, v08-g/i/j/k now separate local from HTTPS hash-attested non-local artifact mechanics, v08-h leaves authenticity/evaluator fixtures blocked on actual benchmark execution, v08-i leaves execution fixtures blocked on attestation, v08-j keeps local/fixture attestations diagnostic, v08-k verifies local and remote-style identity mechanics only, v08-l keeps final-review fixtures blocked with `real_external_benchmark_verified=0`, v08-m can raise `source_import_contract_ready=1`, v08-n can raise `source_import_verifier_ready=1`, v08-o can raise `source_import_live_verifier_ready=1`, v08-p can raise `source_import_independent_live_review_ready=1`, and v08-q can raise `source_import_authoritative_review_ready=1` while still leaving `source_import_verified=0`.
...
- Current verification checkpoint: h6-t/u/v/w/x/y, h10-a/b/c/d/e/f/g/h/i/j/k/l/m/n/o/p/q, and h7-b are wired into the h7 goal closure, v08-b/v08-c/v08-d/v08-e/v08-f/v08-g/v08-h/v08-i/v08-j/v08-k/v08-l/v08-m/v08-n/v08-o/v08-p/v08-q adapter/evidence/import/comparison/real-evidence/artifact-verifier/authenticity/execution/attestation/attestor-identity/final-review/source-import/source-import-verifier/live-verifier/live-review/authoritative-review readiness plus the v08 lower-chain remote-artifact path and v08-l/v08-m/v08-n/v08-o/v08-p/v08-q real-source/remote-review/remote-full source-import guards, h11-a readiness/import, h11-b artifact verifier/import, and h9-g measured-speed gate/import are wired into h9 quick closure, and HIP parity remains optional/environment-dependent.
