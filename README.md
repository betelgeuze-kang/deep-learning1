...
- Branch `codex/route-memory-local-energy-policy` is current through the h10-q remote teacher-source live-network import checkpoint, v08-z external benchmark source-acquisition checkpoint, and the h11-b PC RouteLM artifact verification checkpoint.
...
- v08-z is now the current external-benchmark source-acquisition/intake boundary. v08-m through v08-w carry source-import from contract, live verifier/review, authoritative review, public registry, live registry query, fetch/cache, live-registry network proof, real verification, and official source authority; v08-x adds the result/leaderboard authority layer; v08-y adds the publication-package layer; and v08-z separately requires an official benchmark source acquisition package for RULER, LongBench, codebase retrieval, and real document QA. Supplied fixture acquisition rows can reach `external_benchmark_source_acquisition_review_ready=1` but remain blocked with `external-benchmark-source-acquisition-fixture-only`; a supplied non-fixture acquisition package can reach `external_benchmark_source_acquisition_ready=1` but still keeps `real_external_benchmark_verified=0` until source import/content/result/review/publication evidence is connected.
...
- Latest verified command stack: `bash -n` for the touched v08 scripts, focused v08 source-import live-registry-query/fetcher/network-proof/real-verification/official-authority, v08-x result-authority, v08-y publication-package, v08-z source-acquisition, comparison guard tests, and the full h9 quick closure through v08-z.
...
- Real external benchmark source/result/review/publication evidence must pass the v08-f through v08-z chain with non-local source/result/execution/attestation/identity artifacts, non-local final-review artifacts, official source-acquisition URI/hash packages, live source-import verifier evidence, independent live review, authoritative source-import review, public-registry/source-import authority evidence, non-fixture runner-owned live registry query proof, non-fixture live registry fetch/cache proof, non-fixture live registry network proof, non-placeholder official real-verification records, official authority/trust-root records, official result-authority/leaderboard records, and a non-fixture publication/reproducibility/license package before any v0.8 external comparison can be published.
...
- v08 adds an external benchmark readiness gate. v08-b through v08-l cover adapter/evidence/import/comparison/artifact/authenticity/execution/attestation/identity/final-review mechanics. v08-m can raise `source_import_contract_ready=1`, v08-n can raise `source_import_verifier_ready=1`, v08-o can raise `source_import_live_verifier_ready=1`, v08-p can raise `source_import_independent_live_review_ready=1`, v08-q can raise `source_import_authoritative_review_ready=1`, v08-r can raise `source_import_public_registry_ready=1`, v08-s can raise `source_import_live_registry_query_ready=1`, v08-t can raise `source_import_live_registry_fetch_ready=1`, v08-u can raise `source_import_live_registry_network_proof_ready=1`, v08-v can raise `source_import_real_verification_review_ready=1`, v08-w can raise `source_import_official_authority_review_ready=1`, v08-x can raise `external_benchmark_result_authority_review_ready=1`, v08-y can raise `external_benchmark_publication_review_ready=1`, and v08-z can raise `external_benchmark_source_acquisition_ready=1` while still leaving `real_external_benchmark_verified=0` until acquired sources are imported, verified, reviewed, and bound to publishable results. Source/result/baseline/license evidence is still absent in the default run, so readiness still defers external comparison (`external_benchmark_ready=0`, `action=defer-external-comparison`) until promotion and real evidence exist.
...
- Current verification checkpoint: h6-t/u/v/w/x/y, h10-a/b/c/d/e/f/g/h/i/j/k/l/m/n/o/p/q, and h7-b are wired into the h7 goal closure; v08-b through v08-z readiness is wired into and passing h9 quick closure, and HIP parity remains optional/environment-dependent.
