...
- v08-l is closed as the current external-benchmark evidence boundary. Adapter, evidence schema, supplied CSV import, comparison deltas, real-evidence format, local artifact hash verification, benchmark authenticity, execution-output, independent attestation, attestor identity, and final-review mechanics are all covered. Local/fixture rows remain non-publishable with `real_external_benchmark_verified=0`; the final-review real-source guard now also blocks local `file://` review artifacts even if their real/non-fixture declaration flags are rewritten. v08-e publishability requires v08-l real verification.
...
- Latest verified command stack: `bash -n experiments/*.sh`, `git diff --check`, focused h10-k/h10-l/h10-m/h10-n/h10-o/h10-p/h10-q scorer/source/content/fetch-attestation/runtime-fetcher/live-network-import/distillation tests, focused v08-l final-review/import/real-source-guard tests, focused h11-b verifier/import tests, h11 readiness/import tests, and `bash experiments/test_v09_gpu_backend_closure.sh`.
...
- Real external benchmark source/result/review evidence must pass the v08-f through v08-l chain with non-local final-review artifacts before any v0.8 external comparison can be published.
