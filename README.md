...
This closes the v61a-v61j SSD-resident active-sparse runtime prototype and adds v61k-v61o real-model page evidence for Mixtral 8x22B: deterministic 2 MB SSD weight pages, aligned direct I/O reads, no full-model RAM residency audit rows, RouteHint prefetch/VRAM hot cache, CPU page-dequant-matmul numeric checks, expert routing, predictive prefetch, mixed quant planning, dense full-stream stress blockers, a logical 128B MoE active-sparse contract, a one-command demo bundle, a legally redistributable real-model page manifest, a ROCm page-dequant-matmul timing row over one 2 MiB q4-equivalent page tile, a KV-cache residency/eviction policy, a source-bound QA workload seed, and checkpoint index/shard HTTP/safetensors header plus sampled page-hash probe evidence. It keeps full checkpoint materialization, full page-hash coverage, real Mixtral generation, complete-source 1000+ QA, end-to-end GPU speedup, long-context quality, near-frontier quality, dense hundreds-B local-speed, production-latency, and release claims blocked.
