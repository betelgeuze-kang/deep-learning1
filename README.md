...
The v61gj post-v61gi operator input receiver accepts an optional final `V61GJ_OPERATOR_INPUT_ROOT`, preflights the 12 final files, and only admits assembly when a repo-external `V61GJ_OUTPUT_ROOT` is supplied. Canonical no-input records supplied root 0, preflight-ready rows 0/12, assembly admitted/executed 0, assembled roots 0/2, real review/generation rows 0, `authority_bound_replay_admission_ready=0`, `actual_model_generation_ready=0`, and zero repo checkpoint payload; its template-tree smoke proves scaffold templates cannot pass as final input.
...
The v61gj receiver is the handoff point for real files: once the 12 final inputs are supplied, it preflights them, runs the v61gh assembler, and captures the resulting authority-bound replay gates.
