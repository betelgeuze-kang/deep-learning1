...
- v32 is implemented as the GitHub Actions third-party rerun kit above v31/v22/v18. `.github/workflows/third-party-rerun.yml` runs the v22 capture script on `ubuntu-24.04`, fills GitHub-hosted reviewer/environment provenance, verifies the return with v18, and uploads the return directory as an artifact using `actions/upload-artifact@v4`. `experiments/test_v32_github_actions_third_party_rerun_kit.sh` verifies the workflow/runbook package while keeping `independent_rerun_actual_ready=0` until an actual GitHub Actions artifact is downloaded and passed to v18.
